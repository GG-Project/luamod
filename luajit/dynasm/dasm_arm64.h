...
static int dasm_imm13(int lo, int hi)
{
    ...
    if (c) {
        w = dasm_ffs(c) - xa;
        if (w == 32)
            m = 0x0000000100000001UL;
        else if (w == 16)
            m = 0x0001000100010001UL;
        else if (w == 8)
            m = 0x0101010101010101UL;
        else if (w == 4)
            m = 0x1111111111111111UL;
        else if (w == 2)
...
#ifdef DASM_CHECKS
                int scale = (p[-2] >> 30);
                CK((!(n & ((1 << scale) - 1)) && (unsigned int)(n >> scale) < 4096) || (unsigned int)(n + 256) < 512, RANGE_I);
#endif
                b[pos++] = n;
                break;
            }
    ...
    for (secnum = 0; secnum < D->maxsection; secnum++) {
        ...
        while (b != endb) {
            ...
            while (1) {
                ...
                case DASM_IMML: {
                    int scale = (p[-2] >> 30);
                    cp[-1] |= (!(n & ((1 << scale) - 1)) && (unsigned int)(n >> scale) < 4096) ? ((n << (10 - scale)) | 0x01000000) : ((n & 511) << 12);
                    break;
                }
                default:
                    *cp++ = ins;
