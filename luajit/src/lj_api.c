...
LUA_API void lua_copy(lua_State *L, int fromidx, int toidx) { copy_slot(L, index2adr(L, fromidx), toidx); }
...
LUA_API const void *lua_topointer(lua_State *L, int idx) { return lj_obj_ptr(index2adr(L, idx)); }
...
LUA_API int lua_isyieldable(lua_State *L) { return cframe_canyield(L->cframe); }
