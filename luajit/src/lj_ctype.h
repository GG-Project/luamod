...
#define ctype_isinteger(info) (((info) & (CTMASK_NUM | CTF_BOOL | CTF_FP)) == CTINFO(CT_NUM, 0))
#define ctype_isinteger_or_bool(info) (((info) & (CTMASK_NUM | CTF_FP)) == CTINFO(CT_NUM, 0))
#define ctype_isbool(info) (((info) & (CTMASK_NUM | CTF_BOOL)) == CTINFO(CT_NUM, CTF_BOOL))
#define ctype_isfp(info) (((info) & (CTMASK_NUM | CTF_FP)) == CTINFO(CT_NUM, CTF_FP))

#define ctype_ispointer(info) ((ctype_type(info) >> 1) == (CT_PTR >> 1)) /* Pointer or array. */
#define ctype_isref(info) (((info) & (CTMASK_NUM | CTF_REF)) == CTINFO(CT_PTR, CTF_REF))

#define ctype_isrefarray(info) (((info) & (CTMASK_NUM | CTF_VECTOR | CTF_COMPLEX)) == CTINFO(CT_ARRAY, 0))
#define ctype_isvector(info) (((info) & (CTMASK_NUM | CTF_VECTOR)) == CTINFO(CT_ARRAY, CTF_VECTOR))
#define ctype_iscomplex(info) (((info) & (CTMASK_NUM | CTF_COMPLEX)) == CTINFO(CT_ARRAY, CTF_COMPLEX))

#define ctype_isvltype(info) (((info) & ((CTMASK_NUM | CTF_VLA) - (2u << CTSHIFT_NUM))) == CTINFO(CT_STRUCT, CTF_VLA)) /* VL array or VL struct. */
#define ctype_isvlarray(info) (((info) & (CTMASK_NUM | CTF_VLA)) == CTINFO(CT_ARRAY, CTF_VLA))

#define ctype_isxattrib(info, at) (((info) & (CTMASK_NUM | CTATTRIB(CTMASK_ATTRIB))) == CTINFO(CT_ATTRIB, CTATTRIB(at)))
...
#define CTINFO_REF(ref) CTINFO(CT_PTR, (CTF_CONST | CTF_REF | CTALIGN_PTR) + (ref))
...
#if LJ_TARGET_PPC
#define CTTYDEFP(_) _(LINT32, 4, CT_NUM, CTF_LONG | CTALIGN(2))
#else
#define CTTYDEFP(_)
#endif
...
/* C lexer keywords. */
#define CTOKDEF(_) _(IDENT, "<identifier>") _(STRING, "<string>") _(INTEGER, "<integer>") _(EOF, "<eof>") _(OROR, "||") _(ANDAND, "&&") _(EQ, "==") _(NE, "!=") _(LE, "<=") _(GE, ">=") _(SHL, "<<") _(SHR, ">>") _(DEREF, "->")

/* Simple declaration specifiers. */
#define CDSDEF(_) _(VOID) _(BOOL) _(CHAR) _(INT) _(FP) _(LONG) _(LONGLONG) _(SHORT) _(COMPLEX) _(SIGNED) _(UNSIGNED) _(CONST) _(VOLATILE) _(RESTRICT) _(INLINE) _(TYPEDEF) _(EXTERN) _(STATIC) _(AUTO) _(REGISTER)

/* C keywords. */
#define CKWDEF(_) CDSDEF(_) _(EXTENSION) _(ASM) _(ATTRIBUTE) _(DECLSPEC) _(CCDECL) _(PTRSZ) _(STRUCT) _(UNION) _(ENUM) _(SIZEOF) _(ALIGNOF)
...
#define LJ_CTYPE_RESTORE(cts) ((cts)->top = savects_.top, memcpy((cts)->hash, savects_.hash, sizeof(savects_.hash)))

/* Check C type ID for validity when assertions are enabled. */
static LJ_AINLINE CTypeID ctype_check(CTState *cts, CTypeID id)
{
    lua_assert(id > 0 && id < cts->top);
    UNUSED(cts);
    return id;
}

/* Get C type for C type ID. */
static LJ_AINLINE CType *ctype_get(CTState *cts, CTypeID id) { return &cts->tab[ctype_check(cts, id)]; }
...
{
    lua_assert(!(ctype_isvoid(ct->info) || ctype_isstruct(ct->info) || ctype_isbitfield(ct->info))); /* These don't have children. */
...
LJ_FUNC CTypeID lj_ctype_getname(CTState *cts, CType **ctp, GCstr *name, uint32_t tmask);
LJ_FUNC CType *lj_ctype_getfieldq(CTState *cts, CType *ct, GCstr *name, CTSize *ofs, CTInfo *qual);
#define lj_ctype_getfield(cts, ct, name, ofs) lj_ctype_getfieldq((cts), (ct), (name), (ofs), NULL)
