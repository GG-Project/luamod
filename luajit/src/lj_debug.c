    ...
    if (p) {
        ...
        for (;;) {
            ...
            if (vn < VARNAME__MAX) {
                ...
                do {
                    p++;
                } while (*(const uint8_t *)p); /* Skip over variable name. */
            }
            p++;
...
static TValue *debug_localname(lua_State *L, const lua_Debug *ar, const char **name, BCReg slot1)
{
    ...
    if (pc != NO_BCPOS && (*name = debug_varname(funcproto(fn), pc, slot1 - 1)) != NULL)
        ;
        ...
        if (bcmode_a(op) == BCMbase) {
            ...
            case BC_TGETS:
                *name = strdata(gco2str(proto_kgc(pt, ~(ptrdiff_t)bc_c(ins))));
                if (ip > proto_bc(pt)) {
                    BCIns insp = ip[-1];
                    if (bc_op(insp) == BC_MOV && bc_a(insp) == ra + 1 + LJ_FR2 && bc_d(insp) == bc_b(ins))
                        return "method";
                }
                return "field";
            case BC_UGET:
...
int lj_debug_getinfo(lua_State *L, const char *what, lj_Debug *ar, int ext)
{
    ...
    if (*what == '>') {
        ...
        lua_assert(frame <= tvref(L->maxstack) && (!nextframe || nextframe <= tvref(L->maxstack)));
        fn = frame_func(frame);
    ...
    if (opt_L) {
        if (isluafunc(fn)) {
            ...
            if (lineinfo) {
                ...
                for (i = 0; i < szl; i++) {
                    BCLine line = first + (sz == 1 ? (BCLine)((const uint8_t *)lineinfo)[i] : sz == 2 ? (BCLine)((const uint16_t *)lineinfo)[i] : (BCLine)((const uint32_t *)lineinfo)[i]);
...
LUA_API int lua_getinfo(lua_State *L, const char *what, lua_Debug *ar) { return lj_debug_getinfo(L, what, (lj_Debug *)ar, 0); }
                ...
                case 'l': /* Dump module:line. */
                    if (isluafunc(fn)) {
                        ...
                        if (debug_putchunkname(sb, pt, pathstrip)) {
                            ...
                            BCLine line = c == 'l' ? debug_frameline(L, fn, nextframe) : pt->firstline;
                            lj_buf_putb(sb, ':');
