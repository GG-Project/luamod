...
static IRRef split_call_l(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir, IRCallID id)
{
...
static IRRef split_call_li(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir, IRCallID id)
{
...
static IRRef split_call_ll(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *ir, IRCallID id)
{
    IRRef tmp, op1 = ir->op1, op2 = ir->op2;
    J->cur.nins--;
#if LJ_LE
...
#endif
    ir->prev = tmp = split_emit(J, IRTI(IR_CALLN), tmp, id);
    return split_emit(J, IRT(IR_HIOP, (LJ_SOFTFP && irt_isnum(ir->t)) ? IRT_SOFTFP : IRT_INT), tmp, tmp);
}
...
#if LJ_HASFFI
static IRRef split_bitshift(jit_State *J, IRRef1 *hisubst, IRIns *oir, IRIns *nir, IRIns *ir)
{
    IROp op = ir->o;
    IRRef kref = nir->op2;
    ...
    return split_call_li(J, hisubst, oir, ir, op - IR_BSHL + IRCALL_lj_carith_shl64);
}

static IRRef split_bitop(jit_State *J, IRRef1 *hisubst, IRIns *nir, IRIns *ir)
{
    IROp op = ir->o;
...
static void split_ir(jit_State *J)
{
    ...
    for (ref = REF_FIRST; ref < nins; ref++) {
        ...
        if (irt_isnum(ir->t)) {
            ...
            case IR_FPMATH:
                ...
                if (nir->op2 == IRFPM_EXP2 && nir->op1 > J->loopref) {
                    ...
                    if (irp->o == IR_CALLN && irp->op2 == IRCALL_softfp_mul) {
                        ...
                        if (irm12->op1 > J->loopref && irl1->o == IR_CALLN && irl1->op2 == IRCALL_lj_vm_log2) {
            ...
            case IR_ABS:
                ...
                hi = split_emit(J, IRT(ir->o == IR_NEG ? IR_BXOR : IR_BAND, IRT_SOFTFP), hisubst[ir->op1], lj_ir_kint(J, (int32_t)(0x7fffffffu + (ir->o == IR_NEG))));
                break;
            case IR_SLOAD:
                if ((nir->op2 & IRSLOAD_CONVERT)) { /* Convert from int to number. */
                    nir->op2 &= ~IRSLOAD_CONVERT;
                    ir->prev = nref = split_emit(J, IRTI(IR_CALLN), nref, IRCALL_softfp_i2d);
            ...
            case IR_CONV: { /* Conversion to number. Others handled below. */
                ...
                if (st == IRT_I64 || st == IRT_U64) {
                    hi = split_call_l(J, hisubst, oir, ir, st == IRT_I64 ? IRCALL_fp64_l2d : IRCALL_fp64_ul2d);
                    break;
                }
#endif
                lua_assert(st == IRT_INT || (LJ_32 && LJ_HASFFI && (st == IRT_U32 || st == IRT_FLOAT)));
                nir->o = IR_CALLN;
#if LJ_32 && LJ_HASFFI
                nir->op2 = st == IRT_INT ? IRCALL_softfp_i2d : st == IRT_FLOAT ? IRCALL_softfp_f2d : IRCALL_softfp_ui2d;
#else
            ...
            case IR_PHI:
                ...
                if (hisubst[ir->op1] != hisubst[ir->op2])
                    split_emit(J, IRT(IR_PHI, IRT_SOFTFP), hisubst[ir->op1], hisubst[ir->op2]);
                break;
            case IR_HIOP:
            ...
            default:
                ...
                hi = split_emit(J, IRTG(IR_HIOP, IRT_SOFTFP), hisubst[ir->op1], hisubst[ir->op2]);
                break;
            }
        } else
#endif
            ...
            case IR_MUL:
                ...
                break;
            case IR_DIV:
                hi = split_call_ll(J, hisubst, oir, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_divi64 : IRCALL_lj_carith_divu64);
                break;
            case IR_MOD:
                hi = split_call_ll(J, hisubst, oir, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_modi64 : IRCALL_lj_carith_modu64);
                break;
            case IR_POW:
                hi = split_call_ll(J, hisubst, oir, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_powi64 : IRCALL_lj_carith_powu64);
                break;
            case IR_BNOT:
            ...
            case IR_CONV: { /* Conversion to 64 bit integer. Others handled below. */
                ...
                if (st == IRT_NUM) { /* NUM to 64 bit int conv. */
                    hi = split_call_l(J, hisubst, oir, ir, irt_isi64(ir->t) ? IRCALL_fp64_d2l : IRCALL_fp64_d2ul);
            ...
            case IR_PHI: {
                IRRef hiref2;
                if ((irref_isk(nir->op1) && irref_isk(nir->op2)) || nir->op1 == nir->op2)
                    J->cur.nins--; /* Drop useless PHIs. */
...
#if LJ_SOFTFP
                if (irt_isfloat(ir->t)) {
                    split_call_l(J, hisubst, oir, ir, st == IRT_I64 ? IRCALL_fp64_l2f : IRCALL_fp64_ul2f);
                    J->cur.nins--; /* Drop unused HIOP. */
                }
#else
                if (irt_isfp(ir->t)) { /* 64 bit integer to FP conversion. */
                    ir->prev = split_emit(J, IRT(IR_HIOP, irt_type(ir->t)), hisubst[ir->op1], nref);
                }
#endif
...
#if LJ_32 && LJ_HASFFI
                        st == IRT_NUM ? (irt_isint(ir->t) ? IRCALL_softfp_d2i : IRCALL_softfp_d2ui) : (irt_isint(ir->t) ? IRCALL_softfp_f2i : IRCALL_softfp_f2ui)
#else
                        IRCALL_softfp_d2i
#endif
                    );
        ...
        split_call:
            ...
            if (LJ_SOFTFP ? irt_is64(ir->t) : irt_isint64(ir->t))
                hi = split_emit(J, IRT(IR_HIOP, (LJ_SOFTFP && irt_isnum(ir->t)) ? IRT_SOFTFP : IRT_INT), nref, nref);
    ...
    for (ref = J->cur.nins - 1; ref >= REF_FIRST; ref--) {
        ...
        if (ir->o != IR_PHI)
            break;
        if (!irref_isk(ir->op1))
            irt_setphi(IR(ir->op1)->t);
        if (ir->op2 > J->loopref)
            irt_setphi(IR(ir->op2)->t);
    }
}
...
static int split_needsplit(jit_State *J)
{
    ...
    for (ref = J->chain[IR_CONV]; ref; ref = IR(ref)->prev) {
        ...
        if ((LJ_SOFTFP && (st == IRT_NUM || st == IRT_FLOAT)) || st == IRT_I64 || st == IRT_U64)
            return 1;
    }
    return 0; /* Nope. */
}
