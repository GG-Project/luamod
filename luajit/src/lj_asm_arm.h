...
static uint32_t asm_fuseopm(ASMState *as, ARMIns ai, IRRef ref, RegSet allow)
{
    ...
    if (ra_hasreg(ir->r)) {
        ...
        if (ir->o >= IR_BSHL && ir->o <= IR_BROR) {
            ...
            ARMShift sh = ir->o == IR_BSHL ? ARMSH_LSL : ir->o == IR_BSHR ? ARMSH_LSR : ir->o == IR_BSAR ? ARMSH_ASR : ARMSH_ROR;
            if (irref_isk(ir->op2)) {
...
{
    IRIns *ir = IR(ref);
    if (ra_noreg(ir->r) && mayfuse(as, ref) && ir->o == IR_BSHL && irref_isk(ir->op2) && IR(ir->op2)->i == 2)
        return ir->op1;
    return 0; /* No fusion. */
}

/* Fuse XLOAD/XSTORE reference into load/store operand. */
static void asm_fusexref(ASMState *as, ARMIns ai, Reg rd, IRRef ref, RegSet allow, int32_t ofs)
{
    IRIns *ir = IR(ref);
    Reg base;
    if (ra_noreg(ir->r) && canfuse(as, ir)) {
        int32_t lim = (!LJ_SOFTFP && (ai & 0x08000000)) ? 1024 : (ai & 0x04000000) ? 4096 : 256;
        if (ir->o == IR_ADD) {
            int32_t ofs2;
            if (irref_isk(ir->op2) && (ofs2 = ofs + IR(ir->op2)->i) > -lim && ofs2 < lim && (!(!LJ_SOFTFP && (ai & 0x08000000)) || !(ofs2 & 3))) {
                ofs = ofs2;
                ref = ir->op1;
...
static int asm_fusemadd(ASMState *as, IRIns *ir, ARMIns ai, ARMIns air)
{
    ...
    if (lref != rref && ((mayfuse(as, lref) && (irm = IR(lref), irm->o == IR_MUL) && ra_noreg(irm->r)) || (mayfuse(as, rref) && (irm = IR(rref), irm->o == IR_MUL) && (rref = lref, ai = air, ra_noreg(irm->r))))) {
        ...
        Reg right, left = ra_alloc2(as, irm, rset_exclude(rset_exclude(RSET_FPR, dest), add));
        right = (left >> 8);
        left &= 255;
        emit_dnm(as, ai, (dest & 15), (left & 15), (right & 15));
        if (dest != add)
            emit_dm(as, ARMI_VMOV_D, (dest & 15), (add & 15));
        return 1;
    }
    return 0;
}
...
static void asm_retf(ASMState *as, IRIns *ir)
{
    ...
    emit_nm(as, ARMI_CMP, RID_TMP, ra_allock(as, i32ptr(pc), rset_exclude(RSET_GPR, base)));
...
static void asm_conv(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isfp(ir->t)) {
        ...
        if (stfp) { /* FP to FP conversion. */
            emit_dm(as, st == IRT_NUM ? ARMI_VCVT_F32_F64 : ARMI_VCVT_F64_F32, (dest & 15), (ra_alloc1(as, lref, RSET_FPR) & 15));
        } else { /* Integer to FP conversion. */
            Reg left = ra_alloc1(as, lref, RSET_GPR);
            ARMIns ai = irt_isfloat(ir->t) ? (st == IRT_INT ? ARMI_VCVT_F32_S32 : ARMI_VCVT_F32_U32) : (st == IRT_INT ? ARMI_VCVT_F64_S32 : ARMI_VCVT_F64_U32);
        ...
        if (irt_isguard(ir->t)) {
            ...
            ARMIns ai;
            emit_dn(as, ARMI_VMOV_R_S, dest, (tmp & 15));
            ai = irt_isint(ir->t) ? (st == IRT_NUM ? ARMI_VCVT_S32_F64 : ARMI_VCVT_S32_F32) : (st == IRT_NUM ? ARMI_VCVT_U32_F64 : ARMI_VCVT_U32_F32);
...
#endif
    {
        ...
        if (st >= IRT_I8 && st <= IRT_U16) { /* Extend to 32 bit integer. */
            ...
            if ((as->flags & JIT_F_ARMV6)) {
                ARMIns ai = st == IRT_I8 ? ARMI_SXTB : st == IRT_U8 ? ARMI_UXTB : st == IRT_I16 ? ARMI_SXTH : ARMI_UXTH;
...
#if LJ_SOFTFP
    if (destused) {
        if (ra_hasspill(ir->s) && ra_hasspill((ir + 1)->s) && (ir->s & 1) == 0 && ir->s + 1 == (ir + 1)->s) {
            int i;
            for (i = 0; i < 2; i++) {
                Reg r = (ir + i)->r;
                if (ra_hasreg(r)) {
    ...
    *l_loop = ARMF_CC(ARMI_B, CC_NE) | ((as->mcp - l_loop - 2) & 0x00ffffffu);
    ...
    if (khash == 0) {
        ...
        if (irt_isstr(kt)) { /* Fetch of str->hash is cheaper than ra_allock. */
            ...
            emit_opk(as, ARMI_AND, tmp, RID_TMP, (int32_t)khash, rset_exclude(rset_exclude(RSET_GPR, tab), dest));
            emit_lso(as, ARMI_LDR, dest, tab, (int32_t)offsetof(GCtab, node));
            emit_lso(as, ARMI_LDR, RID_TMP, tab, (int32_t)offsetof(GCtab, hmask));
            ...
            emit_lso(as, ARMI_LDR, dest, tab, (int32_t)offsetof(GCtab, node));
            emit_dnm(as, ARMI_EOR | ARMF_SH(ARMSH_ROR, 32 - ((HASH_ROT2 + HASH_ROT1) & 31)), tmp, tmp + 1, tmp);
            emit_lso(as, ARMI_LDR, RID_TMP, tab, (int32_t)offsetof(GCtab, hmask));
...
#if !LJ_SOFTFP
                emit_dnm(as, ARMI_VMOV_RR_D, key, keynumhi, (ra_alloc1(as, refkey, RSET_FPR) & 15));
#endif
            } else {
                emit_dnm(as, ARMI_EOR, tmp + 1, tmp, key);
                emit_opk(as, ARMI_ADD, tmp, key, (int32_t)HASH_BIAS, rset_exclude(rset_exclude(RSET_GPR, tab), key));
            }
        }
    }
}
...
static void asm_hrefk(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isnum(irkey->t)) {
        emit_opk(as, ARMF_CC(ARMI_CMP, CC_EQ), 0, type, (int32_t)ir_knum(irkey)->u32.hi, allow);
        emit_opk(as, ARMI_CMP, 0, key, (int32_t)ir_knum(irkey)->u32.lo, allow);
    } else {
        if (ra_hasreg(key))
    ...
    if (ra_hasreg(key))
        emit_lso(as, ARMI_LDR, key, idx, kofs);
    if (ofs > 4095)
...
static void asm_uref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ir->op1)) {
        ...
        if (ir->o == IR_UREFC) {
            ...
            emit_opk(as, ARMI_ADD, dest, uv, (int32_t)offsetof(GCupval, tv), RSET_GPR);
            emit_lso(as, ARMI_LDRB, RID_TMP, uv, (int32_t)offsetof(GCupval, closed));
        } else {
            emit_lso(as, ARMI_LDR, dest, uv, (int32_t)offsetof(GCupval, v));
        }
        emit_lso(as, ARMI_LDR, uv, func, (int32_t)offsetof(GCfuncL, uvptr) + 4 * (int32_t)(ir->op2 >> 8));
    }
}
...
static void asm_strref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ref)) {
        ...
        if (ra_hasreg(irr->r)) {
            ra_noweak(as, irr->r);
            right = irr->r;
        } else if (mayfuse(as, irr->op2) && irr->o == IR_ADD && irref_isk(irr->op2) && (k = emit_isk12(ARMI_ADD, (int32_t)sizeof(GCstr) + IR(irr->op2)->i))) {
            m = k;
    ...
    emit_opk(as, ARMI_ADD, dest, r, sizeof(GCstr) + IR(refk)->i, rset_exclude(RSET_GPR, r));
}
...
{
    if (ir->r != RID_SINK) {
        Reg src = ra_alloc1(as, ir->op2, (!LJ_SOFTFP && irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR);
...
static void asm_ahuvload(ASMState *as, IRIns *ir)
{
    ...
    idx = asm_fuseahuref(as, ir->op1, &ofs, allow, (!LJ_SOFTFP && t == IRT_NUM) ? 1024 : 4096);
    if (!hiop || type == RID_NONE) {
        rset_clear(allow, idx);
        if (ofs < 256 && ra_hasreg(dest) && (dest & 1) == 0 && rset_test((as->freeset & allow), dest + 1)) {
            type = dest + 1;
            ra_modified(as, type);
        } else {
            type = RID_TMP;
...
static void asm_ahustore(ASMState *as, IRIns *ir)
{
    if (ir->r != RID_SINK) {
...
#endif
        {
            ...
            if (ra_hasreg(src))
                emit_lso(as, ARMI_STR, src, idx, ofs);
    ...
    if (ra_hasreg(dest)) {
        ...
        if (t == IRT_NUM) {
            if (ofs < 1024) {
                ...
                if (ra_hasreg(type))
                    emit_lso(as, ARMI_LDR, type, base, ofs + 4);
...
static void asm_tbar(ASMState *as, IRIns *ir)
{
    ...
    Reg gr = ra_allock(as, i32ptr(J2G(as->J)), rset_exclude(rset_exclude(RSET_GPR, tab), link));
    Reg mark = RID_TMP;
    ...
    emit_lso(as, ARMI_STR, tab, gr, (int32_t)offsetof(global_State, gc.grayagain));
    emit_dn(as, ARMI_BIC | ARMI_K12 | LJ_GC_BLACK, mark, mark);
    emit_lso(as, ARMI_LDR, link, gr, (int32_t)offsetof(global_State, gc.grayagain));
    emit_branch(as, ARMF_CC(ARMI_B, CC_EQ), l_end);
...
static void asm_obar(ASMState *as, IRIns *ir)
{
    ...
    emit_lso(as, ARMI_LDRB, tmp, obj, (int32_t)offsetof(GCupval, marked) - (int32_t)offsetof(GCupval, tv));
...
static void asm_callround(ASMState *as, IRIns *ir, int id)
{
    ...
    RegSet drop = RID2RSET(RID_R0) | RID2RSET(RID_R1) | RID2RSET(RID_R2) | RID2RSET(RID_R3) | RID2RSET(RID_R12);
    RegSet of;
    Reg dest, src;
    ra_evictset(as, drop);
    dest = ra_dest(as, ir, RSET_FPR);
    emit_dnm(as, ARMI_VMOV_D_RR, RID_RETLO, RID_RETHI, (dest & 15));
    emit_call(as, id == IRFPM_FLOOR ? (void *)lj_vm_floor_sf : id == IRFPM_CEIL ? (void *)lj_vm_ceil_sf : (void *)lj_vm_trunc_sf);
    /* Workaround to protect argument GPRs from being used for remat. */
    of = as->freeset;
...
static int asm_swapops(ASMState *as, IRRef lref, IRRef rref)
{
    ...
    ir = IR(rref);
    if ((ir->o >= IR_BSHL && ir->o <= IR_BROR) || (ir->o == IR_ADD && ir->op1 == ir->op2))
...
static void asm_intmin_max(ASMState *as, IRIns *ir, int cc)
{
    ...
    if (irref_isk(ir->op2)) {
        ...
        if (kcmp)
            kmov = emit_isk12(ARMI_MOV, IR(ir->op2)->i);
    }
    if (!kmov) {
        kcmp = 0;
    ...
    if (dest != left)
        emit_dm(as, ARMF_CC(ARMI_MOV, cc), dest, left);
...
static void asm_sfpmin_max(ASMState *as, IRIns *ir, int cc)
{
    ...
    if (!rset_test(as->freeset, RID_R2) && regcost_ref(as->cost[RID_R2]) == args[2])
        rset_clear(drop, RID_R2);
    if (!rset_test(as->freeset, RID_R3) && regcost_ref(as->cost[RID_R3]) == args[3])
        rset_clear(drop, RID_R3);
    ra_evictset(as, drop);
    ra_destpair(as, ir);
...
static void asm_fpmin_max(ASMState *as, IRIns *ir, int cc)
{
    ...
    left &= 15;
    if (dest != left)
        emit_dm(as, ARMF_CC(ARMI_VMOV_D, cc ^ 1), dest, left);
    if (dest != right)
        emit_dm(as, ARMF_CC(ARMI_VMOV_D, cc), dest, right);
    emit_d(as, ARMI_VMRS, 0);
...
static void asm_sfpcomp(ASMState *as, IRIns *ir)
{
    ...
    for (r = RID_R0; r <= RID_R3; r++)
        if (!rset_test(as->freeset, r) && regcost_ref(as->cost[r]) == args[r - RID_R0])
            rset_clear(drop, r);
    ra_evictset(as, drop);
    ...
    if (irref_isk(rref) && IR(rref)->i == 0) {
        ...
        if (cmpprev0 && irl->o == IR_BAND && !ra_used(irl)) {
            ...
            if (cc == CC_GE)
                ...
                goto notst; /* Other conds don't work with tst. */
            bleft = ra_alloc1(as, blref, RSET_GPR);
            if (!m2)
                m2 = asm_fuseopm(as, 0, brref, rset_exclude(RSET_GPR, bleft));
            asm_guardcc(as, cc);
...
static void asm_stack_check(ASMState *as, BCReg topslot, IRIns *irp, RegSet allow, ExitNo exitno)
{
    ...
    emit_lso(as, ARMI_LDR, RID_TMP, RID_TMP, (int32_t)offsetof(lua_State, maxstack));
...
static void asm_stack_restore(ASMState *as, SnapShot *snap)
{
    ...
    for (n = 0; n < nent; n++) {
        ...
        if (irt_isnum(ir->t)) {
            ...
            tmp = ra_allock(as, (int32_t)ir_knum(ir)->u32.lo, rset_exclude(RSET_GPREVEN, RID_BASE));
...
static void asm_gc_check(ASMState *as)
{
    ...
    emit_lso(as, ARMI_LDR, tmp2, tmp1, (int32_t)offsetof(global_State, gc.threshold));
    emit_lso(as, ARMI_LDR, RID_TMP, tmp1, (int32_t)offsetof(global_State, gc.total));
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
    ...
    for (; p < pe; p++) {
        ...
        uint32_t ins = *p;
        if ((ins & 0x0f000000u) == 0x0b000000u && ins < 0xf0000000u && ((ins ^ (px - p)) & 0x00ffffffu) == 0) {
