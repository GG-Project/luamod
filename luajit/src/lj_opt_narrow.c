...
static BPropEntry *narrow_bpc_get(jit_State *J, IRRef1 key, IRRef mode)
{
    ...
    for (i = 0; i < BPROP_SLOTS; i++) {
        ...
        if (bp->key == key && bp->mode >= mode && ((bp->mode ^ mode) & IRCONV_MODEMASK) == 0)
            return bp;
    }
    return NULL;
}
...
static void narrow_stripov_backprop(NarrowConv *nc, IRRef ref, int depth)
{
    jit_State *J = nc->J;
    IRIns *ir = IR(ref);
    if (ir->o == IR_ADDOV || ir->o == IR_SUBOV || (ir->o == IR_MULOV && (nc->mode & IRCONV_CONVMASK) == IRCONV_ANY)) {
...
static int narrow_conv_backprop(NarrowConv *nc, IRRef ref, int depth)
{
    ...
    while (cref > ref) {
        IRIns *cr = IR(cref);
        if (cr->op1 == ref && (fins->o == IR_TOBIT || ((cr->op2 & IRCONV_MODEMASK) == (nc->mode & IRCONV_MODEMASK) && irt_isguard(cr->t) >= irt_isguard(fins->t)))) {
    ...
    while (next < last) { /* Simple stack machine to process the ins. list. */
        ...
        if (op == NARROW_REF) {
            ...
            *sp++ = emitir_raw(convot, ref, convop2); /* Raw emit avoids a loop. */
            ...
            sp[-1] = emitir(IRT(IR_CONV, IRT_I64), sp[-1], (IRT_I64 << 5) | IRT_INT | IRCONV_SEXT);
        } else if (op == NARROW_INT) {
            lua_assert(next < last);
            *sp++ = nc->t == IRT_I64 ? lj_ir_kint64(J, (int64_t)(int32_t)*next++) : lj_ir_kint(J, *next++);
            ...
            if ((mode & IRCONV_CONVMASK) == IRCONV_INDEX) {
                if (next == last && irref_isk(narrow_ref(sp[0])) && (uint32_t)IR(narrow_ref(sp[0]))->i + 0x40000000u < 0x80000000u)
                    guardot = 0;
...
static TRef narrow_stripov(jit_State *J, TRef tr, int lastop, IRRef mode)
{
    ...
    if (op >= IR_ADDOV && op <= lastop) {
        ...
        if (bp) {
            ...
            tr = emitir(IRT(op - IR_ADDOV + IR_ADD, ((mode & IRCONV_DSTMASK) >> IRCONV_DSH)), op1, op2);
...
TRef LJ_FASTCALL lj_opt_narrow_index(jit_State *J, TRef tr)
{
    ...
    /* Omit some overflow checks for array indexing. See comments above. */
    ir = IR(tref_ref(tr));
    if ((ir->o == IR_ADDOV || ir->o == IR_SUBOV) && irref_isk(ir->op2) && (uint32_t)IR(ir->op2)->i + 0x40000000u < 0x80000000u)
...
TRef LJ_FASTCALL lj_opt_narrow_cindex(jit_State *J, TRef tr)
{
    ...
    /* Undefined overflow semantics allow stripping of ADDOV, SUBOV and MULOV. */
    return narrow_stripov(J, tr, IR_MULOV, LJ_64 ? ((IRT_INTP << 5) | IRT_INT | IRCONV_SEXT) : ((IRT_INTP << 5) | IRT_INT | IRCONV_TOBIT));
}
#endif
...
static int numisint(lua_Number n) { return (n == (lua_Number)lj_num2int(n)); }
...
TRef lj_opt_narrow_arith(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc, IROp op)
{
    ...
    if ((op >= IR_ADD && op <= (LJ_DUALNUM ? IR_MUL : IR_SUB)) && tref_isinteger(rb) && tref_isinteger(rc) && numisint(lj_vm_foldarith(numberVnum(vb), numberVnum(vc), (int)op - (int)IR_ADD)))
        return emitir(IRTGI((int)op - (int)IR_ADD + (int)IR_ADDOV), rb, rc);
    if (!tref_isnum(rb))
        rb = emitir(IRTN(IR_CONV), rb, IRCONV_NUM_INT);
    if (!tref_isnum(rc))
...
TRef lj_opt_narrow_mod(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc)
{
    ...
    rb = conv_str_tonum(J, rb, vb);
    rc = conv_str_tonum(J, rc, vc);
    if ((LJ_DUALNUM || (J->flags & JIT_F_OPT_NARROW)) && tref_isinteger(rb) && tref_isinteger(rc) && (tvisint(vc) ? intV(vc) != 0 : !tviszero(vc))) {
...
{
    lua_assert(tvisnumber(&tv[FORL_IDX]) && tvisnumber(&tv[FORL_STOP]) && tvisnumber(&tv[FORL_STEP]));
    /* Narrow only if the runtime values of start/stop/step are all integers. */
    if (narrow_forl(J, &tv[FORL_IDX]) && narrow_forl(J, &tv[FORL_STOP]) && narrow_forl(J, &tv[FORL_STEP])) {
