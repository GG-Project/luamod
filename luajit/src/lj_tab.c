...
#if LJ_GC64
#define hashgcref(t, r) hashlohi((t), (uint32_t)gcrefu(r), (uint32_t)(gcrefu(r) >> 32))
#else
...
GCtab *lj_tab_new_ah(lua_State *L, int32_t a, int32_t h) { return lj_tab_new(L, (uint32_t)(a > 0 ? a + 1 : 0), hsize2hbits(h)); }
...
#if LJ_HASFFI
void lj_tab_rehash(lua_State *L, GCtab *t) { rehashtab(L, t, niltv(L)); }
#endif

void lj_tab_reasize(lua_State *L, GCtab *t, uint32_t nasize) { lj_tab_resize(L, t, nasize + 1, t->hmask > 0 ? lj_fls(t->hmask) + 1 : 0); }
...
static MSize unbound_search(GCtab *t, MSize j)
{
    ...
    while ((tv = lj_tab_getint(t, (int32_t)j)) && !tvisnil(tv)) {
        ...
        if (j > (MSize)(INT_MAX - 2)) { /* overflow? */
            ...
            i = 1;
            while ((tv = lj_tab_getint(t, (int32_t)i)) && !tvisnil(tv))
                i++;
            return i - 1;
        }
    }
