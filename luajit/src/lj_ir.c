...
LJ_DATADEF const uint8_t lj_ir_mode[IR__MAX + 1] = { IRDEF(IRMODE) 0 };
...
#define IRCALLCI(cond, name, nargs, kind, type, flags) { (ASMFunction)IRCALLCOND_##cond(name), (nargs) | (CCI_CALL_##kind) | (IRT_##type << CCI_OTSHIFT) | (flags) },
    IRCALLDEF(IRCALLCI)
#undef IRCALLCI
        { NULL, 0 }
};
...
TRef lj_ir_knum_u64(jit_State *J, uint64_t u64) { return lj_ir_k64(J, IR_KNUM, u64); }

/* Intern 64 bit integer constant. */
TRef lj_ir_kint64(jit_State *J, uint64_t u64) { return lj_ir_k64(J, IR_KINT64, u64); }
...
TRef LJ_FASTCALL lj_ir_tostr(jit_State *J, TRef tr)
{
    if (!tref_isstr(tr)) {
        if (!tref_isnumber(tr))
            lj_trace_err(J, LJ_TRERR_BADTYPE);
        tr = emitir(IRT(IR_TOSTR, IRT_STR), tr, tref_isnum(tr) ? IRTOSTR_NUM : IRTOSTR_INT);
    }
    return tr;
}
