...
static Reg asm_fuseahuref(ASMState *as, IRRef ref, int32_t *ofsp, RegSet allow, A64Ins ins)
{
    IRIns *ir = IR(ref);
    if (ra_noreg(ir->r)) {
        if (ir->o == IR_AREF) {
...
static uint32_t asm_fuseopm(ASMState *as, A64Ins ai, IRRef ref, RegSet allow)
{
    ...
    if (ra_hasreg(ir->r)) {
        ...
        if ((ir->o >= IR_BSHL && ir->o <= IR_BSAR && irref_isk(ir->op2)) || (ir->o == IR_ADD && ir->op1 == ir->op2)) {
            A64Shift sh = ir->o == IR_BSHR ? A64SH_LSR : ir->o == IR_BSAR ? A64SH_ASR : A64SH_LSL;
            int shift = ir->o == IR_ADD ? 1 : (IR(ir->op2)->i & (irt_is64(ir->t) ? 63 : 31));
            IRIns *irl = IR(ir->op1);
            if (sh == A64SH_LSL && irl->o == IR_CONV && irl->op2 == ((IRT_I64 << IRCONV_DSH) | IRT_INT | IRCONV_SEXT) && shift <= 4 && canfuse(as, irl)) {
        ...
        } else if (ir->o == IR_CONV && ir->op2 == ((IRT_I64 << IRCONV_DSH) | IRT_INT | IRCONV_SEXT)) {
            Reg m = ra_alloc1(as, ir->op1, allow);
...
static void asm_fusexref(ASMState *as, A64Ins ai, Reg rd, IRRef ref, RegSet allow)
{
    IRIns *ir = IR(ref);
    Reg base;
    int32_t ofs = 0;
    if (ra_noreg(ir->r) && canfuse(as, ir)) {
        if (ir->o == IR_ADD) {
            if (asm_isk32(as, ir->op2, &ofs) && emit_checkofs(ai, ofs)) {
                ...
                if (irl->o == IR_CONV && irl->op2 == ((IRT_I64 << IRCONV_DSH) | IRT_INT | IRCONV_SEXT) && canfuse(as, irl)) {
                    lref = irl->op1;
                    ai |= A64I_LS_SXTWx;
                } else {
                    ai |= A64I_LS_LSLx;
            ...
            if (asm_isk32(as, ir->op2, &ofs)) {
                ...
                IRIns *irr = IR(ir->op2);
                uint32_t m;
                if (irr + 1 == ir && !ra_used(irr) && irr->o == IR_ADD && irref_isk(irr->op2)) {
...
static int asm_fusemadd(ASMState *as, IRIns *ir, A64Ins ai, A64Ins air)
{
    IRRef lref = ir->op1, rref = ir->op2;
    IRIns *irm;
    if (lref != rref && ((mayfuse(as, lref) && (irm = IR(lref), irm->o == IR_MUL) && ra_noreg(irm->r)) || (mayfuse(as, rref) && (irm = IR(rref), irm->o == IR_MUL) && (rref = lref, ai = air, ra_noreg(irm->r))))) {
...
static int asm_fuseorshift(ASMState *as, IRIns *ir)
{
    ...
    if (canfuse(as, irl) && canfuse(as, irr) && ((irl->o == IR_BSHR && irr->o == IR_BSHL) || (irl->o == IR_BSHL && irr->o == IR_BSHR))) {
    ...
    if (ra_used(ir)) {
        ...
        if (irt_isfp(ir->t)) {
            if (ci->flags & CCI_CASTU64) {
                ...
                emit_dn(as, irt_isnum(ir->t) ? A64I_FMOV_D_R : A64I_FMOV_S_R, dest, RID_RET);
            } else {
...
static void asm_retf(ASMState *as, IRIns *ir)
{
    ...
    emit_nm(as, A64I_CMPx, RID_TMP, ra_allock(as, i64ptr(pc), rset_exclude(RSET_GPR, base)));
...
static void asm_conv(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isfp(ir->t)) {
        ...
        if (stfp) { /* FP to FP conversion. */
            emit_dn(as, st == IRT_NUM ? A64I_FCVT_F32_F64 : A64I_FCVT_F64_F32, (dest & 31), (ra_alloc1(as, lref, RSET_FPR) & 31));
        } else { /* Integer to FP conversion. */
            Reg left = ra_alloc1(as, lref, RSET_GPR);
            A64Ins ai = irt_isfloat(ir->t) ? (((IRT_IS64 >> st) & 1) ? (st == IRT_I64 ? A64I_FCVT_F32_S64 : A64I_FCVT_F32_U64) : (st == IRT_INT ? A64I_FCVT_F32_S32 : A64I_FCVT_F32_U32)) : (((IRT_IS64 >> st) & 1) ? (st == IRT_I64 ? A64I_FCVT_F64_S64 : A64I_FCVT_F64_U64) : (st == IRT_INT ? A64I_FCVT_F64_S32 : A64I_FCVT_F64_U32));
        ...
        if (irt_isguard(ir->t)) {
            ...
            Reg dest = ra_dest(as, ir, RSET_GPR);
            A64Ins ai = irt_is64(ir->t) ? (st == IRT_NUM ? (irt_isi64(ir->t) ? A64I_FCVT_S64_F64 : A64I_FCVT_U64_F64) : (irt_isi64(ir->t) ? A64I_FCVT_S64_F32 : A64I_FCVT_U64_F32)) : (st == IRT_NUM ? (irt_isint(ir->t) ? A64I_FCVT_S32_F64 : A64I_FCVT_U32_F64) : (irt_isint(ir->t) ? A64I_FCVT_S32_F32 : A64I_FCVT_U32_F32));
            emit_dn(as, ai, dest, (left & 31));
        }
    } else if (st >= IRT_I8 && st <= IRT_U16) { /* Extend to 32 bit integer. */
        Reg dest = ra_dest(as, ir, RSET_GPR);
        Reg left = ra_alloc1(as, lref, RSET_GPR);
        A64Ins ai = st == IRT_I8 ? A64I_SXTBw : st == IRT_U8 ? A64I_UXTBw : st == IRT_I16 ? A64I_SXTHw : A64I_UXTHw;
    ...
    if (khash == 0) {
        ...
        if (isk) {
            ...
            if (irt_isnum(kt)) {
                ...
                emit_dnm(as, A64I_EORw, dest, dest, ra_allock(as, irt_toitype(kt) << 15, allow));
                emit_dn(as, A64I_LSRx | A64F_IMMR(32) | A64F_IMMS(32), dest, dest);
...
static void asm_uref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ir->op1)) {
        ...
        if (ir->o == IR_UREFC) {
            ...
            emit_opk(as, A64I_ADDx, dest, uv, (int32_t)offsetof(GCupval, tv), RSET_GPR);
            emit_lso(as, A64I_LDRB, RID_TMP, uv, (int32_t)offsetof(GCupval, closed));
        } else {
            emit_lso(as, A64I_LDRx, dest, uv, (int32_t)offsetof(GCupval, v));
        }
        emit_lso(as, A64I_LDRx, uv, func, (int32_t)offsetof(GCfuncL, uvptr) + 8 * (int32_t)(ir->op2 >> 8));
    }
}
...
static A64Ins asm_fxloadins(IRIns *ir)
{
    ...
    case IRT_I8:
        return A64I_LDRB ^ A64I_LS_S;
    case IRT_U8:
        return A64I_LDRB;
    case IRT_I16:
        return A64I_LDRH ^ A64I_LS_S;
    case IRT_U16:
        return A64I_LDRH;
    case IRT_NUM:
        return A64I_LDRd;
    case IRT_FLOAT:
        return A64I_LDRs;
    default:
        return irt_is64(ir->t) ? A64I_LDRx : A64I_LDRw;
    }
}
...
static A64Ins asm_fxstoreins(IRIns *ir)
{
    ...
    case IRT_FLOAT:
        return A64I_STRs;
    default:
        return irt_is64(ir->t) ? A64I_STRx : A64I_STRw;
    }
}
...
static void asm_xstore(ASMState *as, IRIns *ir)
{
    if (ir->r != RID_SINK) {
        ...
        asm_fusexref(as, asm_fxstoreins(ir), src, ir->op1, rset_exclude(RSET_GPR, src));
    }
}
...
static void asm_ahuvload(ASMState *as, IRIns *ir)
{
    ...
    lua_assert(irt_isnum(ir->t) || irt_ispri(ir->t) || irt_isaddr(ir->t) || irt_isint(ir->t));
    if (ra_used(ir)) {
        Reg dest = ra_dest(as, ir, allow);
    ...
    if (irt_type(ir->t) >= IRT_NUM) {
        ...
        emit_nm(as, A64I_CMPx | A64F_SH(A64SH_LSR, 32), ra_allock(as, LJ_TISNUM << 15, rset_exclude(gpr, idx)), tmp);
        ...
        emit_nm(as, A64I_CMPx | A64F_SH(A64SH_LSR, 32), ra_allock(as, (irt_toitype(ir->t) << 15) | 0x7fff, allow), tmp);
    }
    if (ofs & FUSE_REG)
...
static void asm_ahustore(ASMState *as, IRIns *ir)
{
    if (ir->r != RID_SINK) {
        ...
        if (irt_isnum(ir->t)) {
            ...
            idx = asm_fuseahuref(as, ir->op1, &ofs, rset_exclude(allow, type), A64I_STRx);
            if (ofs & FUSE_REG)
...
dotypecheck:
    ...
    if ((ir->op2 & IRSLOAD_TYPECHECK)) {
        ...
        if (irt_type(t) >= IRT_NUM) {
            ...
            emit_nm(as, A64I_CMPx | A64F_SH(A64SH_LSR, 32), ra_allock(as, LJ_TISNUM << 15, allow), tmp);
        } else if (irt_isnil(t)) {
            emit_n(as, (A64I_CMNx ^ A64I_K12) | A64F_U12(1), tmp);
        } else if (irt_ispri(t)) {
            emit_nm(as, A64I_CMPx, ra_allock(as, ~((int64_t)~irt_toitype(t) << 47), allow), tmp);
        } else {
    ...
    if (ra_hasreg(dest)) {
        emit_lso(as, irt_isnum(t) ? A64I_LDRd : (irt_isint(t) ? A64I_LDRw : A64I_LDRx), (dest & 31), base, ofs ^ ((LJ_BE && irt_isint(t) ? 4 : 0)));
    }
}
...
static void asm_cnew(ASMState *as, IRIns *ir)
{
    ...
    /* Initialize gct and ctypeid. lj_mem_newgco() already sets marked. */
    {
        ...
        if (id < 65536)
            emit_d(as, A64I_MOVZw | A64F_U16(id), RID_X1);
    }
    args[0] = ASMREF_L; /* lua_State *L */
...
static void asm_tbar(ASMState *as, IRIns *ir)
{
    ...
    Reg gr = ra_allock(as, i64ptr(J2G(as->J)), rset_exclude(rset_exclude(RSET_GPR, tab), link));
    Reg mark = RID_TMP;
    ...
    emit_lso(as, A64I_STRx, tab, gr, (int32_t)offsetof(global_State, gc.grayagain));
    emit_dn(as, A64I_ANDw ^ emit_isk13(~LJ_GC_BLACK, 0), mark, mark);
    emit_lso(as, A64I_LDRx, link, gr, (int32_t)offsetof(global_State, gc.grayagain));
    emit_cond_branch(as, CC_EQ, l_end);
...
static void asm_obar(ASMState *as, IRIns *ir)
{
    ...
    emit_lso(as, A64I_LDRB, tmp, obj, (int32_t)offsetof(GCupval, marked) - (int32_t)offsetof(GCupval, tv));
...
static void asm_fpmath(ASMState *as, IRIns *ir)
{
    ...
    if (fpm == IRFPM_SQRT) {
        ...
        asm_fpunary(as, ir, fpm == IRFPM_FLOOR ? A64I_FRINTMd : fpm == IRFPM_CEIL ? A64I_FRINTPd : A64I_FRINTZd);
...
static int asm_swapops(ASMState *as, IRRef lref, IRRef rref)
{
    ...
    ir = IR(rref);
    if ((ir->o >= IR_BSHL && ir->o <= IR_BSAR) || (ir->o == IR_ADD && ir->op1 == ir->op2) || (ir->o == IR_CONV && ir->op2 == ((IRT_I64 << IRCONV_DSH) | IRT_INT | IRCONV_SEXT)))
...
static void asm_borbxor(ASMState *as, IRIns *ir, A64Ins ai)
{
    ...
    if ((canfuse(as, irl) && irl->o == IR_BNOT && !irref_isk(rref)) || (canfuse(as, irr) && irr->o == IR_BNOT && !irref_isk(lref))) {
        Reg left, dest = ra_dest(as, ir, RSET_GPR);
        uint32_t m;
        if (irl->o == IR_BNOT) {
            IRRef tmp = lref;
...
static void asm_bitshift(ASMState *as, IRIns *ir, A64Ins ai, A64Shift sh)
{
    ...
    if (irref_isk(ir->op2)) { /* Constant shifts. */
        ...
        switch (sh) {
        case A64SH_LSL:
            emit_dn(as, ai | A64F_IMMS(shmask - shift) | A64F_IMMR((shmask - shift + 1) & shmask), dest, left);
            break;
        case A64SH_LSR:
        case A64SH_ASR:
...
static void asm_intcomp(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(rref) && get_k64val(IR(rref)) == 0) {
        ...
        if (cc == CC_GE)
            ...
            goto nocombine; /* Other conds don't work with tst. */
        cmpprev0 = (irl + 1 == ir);
        ...
        if (cmpprev0 && irl->o == IR_BAND && !ra_used(irl)) {
            ...
            if (irref_isk(brref)) {
                ...
                if (k && !(k & (k - 1)) && (cc == CC_EQ || cc == CC_NE)) {
                    asm_guardtnb(as, cc == CC_EQ ? A64I_TBZ : A64I_TBNZ, ra_alloc1(as, blref, RSET_GPR), emit_ctz64(k));
                    return;
                }
...
static void asm_stack_check(ASMState *as, BCReg topslot, IRIns *irp, RegSet allow, ExitNo exitno)
{
    ...
    emit_lso(as, A64I_LDRx, RID_TMP, RID_TMP, (int32_t)offsetof(lua_State, maxstack));
...
static void asm_gc_check(ASMState *as)
{
    ...
    emit_lso(as, A64I_LDRx, tmp2, tmp1, (int32_t)offsetof(global_State, gc.threshold));
    emit_lso(as, A64I_LDRx, RID_TMP, tmp1, (int32_t)offsetof(global_State, gc.total));
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
    ...
    for (; p < pe; p++) {
        ...
        MCode ins = A64I_LE(*p);
        if ((ins & 0xff000000u) == 0x54000000u && ((ins ^ ((px - p) << 5)) & 0x00ffffe0u) == 0) {
            /* Patch bcc exitstub. */
            *p = A64I_LE((ins & 0xff00001fu) | (((target - p) << 5) & 0x00ffffe0u));
            cend = p + 1;
            if (!cstart)
                cstart = p;
        } else if ((ins & 0xfc000000u) == 0x14000000u && ((ins ^ (px - p)) & 0x03ffffffu) == 0) {
            /* Patch b exitstub. */
            *p = A64I_LE((ins & 0xfc000000u) | ((target - p) & 0x03ffffffu));
            cend = p + 1;
            if (!cstart)
                cstart = p;
        } else if ((ins & 0x7e000000u) == 0x34000000u && ((ins ^ ((px - p) << 5)) & 0x00ffffe0u) == 0) {
            /* Patch cbz/cbnz exitstub. */
            *p = A64I_LE((ins & 0xff00001f) | (((target - p) << 5) & 0x00ffffe0u));
            cend = p + 1;
            if (!cstart)
                cstart = p;
        } else if ((ins & 0x7e000000u) == 0x36000000u && ((ins ^ ((px - p) << 5)) & 0x0007ffe0u) == 0) {
