...
static MCode *asm_exitstub_gen(ASMState *as, ExitNo group)
{
    ...
    *mxp++ = 2 * sizeof(void *);
    *(int32_t *)mxp = ptr2addr(J2GG(as->J)->dispatch);
    mxp += 4;
#endif
...
static IRRef asm_fuseabase(ASMState *as, IRRef ref)
{
    ...
    if (irb->o == IR_FLOAD) {
        ...
        if (ira->o == IR_TNEW && ira->op1 <= LJ_MAX_COLOSIZE && !neverfuse(as) && noconflict(as, irb->op1, IR_NEWREF, 1)) {
    ...
    if (irref_isk(ir->op2)) {
        ...
        */
        if (!LJ_64 && /* Has bad effects with negative index on x64. */
            mayfuse(as, ir->op2) && ra_noreg(irx->r) && irx->o == IR_ADD && irref_isk(irx->op2)) {
...
static void asm_fuseahuref(ASMState *as, IRRef ref, RegSet allow)
{
    ...
    if (ra_noreg(ir->r)) {
        ...
        case IR_UREFC:
            ...
            break;
        default:
            lua_assert(ir->o == IR_HREF || ir->o == IR_NEWREF || ir->o == IR_UREFO || ir->o == IR_KKPTR);
            break;
        }
    }
...
static Reg asm_fuseloadk64(ASMState *as, IRIns *ir)
{
    ...
    if (!LJ_GC64 || checki32((intptr_t)k)) {
        ...
        as->mrm.base = RID_DISPATCH;
    } else if (checki32(mcpofs(as, k)) && checki32(mcpofs(as, k + 1)) && checki32(mctopofs(as, k)) && checki32(mctopofs(as, k + 1))) {
...
static Reg asm_fuseload(ASMState *as, IRRef ref, RegSet allow)
{
    ...
    if (ir->o == IR_KNUM) {
        ...
        if (ir->o == IR_SLOAD) {
            if (!(ir->op2 & (IRSLOAD_PARENT | IRSLOAD_CONVERT)) && noconflict(as, ref, IR_RETF, 0) && !(LJ_GC64 && irt_isaddr(ir->t))) {
                as->mrm.base = (uint8_t)ra_alloc1(as, REF_BASE, xallow);
                as->mrm.ofs = 8 * ((int32_t)ir->op1 - 1 - LJ_FR2) + (!LJ_FR2 && (ir->op2 & IRSLOAD_FRAME) ? 4 : 0);
                as->mrm.idx = RID_NONE;
                return RID_MRM;
            }
        } else if (ir->o == IR_FLOAD) {
            /* Generic fusion is only ok for 32 bit operand (but see asm_comp). */
            if ((irt_isint(ir->t) || irt_isu32(ir->t) || irt_isaddr(ir->t)) && noconflict(as, ref, IR_FSTORE, 0)) {
                asm_fusefref(as, ir, xallow);
                return RID_MRM;
            }
        } else if (ir->o == IR_ALOAD || ir->o == IR_HLOAD || ir->o == IR_ULOAD) {
            if (noconflict(as, ref, ir->o + IRDELTA_L2S, 0) && !(LJ_GC64 && irt_isaddr(ir->t))) {
            ...
            */
            if ((!irt_typerange(ir->t, IRT_I8, IRT_U16)) && noconflict(as, ref, IR_XSTORE, 0)) {
    ...
    if (!(as->freeset & allow) && !emit_canremat(ref) && (allow == RSET_EMPTY || ra_hasspill(ir->s) || iscrossref(as, ref)))
        goto fusespill;
    ...
    if ((ci->flags & CCI_VARARG)) { /* Special handling for vararg calls. */
        ...
        for (n = 0; n < 4 && n < nargs; n++) {
            ...
            if (irt_isfp(ir->t)) /* Duplicate FPRs in GPRs. */
                emit_rr(as, XO_MOVDto, (irt_isnum(ir->t) ? REX_64 : 0) | (fpr + n), ((gprs >> (n * 5)) & 31)); /* Either MOVD or MOVQ. */
        }
#else
    ...
    if (ra_used(ir)) {
        if (irt_isfp(ir->t)) {
            ...
            if ((ci->flags & CCI_CASTU64)) {
                ...
                emit_rmro(as, irt_isnum(ir->t) ? XO_FSTPq : XO_FSTPd, irt_isnum(ir->t) ? XOg_FSTPq : XOg_FSTPd, RID_ESP, ofs);
            }
#endif
#if LJ_32
        } else if (hiop) {
...
static void asm_callx(ASMState *as, IRIns *ir)
{
    ...
    if (!(void *)ci.func) {
        ...
        if (LJ_32)
            emit_spsub(as, spadj); /* Above code may cause restores! */
...
{
    Reg dest = ra_dest(as, ir, RSET_GPR);
    Reg tmp = ra_noreg(IR(ir->op1)->r) ? ra_alloc1(as, ir->op1, RSET_FPR) : ra_scratch(as, RSET_FPR);
    Reg right;
...
static void asm_conv(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isfp(ir->t)) {
        ...
        if (stfp) { /* FP to FP conversion. */
            ...
            Reg left = (LJ_64 && (st == IRT_U32 || st == IRT_U64)) ? ra_alloc1(as, lref, RSET_GPR) : asm_fuseloadm(as, lref, RSET_GPR, st64);
            if (LJ_64 && st == IRT_U64) {
            ...
            emit_mrm(as, irt_isnum(ir->t) ? XO_CVTSI2SD : XO_CVTSI2SS, dest | ((LJ_64 && (st64 || st == IRT_U32)) ? REX_64 : 0), left);
        }
        ...
        if (irt_isguard(ir->t)) {
            ...
            if (LJ_64 ? irt_isu64(ir->t) : irt_isu32(ir->t)) {
                ...
                Reg tmp = ra_noreg(IR(lref)->r) ? ra_alloc1(as, lref, RSET_FPR) : ra_scratch(as, RSET_FPR);
                MCLabel l_end = emit_label(as);
                if (LJ_32)
                ...
                emit_mrm(as, op, dest | ((LJ_64 && (irt_is64(ir->t) || irt_isu32(ir->t))) ? REX_64 : 0), asm_fuseload(as, lref, RSET_FPR));
            }
        }
...
static void asm_conv_int64_fp(ASMState *as, IRIns *ir)
{
    ...
    if (ra_used(ir - 1))
        emit_rmro(as, XO_MOV, lo, RID_ESP, 0);
    ...
    emit_mrm(as, st == IRT_NUM ? XO_FLDq : XO_FLDd, st == IRT_NUM ? XOg_FLDq : XOg_FLDd, asm_fuseload(as, ir->op1, RSET_EMPTY));
}
...
static void asm_strto(ASMState *as, IRIns *ir)
{
    ...
    emit_rmro(as, XO_LEA, ra_releasetmp(as, ASMREF_TMP1) | REX_64, RID_ESP, sps_scale(ir->s));
}
...
static void asm_href(ASMState *as, IRIns *ir, IROp merge)
{
    ...
    if (irt_isnum(kt)) {
        if (isk) {
            ...
            emit_gmroi(as, XG_ARITHi(XOg_CMP), dest, offsetof(Node, key.u32.hi), (int32_t)ir_knum(irkey)->u32.hi);
        } else {
        ...
        if (isk) {
            ...
            emit_gmroi(as, XG_ARITHi(XOg_CMP), dest, offsetof(Node, key.u32.lo), k.u32.lo);
            emit_sjcc(as, CC_NE, l_next);
            emit_gmroi(as, XG_ARITHi(XOg_CMP), dest, offsetof(Node, key.u32.hi), k.u32.hi);
        } else {
        ...
        if (!irt_ispri(kt)) {
            ...
            if (isk)
                emit_gmroi(as, XG_ARITHi(XOg_CMP), dest, offsetof(Node, key.gcr), ptr2addr(ir_kgc(irkey)));
            else
...
static void asm_hrefk(ASMState *as, IRIns *ir)
{
    ...
    if (!irt_ispri(irkey->t)) {
        ...
        emit_rmro(as, XO_CMP, key | REX_64, node, ofs + (int32_t)offsetof(Node, key.u64));
        lua_assert(irt_isnum(irkey->t) || irt_isgcv(irkey->t));
        /* Assumes -0.0 is already canonicalized to +0.0. */
        emit_loadu64(as, key,
            irt_isnum(irkey->t) ? ir_knum(irkey)->u64 :
#if LJ_GC64
                                ((uint64_t)irt_toitype(irkey->t) << 47) | (uint64_t)ir_kgc(irkey));
#else
                                ((uint64_t)irt_toitype(irkey->t) << 32) | (uint64_t)(uint32_t)ptr2addr(ir_kgc(irkey)));
#endif
    } else {
        lua_assert(!irt_isnil(irkey->t));
#if LJ_GC64
        emit_i32(as, (irt_toitype(irkey->t) << 15) | 0x7fff);
        emit_rmro(as, XO_ARITHi, XOg_CMP, node, ofs + (int32_t)offsetof(Node, key.it));
#else
        emit_i8(as, irt_toitype(irkey->t));
        emit_rmro(as, XO_ARITHi8, XOg_CMP, node, ofs + (int32_t)offsetof(Node, key.it));
#endif
    }
#else
    ...
    if (irt_isnum(irkey->t)) {
        /* Assumes -0.0 is already canonicalized to +0.0. */
        emit_gmroi(as, XG_ARITHi(XOg_CMP), node, ofs + (int32_t)offsetof(Node, key.u32.lo), (int32_t)ir_knum(irkey)->u32.lo);
        emit_sjcc(as, CC_NE, l_exit);
        emit_gmroi(as, XG_ARITHi(XOg_CMP), node, ofs + (int32_t)offsetof(Node, key.u32.hi), (int32_t)ir_knum(irkey)->u32.hi);
    } else {
        if (!irt_ispri(irkey->t)) {
            lua_assert(irt_isgcv(irkey->t));
            emit_gmroi(as, XG_ARITHi(XOg_CMP), node, ofs + (int32_t)offsetof(Node, key.gcr), ptr2addr(ir_kgc(irkey)));
            emit_sjcc(as, CC_NE, l_exit);
        }
        lua_assert(!irt_isnil(irkey->t));
        emit_i8(as, irt_toitype(irkey->t));
...
static void asm_uref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ir->op1)) {
        ...
        emit_rmro(as, XO_MOV, uv | REX_GC64, func, (int32_t)offsetof(GCfuncL, uvptr) + (int32_t)sizeof(MRef) * (int32_t)(ir->op2 >> 8));
    }
}
...
static void asm_fxstore(ASMState *as, IRIns *ir)
{
    ...
    */
    if (irt_isi16(ir->t) || irt_isu16(ir->t) || irt_isfp(ir->t) || !asm_isk32(as, ir->op2, &k)) {
        RegSet allow8 = irt_isfp(ir->t) ? RSET_FPR : (irt_isi8(ir->t) || irt_isu8(ir->t)) ? RSET_GPR8 : RSET_GPR;
    ...
    if (ra_hasreg(src)) {
        ...
        if (irt_isi8(ir->t) || irt_isu8(ir->t)) {
            ...
            lua_assert(irt_is64(ir->t) || irt_isint(ir->t) || irt_isu32(ir->t) || irt_isaddr(ir->t));
            emit_i32(as, k);
...
static void asm_ahuvload(ASMState *as, IRIns *ir)
{
    ...
    Reg tmp = RID_NONE;
#endif
    lua_assert(irt_isnum(ir->t) || irt_ispri(ir->t) || irt_isaddr(ir->t) || (LJ_DUALNUM && irt_isint(ir->t)));
#if LJ_64 && !LJ_GC64
...
{
    int32_t ofs = 8 * ((int32_t)ir->op1 - 1 - LJ_FR2) + (!LJ_FR2 && (ir->op2 & IRSLOAD_FRAME) ? 4 : 0);
    IRType1 t = ir->t;
    Reg base;
    lua_assert(!(ir->op2 & IRSLOAD_PARENT)); /* Handled by asm_head_side(). */
    lua_assert(irt_isguard(t) || !(ir->op2 & IRSLOAD_TYPECHECK));
    lua_assert(LJ_DUALNUM || !irt_isint(t) || (ir->op2 & (IRSLOAD_CONVERT | IRSLOAD_FRAME)));
...
static void asm_cnew(ASMState *as, IRIns *ir)
{
    ...
    if (ir->o == IR_CNEWI) {
        ...
        if (irref_isk(ir->op2)) {
            IRIns *irk = IR(ir->op2);
            uint64_t k = (irk->o == IR_KINT64 || (LJ_GC64 && (irk->o == IR_KPTR || irk->o == IR_KKPTR))) ? ir_k64(irk)->u64 : (uint64_t)(uint32_t)irk->i;
    ...
    emit_gri(as, XG_ARITHi(XOg_OR), RID_ECX, (int32_t)((~LJ_TCDATA << 8) + (id << 16)));
...
static void asm_x87load(ASMState *as, IRRef ref)
{
    ...
    } else if (ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT && !ra_used(ir) && !irref_isk(ir->op1) && mayfuse(as, ir->op1)) {
        IRIns *iri = IR(ir->op1);
    ...
    if (fpm == IRFPM_SQRT) {
        ...
        if (as->flags & JIT_F_SSE4_1) { /* SSE4.1 has a rounding instruction. */
            ...
            *--as->mcp = 0x66; /* 1st byte of ROUNDSD opcode. */
            ...
            emit_call(as, fpm == IRFPM_FLOOR ? lj_vm_floor_sse : fpm == IRFPM_CEIL ? lj_vm_ceil_sse : lj_vm_trunc_sse);
...
#if LJ_64 && LJ_HASFFI
    if (!irt_isnum(ir->t))
        asm_callid(as, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_powi64 : IRCALL_lj_carith_powu64);
    else
#endif
        asm_fppowi(as, ir);
}
...
static int asm_lea(ASMState *as, IRIns *ir)
{
    ...
    if (ra_hasreg(irl->r)) {
        ...
        if (irref_isk(ir->op2) || ra_hasreg(irr->r)) {
            ...
            if (ra_hasreg(ir->r) && ((irt_isphi(irl->t) && as->phireg[ir->r] == ir->op1) || (irt_isphi(irr->t) && as->phireg[ir->r] == ir->op2)))
                return 0;
            if (irref_isk(ir->op2)) {
                as->mrm.ofs = irr->i;
            } else {
        ...
        } else if (irr->o == IR_ADD && mayfuse(as, ir->op2) && irref_isk(irr->op2)) {
    ...
    } else if (ir->op1 != ir->op2 && irl->o == IR_ADD && mayfuse(as, ir->op1) && (irref_isk(ir->op2) || irref_isk(irl->op2))) {
...
static void asm_add(ASMState *as, IRIns *ir)
{
    ...
    else if ((as->flags & JIT_F_LEA_AGU) || as->flagmcp == as->mcp || irt_is64(ir->t) || !asm_lea(as, ir))
...
#if LJ_64 && LJ_HASFFI
    if (!irt_isnum(ir->t))
        asm_callid(as, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_divi64 : IRCALL_lj_carith_divu64);
    else
#endif
...
#if LJ_64 && LJ_HASFFI
    if (!irt_isint(ir->t))
        asm_callid(as, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_modi64 : IRCALL_lj_carith_modu64);
    else
#endif
...
{
    Reg dest = ra_dest(as, ir, RSET_GPR);
    as->mcp = emit_op(XO_BSWAP + ((dest & 7) << 24), REX_64IR(ir, 0), dest, 0, as->mcp, 1);
    ra_left(as, dest, ir->op1);
}
...
static void asm_bitshift(ASMState *as, IRIns *ir, x86Shift xs, x86Op xv)
{
    ...
    if (irref_isk(rref)) { /* Constant shifts. */
        ...
        case 0:
            break;
        case 1:
            emit_rr(as, XO_SHIFT1, REX_64IR(ir, xs), dest);
            break;
        default:
            emit_shifti(as, REX_64IR(ir, xs), dest, shift);
            break;
        }
        ...
        left = asm_fuseloadm(as, ir->op1, rset_exclude(RSET_GPR, right), irt_is64(ir->t));
...
static void asm_comp(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isnum(ir->t)) {
        ...
        */
        ...
        int32_t imm = 0;
        lua_assert(irt_is64(ir->t) || irt_isint(ir->t) || irt_isu32(ir->t) || irt_isaddr(ir->t) || irt_isu8(ir->t));
        ...
        if (asm_isk32(as, rref, &imm)) {
            ...
            if (usetest && irl->o == IR_BAND && irl + 1 == ir && !ra_used(irl)) {
                ...
                if (!asm_isk32(as, irl->op2, &imm)) {
                    ...
                    if (opisfusableload((IROp)irll->o) && (irt_isi8(irll->t) || irt_isu8(irll->t))) {
            ...
            test_nofuse:
                ...
                Reg left;
                if (opisfusableload((IROp)irl->o) && ((irt_isu8(irl->t) && checku8(imm)) || ((irt_isi8(irl->t) || irt_isi16(irl->t)) && checki8(imm)) || (irt_isu16(irl->t) && checku16(imm) && checki8((int16_t)imm)))) {
                    ...
                    */
                    ...
                    if (left == RID_MRM) { /* Fusion succeeded? */
                        ...
                        asm_guardcc(as, cc);
                        emit_i8(as, imm);
                        emit_mrm(as, (irt_isi8(origt) || irt_isu8(origt)) ? XO_ARITHib : XO_ARITHiw8, r64 + XOg_CMP, RID_MRM);
                        return;
...
static void asm_hiop(ASMState *as, IRIns *ir)
{
    ...
    if (LJ_UNLIKELY(!(as->flags & JIT_F_OPT_DCE)))
        uselo = usehi = 1;
...
#if LJ_GC64
        emit_rmro(as, XO_ARITH(XOg_SUB), r | REX_64, RID_DISPATCH, (int32_t)dispofs(as, &J2G(as->J)->jit_base));
#else
...
static void asm_stack_restore(ASMState *as, SnapShot *snap)
{
    ...
    for (n = 0; n < nent; n++) {
        ...
        if (irt_isnum(ir->t)) {
            ...
            lua_assert(irt_ispri(ir->t) || irt_isaddr(ir->t) || (LJ_DUALNUM && irt_isinteger(ir->t)));
            if (!irref_isk(ref)) {
...
static const uint8_t map_op1[256] = { 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x20, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x10, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x10, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x10, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x45, 0x10, 0x51,
#if LJ_64
    0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x14,
#else
    0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51,
#endif
    0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x92, 0x92, 0x10, 0x10, 0x12, 0x11, 0x45, 0x86, 0x52, 0x93, 0x51, 0x51, 0x51, 0x51, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x93, 0x86, 0x93, 0x93, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x47, 0x51, 0x51, 0x51, 0x51, 0x51,
#if LJ_64
...
#endif
    0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x93, 0x93, 0x53, 0x51, 0x70, 0x71, 0x93, 0x86, 0x54, 0x51, 0x53, 0x51, 0x51, 0x52, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92, 0x52, 0x52, 0x51, 0x51, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x92, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x52, 0x45, 0x45, 0x47, 0x52, 0x51, 0x51, 0x51, 0x51, 0x10, 0x51, 0x10, 0x10, 0x51, 0x51, 0x63, 0x66, 0x51, 0x51, 0x51, 0x51, 0x51, 0x51, 0x92, 0x92 };

static const uint8_t map_op2[256] = {  };
