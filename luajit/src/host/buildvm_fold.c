...
static int tryhash(uint32_t *htab, uint32_t sz, uint32_t r, int dorol)
{
    ...
    for (i = 0; i < nkeys; i++) {
        ...
        uint32_t h = (dorol ? lj_rol(lj_rol(k, r >> 5) - k, r & 31) : (((k << (r >> 5)) - k) << (r & 31))) % sz;
        if (htab[h] != 0xffffffff) { /* Collision on primary slot. */
            if (htab[h + 1] != 0xffffffff) { /* Collision on secondary slot. */
                ...
                if (h < sz - 1 && htab[h + 2] == 0xffffffff) {
                    ...
                    uint32_t h2 = (dorol ? lj_rol(lj_rol(k2, r >> 5) - k2, r & 31) : (((k2 << (r >> 5)) - k2) << (r & 31))) % sz;
                    if (h2 != h + 1)
                        return 0; /* Cannot resolve collision. */
    ...
    for (sz = (nkeys | 1); sz < BUILD_MAX_FOLD * 2; sz += 2) {
        ...
        for (r = 0; r < 32 * 32; r++) {
            if (tryhash(htab, sz, r, 0)) {
                ...
                fprintf(ctx->fp, "#define fold_hashkey(k)\t(((((k)<<%u)-(k))<<%u)%%%u)\n\n", r >> 5, r & 31, sz);
                return;
            }
        }
        ...
        for (r = 0; r < 32 * 32; r++) {
            if (tryhash(htab, sz, r, 1)) {
                printhash(ctx, htab, sz);
                fprintf(ctx->fp, "#define fold_hashkey(k)\t(lj_rol(lj_rol((k),%u)-(k),%u)%%%u)\n\n", r >> 5, r & 31, sz);
                return;
            }
        }
    }
