...
static const char *const jccnames[] = { "jo", "jno", "jb", "jnb", "jz", "jnz", "jbe", "ja", "js", "jns", "jpe", "jpo", "jl", "jge", "jle", "jg" };

/* Emit x86/x64 text relocations. */
static void emit_asm_reloc_text(BuildCtx *ctx, uint8_t *cp, int n, const char *sym)
{
...
static void emit_asm_wordreloc(BuildCtx *ctx, uint8_t *p, int n, const char *sym)
{
    uint32_t ins;
    ...
    if ((ins & 0xff000000u) == 0xfa000000u) {
        ...
        fprintf(ctx->fp, "\t%s%.2s %s\n", (ins & 0x01000000u) ? "bl" : "b", &"eqnecsccmiplvsvchilsgeltgtle"[2 * (ins >> 28)], sym);
    } else {
        fprintf(stderr, "Error: unsupported opcode %08x for %s symbol relocation.\n", ins, sym);
        exit(1);
    }
...
static void emit_asm_label(BuildCtx *ctx, const char *name, int size, int isfunc)
{
    ...
    case BUILD_elfasm:
#if LJ_TARGET_PS3
        if (!strncmp(name, "lj_vm_", 6) && strcmp(name, ctx->beginsym) && !strstr(name, "hook")) {
            fprintf(ctx->fp,
                "\n\t.globl %s\n"
                "\t.section \".opd\",\"aw\"\n"
                "%s:\n"
...
void emit_asm(BuildCtx *ctx)
{
    ...
    for (i = rel = 0; i < ctx->nsym; i++) {
        ...
        while (rel < ctx->nreloc && ctx->reloc[rel].ofs <= next) {
            ...
            int n = r->ofs - ofs;
#if LJ_TARGET_X86ORX64
            if (r->type != 0 && (ctx->mode == BUILD_elfasm || ctx->mode == BUILD_machasm)) {
