...
static void emit_raw(BuildCtx *ctx) { owrite(ctx, ctx->code, ctx->codesz); }
...
static int build_code(BuildCtx *ctx)
{
    ...
    if ((status = dasm_link(Dst, &ctx->codesz)))
        return status;
    ctx->code = (uint8_t *)malloc(ctx->codesz);
    if ((status = dasm_encode(Dst, (void *)ctx->code)))
        return status;
    ...
    for (i = 0; i < ctx->npc; i++) {
        ...
        if (ofs < 0)
            return 0x22000000 | i;
        ctx->bc_ofs[i] = ofs;
        if ((LJ_HASJIT || !(i == BC_JFORI || i == BC_JFORL || i == BC_JITERL || i == BC_JLOOP || i == BC_IFORL || i == BC_IITERL || i == BC_ILOOP)) && (LJ_HASFFI || i != BC_KCDATA))
    ...
    for (i = 0; i < ctx->nglob; i++) {
        ...
        if (!(len >= 2 && gl[len - 2] == '_' && gl[len - 1] == 'Z'))
            sym_insert(ctx, (int32_t)((uint8_t *)(ctx->glob[i]) - ctx->code), LABEL_PREFIX, globnames[i]);
    }
...
static void emit_vmdef(BuildCtx *ctx)
{
    ...
    for (i = 0; bc_names[i]; i++)
        fprintf(ctx->fp, "%-6s", bc_names[i]);
    fprintf(ctx->fp, "\",\n\n");

    fprintf(ctx->fp, "irnames = \"");
    for (i = 0; ir_names[i]; i++)
        fprintf(ctx->fp, "%-6s", ir_names[i]);
    fprintf(ctx->fp, "\",\n\n");
