...
static void gc_mark(global_State *g, GCobj *o)
{
    ...
    if (LJ_UNLIKELY(gct == ~LJ_TUDATA)) {
        ...
        lua_assert(gct == ~LJ_TFUNC || gct == ~LJ_TTAB || gct == ~LJ_TTHREAD || gct == ~LJ_TPROTO || gct == ~LJ_TTRACE);
...
static const GCFreeFunc gc_freefunc[] = { (GCFreeFunc)lj_str_free, (GCFreeFunc)lj_func_freeuv, (GCFreeFunc)lj_state_free, (GCFreeFunc)lj_func_freeproto, (GCFreeFunc)lj_func_free,
#if LJ_HASJIT
    ...
    (GCFreeFunc)0,
#endif
    (GCFreeFunc)lj_tab_free, (GCFreeFunc)lj_udata_free };
    ...
    while (o) {
        ...
        if (t->hmask > 0) {
            ...
            for (i = 0; i <= hmask; i++) {
                ...
                if (!tvisnil(&n->val) && (gc_mayclear(&n->key, 0) || gc_mayclear(&n->val, 1)))
                    setnilV(&n->val);
            }
        }
        o = gcref(t->gclist);
...
{
#define TV2MARKED(x) (*((uint8_t *)(x)-offsetof(GCupval, tv) + offsetof(GCupval, marked)))
    if (g->gc.state == GCSpropagate || g->gc.state == GCSatomic)
        gc_mark(g, gcV(tv));
    else
