...
#define emit_x87op(as, xo) (*(uint16_t *)(as->mcp - 2) = (uint16_t)(xo), as->mcp -= 2)

/* op */
static LJ_AINLINE MCode *emit_op(x86Op xo, Reg rr, Reg rb, Reg rx, MCode *p, int delta)
{
    int n = (int8_t)xo;
...
/* op + modrm */
#define emit_opm(xo, mode, rr, rb, p, delta) (p[(delta)-1] = MODRM((mode), (rr), (rb)), emit_op((xo), (rr), (rb), 0, (p), (delta)))

/* op + modrm + sib */
#define emit_opmx(xo, mode, scale, rr, rb, rx, p) (p[-1] = MODRM((scale), (rx), (rb)), p[-2] = MODRM((mode), (rr), RID_ESP), emit_op((xo), (rr), (rb), (rx), (p), -1))
...
static void emit_rmrxo(ASMState *as, x86Op xo, Reg rr, Reg rb, Reg rx, x86Mode scale, int32_t ofs)
{
    MCode *p = as->mcp;
    x86Mode mode;
...
/* op [base+ofs], i */
static void emit_gmroi(ASMState *as, x86Group xg, Reg rb, int32_t ofs, int32_t i)
{
    x86Op xo;
    if (checki8(i)) {
        emit_i8(as, i);
...
#define emit_shifti(as, xg, r, i) (emit_i8(as, (i)), emit_rr(as, XO_SHIFTi, (Reg)(xg), (r)))
...
/* mov [base+ofs], r */
#define emit_movtomro(as, r, base, ofs) emit_rmro(as, XO_MOVto, (r), (base), (ofs))

/* Get/set global_State fields. */
#define emit_opgl(as, xo, r, field) emit_rma(as, (xo), (r), (void *)&J2G(as->J)->field)
...
#define emit_setvmstate(as, i) (emit_i32(as, i), emit_opgl(as, XO_MOVmi, 0, vmstate))

/* mov r, i / xor r, r */
static void emit_loadi(ASMState *as, Reg r, int32_t i)
{
    /* XOR r,r is shorter, but modifies the flags. This is bad for HIOP. */
    if (i == 0 && !(LJ_32 && (IR(as->curins)->o == IR_HIOP || (as->curins + 1 < as->T->nins && IR(as->curins + 1)->o == IR_HIOP)))) {
...
#if LJ_GC64
#define dispofs(as, k) ((intptr_t)((uintptr_t)(k) - (uintptr_t)J2GG(as->J)->dispatch))
#define mcpofs(as, k) ((intptr_t)((uintptr_t)(k) - (uintptr_t)as->mcp))
#define mctopofs(as, k) ((intptr_t)((uintptr_t)(k) - (uintptr_t)as->mctop))
/* mov r, addr */
#define emit_loada(as, r, addr) emit_loadu64(as, (r), (uintptr_t)(addr))
#else
/* mov r, addr */
#define emit_loada(as, r, addr) emit_loadi(as, (r), ptr2addr((addr)))
#endif
...
#if LJ_GC64
    } else if (checki32((intptr_t)k) || checki32(dispofs(as, k)) || (checki32(mcpofs(as, k)) && checki32(mctopofs(as, k)))) {
...
static void emit_sfixup(ASMState *as, MCLabel source) { source[-1] = (MCode)(as->mcp - source); }
