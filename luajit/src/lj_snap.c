...
static MSize snapshot_slots(jit_State *J, SnapEntry *map, BCReg nslots)
{
    ...
    for (s = 0; s < nslots; s++) {
        ...
        if (ref) {
            ...
            if ((LJ_FR2 || !(sn & (SNAP_CONT | SNAP_FRAME))) && ir->o == IR_SLOAD && ir->op1 == s && ref > retf) {
                /* No need to snapshot unmodified non-inherited slots. */
                if (!(ir->op2 & IRSLOAD_INHERIT))
                    continue;
                /* No need to restore readonly slots and unmodified non-parent slots. */
                if (!(LJ_DUALNUM && (ir->op2 & IRSLOAD_CONVERT)) && (ir->op2 & (IRSLOAD_READONLY | IRSLOAD_PARENT)) != IRSLOAD_PARENT)
                    sn |= SNAP_NORESTORE;
            }
...
void lj_snap_add(jit_State *J)
{
    ...
    if ((nsnap > 0 && J->cur.snap[nsnap - 1].ref == J->cur.nins) || (J->mergesnap && !irt_isguard(J->guardemit))) {
    ...
    for (;;) {
        ...
        handle_jump : {
            ...
            if (op >= BC_FORI && op <= BC_JFORL)
                minslot += FORL_EXT;
            else if (op >= BC_ITERL && op <= BC_JITERL)
                minslot += bc_b(pc[-2]) - 1;
            else if (op == BC_UCLO) {
                pc += bc_j(ins);
                break;
        ...
        case BCMbase:
            if (op >= BC_CALLM && op <= BC_VARG) {
                BCReg top = (op == BC_CALLM || op == BC_CALLMT || bc_c(ins) == 0) ? maxslot : (bc_a(ins) + bc_c(ins) + LJ_FR2);
                if (LJ_FR2)
                    DEF_SLOT(bc_a(ins) + 1);
                ...
                for (s = bc_a(ins); s <= bc_d(ins); s++)
                    DEF_SLOT(s);
            } else if (op == BC_TSETM) {
                for (s = bc_a(ins) - 1; s < maxslot; s++)
                    USE_SLOT(s);
            }
            break;
        default:
...
void lj_snap_shrink(jit_State *J)
{
    ...
    while (n <= nlim)
        map[m++] = map[n++]; /* Move PC + frame links down. */
...
static TRef snap_replay_const(jit_State *J, IRIns *ir)
{
    ...
    switch ((IROp)ir->o) {
    case IR_KPRI:
        return TREF_PRI(irt_type(ir->t));
    case IR_KINT:
        return lj_ir_kint(J, ir->i);
    case IR_KGC:
        return lj_ir_kgc(J, ir_kgc(ir), irt_t(ir->t));
    case IR_KNUM:
    case IR_KINT64:
        return lj_ir_k64(J, (IROp)ir->o, ir_k64(ir)->u64);
    case IR_KPTR:
        return lj_ir_kptr(J, ir_kptr(ir)); /* Continuation. */
    default:
        lua_assert(0);
        return TREF_NIL;
        break;
    ...
    if (pass23) {
        ...
        for (n = 0; n < nent; n++) {
            ...
            if (regsp_reg(ir->r) == RID_SUNK) {
                if (J->slot[snap_slot(sn)] != snap_slot(sn))
                    continue;
                pass23 = 1;
                lua_assert(ir->o == IR_TNEW || ir->o == IR_TDUP || ir->o == IR_CNEW || ir->o == IR_CNEWI);
                if (ir->op1 >= T->nk)
                    snap_pref(J, T, map, nent, seen, ir->op1);
                if (ir->op2 >= T->nk)
                    snap_pref(J, T, map, nent, seen, ir->op2);
                if (LJ_HASFFI && ir->o == IR_CNEWI) {
                    ...
                    for (irs = ir + 1; irs < irlast; irs++)
                        if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs)) {
                            ...
                            else if ((LJ_SOFTFP || (LJ_32 && LJ_HASFFI)) && irs + 1 < irlast && (irs + 1)->o == IR_HIOP)
        ...
        for (n = 0; pass23 && n < nent; n++) {
            ...
            if (regsp_reg(ir->r) == RID_SUNK) {
                ...
                op1 = ir->op1;
                if (op1 >= T->nk)
                    op1 = snap_pref(J, T, map, nent, seen, op1);
                op2 = ir->op2;
                if (op2 >= T->nk)
                    op2 = snap_pref(J, T, map, nent, seen, op2);
                if (LJ_HASFFI && ir->o == IR_CNEWI) {
                    if (LJ_32 && refp + 1 < T->nins && (ir + 1)->o == IR_HIOP) {
                        lj_needsplit(J); /* Emit joining HIOP. */
                        op2 = emitir_raw(IRT(IR_HIOP, IRT_I64), op2, snap_pref(J, T, map, nent, seen, (ir + 1)->op2));
                    }
                    ...
                    for (irs = ir + 1; irs < irlast; irs++)
                        if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs)) {
                            ...
                            if (irr->o != IR_FREF) {
                                IRIns *irk = &T->ir[key];
                                if (irr->o == IR_HREFK)
                                    key = lj_ir_kslot(J, snap_replay_const(J, &T->ir[irk->op1]), irk->op2);
                                else
                            ...
                            if (val == 0) {
                                ...
                                if (irref_isk(irs->op2) && irref_isk((irs + 1)->op2)) {
                                    uint64_t k = (uint32_t)T->ir[irs->op2].i + ((uint64_t)T->ir[(irs + 1)->op2].i << 32);
                                    val = lj_ir_k64(J, t == IRT_I64 ? IR_KINT64 : IR_KNUM, k);
                                } else {
                                    val = emitir_raw(IRT(IR_HIOP, t), val, snap_pref(J, T, map, nent, seen, (irs + 1)->op2));
                                }
...
static void snap_unsink(jit_State *J, GCtrace *T, ExitState *ex, SnapNo snapno, BloomFilter rfilt, IRIns *ir, TValue *o);
...
static void snap_restoredata(GCtrace *T, ExitState *ex, SnapNo snapno, BloomFilter rfilt, IRRef ref, void *dst, CTSize sz)
{
    IRIns *ir = &T->ir[ref];
    RegSP rs = ir->prev;
    int32_t *src;
...
static void snap_unsink(jit_State *J, GCtrace *T, ExitState *ex, SnapNo snapno, BloomFilter rfilt, IRIns *ir, TValue *o)
{
        ...
        if (ir->o == IR_CNEWI) {
            ...
            for (irs = ir + 1; irs < irlast; irs++)
                if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs)) {
                    ...
                    if (irt_is64(irs->t))
                        szs = 8;
                    else if (irt_isi8(irs->t) || irt_isu8(irs->t))
                        szs = 1;
                    else if (irt_isi16(irs->t) || irt_isu16(irs->t))
                        szs = 2;
                    else
                        szs = 4;
                    if (LJ_64 && iro->o == IR_KINT64)
                        ...
                        p += iro->i;
                    lua_assert(p >= (uint8_t *)cdataptr(cd) && p + szs <= (uint8_t *)cdataptr(cd) + sz);
    ...
    {
        IRIns *irs, *irlast;
        GCtab *t = ir->o == IR_TNEW ? lj_tab_new(J->L, ir->op1, ir->op2) : lj_tab_dup(J->L, ir_ktab(&T->ir[ir->op1]));
        settabV(J->L, o, t);
        ...
        for (irs = ir + 1; irs < irlast; irs++)
            if (irs->r == RID_SINK && snap_sunk_store(T, ir, irs)) {
                IRIns *irk = &T->ir[irs->op1];
                TValue tmp, *val;
                lua_assert(irs->o == IR_ASTORE || irs->o == IR_HSTORE || irs->o == IR_FSTORE);
                if (irk->o == IR_FREF) {
