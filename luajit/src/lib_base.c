...
LJLIB_PUSH(lastcl)
LJLIB_ASM(pairs) LJLIB_REC(xpairs 0) { return ffh_pairs(L, MM_pairs); }
...
LJLIB_PUSH(lastcl)
LJLIB_ASM(ipairs) LJLIB_REC(xpairs 1) { return ffh_pairs(L, MM_ipairs); }
...
LJLIB_CF(unpack)
{
    ...
    int32_t e = (L->base + 3 - 1 < L->top && !tvisnil(L->base + 3 - 1)) ? lj_lib_checkint(L, 3) : (int32_t)lj_tab_len(t);
    if (i > e)
        return 0;
    n = e - i + 1;
    ...
    if (base == 10) {
        ...
        if (tviscdata(o)) {
            ...
            if (ctype_isnum(ct->info) || ctype_iscomplex(ct->info)) {
                if (LJ_DUALNUM && ctype_isinteger_or_bool(ct->info) && ct->size <= 4 && !(ct->size == 4 && (ct->info & CTF_UNSIGNED))) {
                    int32_t i;
                    lj_cconv_ct_tv(cts, ctype_get(cts, CTID_INT32), (uint8_t *)&i, o, 0);
                    setintV(L->base - 1 - LJ_FR2, i);
                    return FFH_RES(1);
                }
                lj_cconv_ct_tv(cts, ctype_get(cts, CTID_DOUBLE), (uint8_t *)&(L->base - 1 - LJ_FR2)->n, o, 0);
                return FFH_RES(1);
            }
        }
#endif
        ...
        if (p != ep) {
            while (lj_char_isspace((unsigned char)(*ep)))
                ep++;
            if (*ep == '\0') {
...
LJLIB_CF(load)
{
    ...
    if (L->base < L->top && (tvisstr(L->base) || tvisnumber(L->base))) {
        ...
        status = luaL_loadbufferx(L, strdata(s), s->len, strdata(name ? name : s), mode ? strdata(mode) : NULL);
    } else {
        lj_lib_checkfunc(L, 1);
        lua_settop(L, 5); /* Reserve a slot for the string from the reader. */
        status = lua_loadx(L, reader_func, NULL, name ? strdata(name) : "=(load)", mode ? strdata(mode) : NULL);
    }
    return load_aux(L, status, 4);
}

LJLIB_CF(loadstring) { return lj_cf_load(L); }
...
LJLIB_CF(coroutine_status)
{
    ...
    if (co == L)
        ...
        s = "dead";
    else if (co->base > tvref(co->stack) + 1 + LJ_FR2)
        s = "normal";
...
{
    if (co->cframe != NULL || co->status > LUA_YIELD || (co->status == LUA_OK && co->top == co->base)) {
...
LJLIB_NOREG LJLIB_ASM(coroutine_wrap_aux) { return ffh_resume(L, threadV(lj_lib_upvalue(L, 1)), 1); }
...
static void setpc_wrap_aux(lua_State *L, GCfunc *fn) { setmref(fn->c.pc, &L2GG(L)->bcff[lj_lib_init_coroutine[1] + 2]); }
