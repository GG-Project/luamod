...
#define opisfusableload(o) ((o) == IR_ALOAD || (o) == IR_HLOAD || (o) == IR_ULOAD || (o) == IR_FLOAD || (o) == IR_XLOAD || (o) == IR_SLOAD || (o) == IR_VLOAD)

/* Sparse limit checks using a red zone before the actual limit. */
#define MCLIM_REDZONE 64

static LJ_NORET LJ_NOINLINE void asm_mclimit(ASMState *as) { lj_mcode_limiterr(as->J, (size_t)(as->mctop - as->mcp + 4 * MCLIM_REDZONE)); }
...
static LJ_AINLINE void checkmclim(ASMState *as)
{
    ...
    if (as->mcp + MCLIM_REDZONE < as->mcp_prev) {
        ...
        fprintf(stderr, "RED ZONE OVERFLOW: %p IR %04d  %02d %04d %04d\n", as->mcp, as->curins + 1 - REF_BIAS, ir->o, ir->op1 - REF_BIAS, ir->op2 - REF_BIAS);
        lua_assert(0);
    }
#endif
    if (LJ_UNLIKELY(as->mcp < as->mclim))
        asm_mclimit(as);
#ifdef LUA_USE_ASSERT
    as->mcp_prev = as->mcp;
#endif
...
#define emit_spload(as, ir, r, ofs) emit_loadofs(as, ir, (r), RID_SP, (ofs))
#define emit_spstore(as, ir, r, ofs) emit_storeofs(as, ir, (r), RID_SP, (ofs))
...
static const char *const ra_regname[] = { GPRDEF(RIDNAME) FPRDEF(RIDNAME) VRIDDEF(RIDNAME) NULL };
#undef RIDNAME
...
static Reg ra_rematk(ASMState *as, IRRef ref)
{
    ...
    if (ir->o == IR_KNUM) {
        ...
        lua_assert(ir->o == IR_KINT || ir->o == IR_KGC || ir->o == IR_KPTR || ir->o == IR_KKPTR || ir->o == IR_KNULL);
...
#define MINCOST(name)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
    if (rset_test(RSET_ALL, RID_##name) && LJ_LIKELY(allow & RID2RSET(RID_##name)) && as->cost[RID_##name] < cost)                                                                                                                                                                                                                                                                                                                                                                                                 \
...
#if LJ_GC64
                    (ir->o == IR_KINT && k == ir->i) || (ir->o == IR_KGC && k == (intptr_t)ir_kgc(ir)) || ((ir->o == IR_KPTR || ir->o == IR_KKPTR) && k == (intptr_t)ir_kptr(ir))
#else
...
#else
        if (ref < ASMREF_L && k == (ra_iskref(ref) ? ra_krefk(as, ref) : IR(ref)->i))
            return r;
#endif
        rset_clear(work, r);
    }
...
static Reg ra_dest(ASMState *as, IRIns *ir, RegSet allow)
{
    ...
    if (LJ_UNLIKELY(ra_hasspill(ir->s)))
        ra_save(as, ir, dest);
    return dest;
}
...
static void ra_leftov(ASMState *as, Reg dest, IRRef lref)
{
    ...
    if (ra_noreg(left)) {
        ra_sethint(ir->r, dest); /* Propagate register hint. */
        left = ra_allocref(as, lref, (LJ_SOFTFP || dest < RID_MAX_GPR) ? RSET_GPR : RSET_FPR);
    }
    ra_noweak(as, left);
    if (dest != left) {
        /* Use register renaming if dest is the PHI reg. */
...
static void ra_destpair(ASMState *as, IRIns *ir)
{
    ...
    if (!rset_test(as->freeset, RID_RETHI) && destlo != RID_RETHI && desthi != RID_RETHI)
    ...
    if (destlo == RID_RETHI) {
        ...
        emit_movrr(as, ir, RID_RETLO, RID_RETHI);
        if (destlo != RID_RETLO)
            emit_movrr(as, ir, destlo, RID_RETLO);
    } else {
        if (desthi != RID_RETHI)
            emit_movrr(as, ir, desthi, RID_RETHI);
        if (destlo != RID_RETLO)
    ...
    if (ra_hasspill((ir + 1)->s))
        ra_save(as, ir + 1, RID_RETHI);
    if (ra_hasspill(ir->s))
        ra_save(as, ir, RID_RETLO);
}
#endif
...
{
    if (irs->s == 255) {
        if (irs->o == IR_ASTORE || irs->o == IR_HSTORE || irs->o == IR_FSTORE || irs->o == IR_XSTORE) {
...
static void asm_snap_alloc1(ASMState *as, IRRef ref)
{
    ...
    if (!irref_isk(ref) && (!(ra_used(ir) || ir->r == RID_SUNK))) {
        if (ir->r == RID_SINK) {
            ...
            { /* Allocate stored values for TNEW, TDUP and CNEW. */
                ...
                for (irs = IR(as->snapref - 1); irs > ir; irs--)
                    if (irs->r == RID_SINK && asm_sunk_store(as, ir, irs)) {
                        lua_assert(irs->o == IR_ASTORE || irs->o == IR_HSTORE || irs->o == IR_FSTORE || irs->o == IR_XSTORE);
            ...
            if (ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT) {
                ...
                for (irc = IR(as->curins); irc > ir; irc--)
                    if ((irc->op1 == ref || irc->op2 == ref) && !(irc->r == RID_SINK || irc->r == RID_SUNK))
        ...
        nosink:
            ...
            if ((as->freeset & allow) || (allow == RSET_FPR && asm_snap_canremat(as))) {
...
static void asm_snap_prep(ASMState *as)
{
    if (as->curins < as->snapref) {
        do {
            if (as->snapno == 0)
                return; /* Called by sunk stores before snap #0. */
            as->snapno--;
...
static void asm_gcstep(ASMState *as, IRIns *ir)
{
    ...
    for (ira = IR(as->stopins + 1); ira < ir; ira++)
        if ((ira->o == IR_TNEW || ira->o == IR_TDUP || (LJ_HASFFI && (ira->o == IR_CNEW || ira->o == IR_CNEWI))) && ra_used(ira))
            as->gcsteps++;
    if (as->gcsteps)
        asm_gc_check(as);
...
static void asm_bufhdr(ASMState *as, IRIns *ir)
{
    ...
    if ((ir->op2 & IRBUFHDR_APPEND)) {
        ...
        IRIns *irp = IR(ir->op1);
        if (!(ra_hasreg(irp->r) || irp == ir - 1 || (irp == ir - 2 && !ra_used(ir - 1)))) {
...
static int asm_fpjoin_pow(ASMState *as, IRIns *ir)
{
    ...
    if (irp == ir - 1 && irp->o == IR_MUL && !ra_used(irp)) {
        ...
        if (irpp == ir - 2 && irpp->o == IR_FPMATH && irpp->op2 == IRFPM_LOG2 && !ra_used(irpp)) {
...
static void asm_phi_break(ASMState *as, RegSet blocked, RegSet blockedby, RegSet allow)
{
...
static void asm_head_side(ASMState *as)
{
    ...
    for (i = as->stopins; i > REF_BASE; i--) {
        IRIns *ir = IR(i);
        RegSP rs;
        lua_assert((ir->o == IR_SLOAD && (ir->op2 & IRSLOAD_PARENT)) || (LJ_SOFTFP && ir->o == IR_HIOP) || ir->o == IR_PVAL);
...
static void asm_tail_link(ASMState *as)
{
    ...
    if (as->T->link == 0) {
        ...
        case BC_CALLM:
        case BC_CALLMT:
            mres -= (int32_t)(1 + LJ_FR2 + bc_a(*pc) + bc_c(*pc));
            break;
        case BC_RETM:
            mres -= (int32_t)(bc_a(*pc) + bc_d(*pc));
            break;
        case BC_TSETM:
            mres -= (int32_t)bc_a(*pc);
            break;
        default:
...
static void asm_setup_regsp(ASMState *as)
{
    ...
    for (lastir = IR(nins); ir < lastir; ir++) {
        ...
        case IR_CALLS: {
            ...
            ir->prev = asm_setup_call_slots(as, ir, ci);
            if (inloop)
                as->modset |= (ci->flags & CCI_NOFPRCLOBBER) ? (RSET_SCRATCH & ~RSET_FPR) : RSET_SCRATCH;
            continue;
        }
        ...
        case IR_VLOAD:
            ...
            break;
        case IR_CONV:
            if (irt_isfp(ir->t) || (ir->op2 & IRCONV_SRCMASK) == IRT_NUM || (ir->op2 & IRCONV_SRCMASK) == IRT_FLOAT)
                break;
            /* fallthrough */
        default:
            /* Propagate hints across likely 'op reg, imm' or 'op reg'. */
            if (irref_isk(ir->op2) && !irref_isk(ir->op1) && ra_hashint(regsp_reg(IR(ir->op1)->prev))) {
    ...
    /* Remove nops/renames left over from ASM restart due to LJ_TRERR_MCODELM. */
    {
        ...
        if (ir->o == IR_NOP || ir->o == IR_RENAME) {
            do {
                ir--;
                nins--;
            } while (ir->o == IR_NOP || ir->o == IR_RENAME);
            T->nins = nins;
        }
    }
    ...
    */
    for (;;) {
        ...
        if (J->curfinal->nins >= T->nins) { /* IR didn't grow? */
            ...
            memcpy(J->curfinal->ir + as->orignins, T->ir + as->orignins, (T->nins - as->orignins) * sizeof(IRIns)); /* Copy RENAMEs. */
