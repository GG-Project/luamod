...
LJ_FUNCA int lj_err_unwind_arm(int state, _Unwind_Control_Block *ucb, _Unwind_Context *ctx)
{
    ...
    case _US_UNWIND_FRAME_STARTING:
        ...
        cf = err_unwind(L, cf, errcode);
        if ((state & _US_FORCE_UNWIND) || cf == NULL)
            break;
        _Unwind_SetGR(ctx, 15, (uint32_t)lj_vm_unwind_ext);
        _Unwind_SetGR(ctx, 0, (uint32_t)ucb);
        _Unwind_SetGR(ctx, 1, (uint32_t)errcode);
        _Unwind_SetGR(ctx, 2, cframe_unwind_ff(cf) ? (uint32_t)lj_vm_unwind_ff_eh : (uint32_t)lj_vm_unwind_c_eh);
        return _URC_INSTALL_CONTEXT;
    default:
        return _URC_FAILURE;
    }
...
LJ_FUNCA int lj_err_unwind_win(EXCEPTION_RECORD *rec, void *f, CONTEXT *ctx, UndocumentedDispatcherContext *dispatch)
{
    ...
    if ((rec->ExceptionFlags & 6)) { /* EH_UNWINDING|EH_EXIT_UNWIND */
        ...
        if (cf2) { /* We catch it, so start unwinding the upper frames. */
            ...
            */
            RtlUnwindEx(cf, (void *)((cframe_unwind_ff(cf2) && errcode != LUA_YIELD) ? lj_vm_unwind_ff_eh : lj_vm_unwind_c_eh), rec, (void *)(uintptr_t)errcode, ctx, dispatch->HistoryTable);
...
LJ_NOINLINE void lj_err_msg(lua_State *L, ErrMsg em) { err_msgv(L, em); }
...
LJ_NOINLINE void lj_err_callermsg(lua_State *L, const char *msg)
{
    ...
    if (frame_islua(frame)) {
        ...
        if (frame_iscont_fficb(frame)) {
            ...
            if (frame_func(frame)->c.ffid >= FF_ffi_meta___index && frame_func(frame)->c.ffid <= FF_ffi_meta___tostring) {
                L->base = pframe + 1;
                L->top = frame;
...
/* Argument error. */
LJ_NOINLINE void lj_err_arg(lua_State *L, int narg, ErrMsg em) { err_argmsg(L, narg, err2msg(em)); }
...
/* Typecheck error for arguments. */
LJ_NOINLINE void lj_err_argt(lua_State *L, int narg, int tt) { lj_err_argtype(L, narg, lj_obj_typename[tt + 1]); }
