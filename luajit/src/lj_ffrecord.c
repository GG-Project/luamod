...
static void LJ_FASTCALL recff_nyi(jit_State *J, RecordFFData *rd)
{
    if (J->cur.nins < (IRRef)J->param[JIT_P_minstitch] + REF_BASE) {
        ...
        if (J->framedepth && frame_islua(J->L->base - 1)) {
            ...
            if (!(op == BC_CALLM || op == BC_CALLMT || op == BC_RETM || op == BC_TSETM)) {
                switch (J->fn->c.ffid) {
                case FF_error:
...
static TRef recff_bufhdr(jit_State *J) { return emitir(IRT(IR_BUFHDR, IRT_PGC), lj_ir_kptr(J, &J2G(J)->tmpbuf), IRBUFHDR_RESET); }
...
int32_t lj_ffrecord_select_mode(jit_State *J, TRef tr, TValue *tv)
{
    if (tref_isstr(tr) && *strVdata(tv) == '#') { /* select('#', ...) */
        ...
        if (start == 0)
            lj_trace_err(J, LJ_TRERR_BADTYPE); /* A bit misleading. */
        return start;
    }
}
...
{
    TRef tr = J->base[0];
    if (!((LJ_52 || (LJ_HASFFI && tref_iscdata(tr))) && recff_metacall(J, rd, MM_pairs + rd->data))) {
        if (tref_istab(tr)) {
...
static void LJ_FASTCALL recff_math_unary(jit_State *J, RecordFFData *rd) { J->base[0] = emitir(IRTN(IR_FPMATH), lj_ir_tonum(J, J->base[0]), rd->data); }
...
static void LJ_FASTCALL recff_string_range(jit_State *J, RecordFFData *rd)
{
    ...
    if (end < 0) {
        ...
        trend = emitir(IRTI(IR_ADD), emitir(IRTI(IR_ADD), trlen, trend), lj_ir_kint(J, 1));
...
static void LJ_FASTCALL recff_string_find(jit_State *J, RecordFFData *rd)
{
    ...
    if ((J->base[2] && tref_istruecond(J->base[3])) || (emitir(IRTG(IR_EQ, IRT_STR), trpat, lj_ir_kstr(J, pat)), !lj_str_haspattern(pat))) { /* Search for fixed string. */
        ...
        if (lj_str_find(strdata(str) + (MSize)start, strdata(pat), str->len - (MSize)start, pat->len)) {
            TRef pos;
...
static void LJ_FASTCALL recff_string_format(jit_State *J, RecordFFData *rd)
{
    ...
    while ((sf = lj_strfmt_parse(&fs)) != STRFMT_EOF) { /* Parse format. */
        ...
        case STRFMT_LIT:
            tr = emitir(IRT(IR_BUFPUT, IRT_PGC), tr, lj_ir_kstr(J, lj_str_new(J->L, fs.str, fs.len)));
            break;
        case STRFMT_INT:
        ...
        handle_int:
            ...
            if (sf == STRFMT_INT) { /* Shortcut for plain %d. */
                tr = emitir(IRT(IR_BUFPUT, IRT_PGC), tr, emitir(IRT(IR_TOSTR, IRT_STR), tra, IRTOSTR_INT));
            } else {
#if LJ_HASFFI
                tra = emitir(IRT(IR_CONV, IRT_U64), tra, (IRT_INT | (IRT_U64 << 5) | IRCONV_SEXT));
        ...
        case STRFMT_CHAR:
            ...
            if (sf == STRFMT_CHAR) /* Shortcut for plain %c. */
                tr = emitir(IRT(IR_BUFPUT, IRT_PGC), tr, emitir(IRT(IR_TOSTR, IRT_STR), tra, IRTOSTR_CHAR));
            else
...
static void LJ_FASTCALL recff_table_concat(jit_State *J, RecordFFData *rd)
{
    ...
    if (tref_istab(tab)) {
        ...
        TRef tri = (J->base[1] && !tref_isnil(J->base[2])) ? lj_opt_narrow_toint(J, J->base[2]) : lj_ir_kint(J, 1);
        TRef tre = (J->base[1] && J->base[2] && !tref_isnil(J->base[3])) ? lj_opt_narrow_toint(J, J->base[3]) : lj_ir_call(J, IRCALL_lj_tab_len, tab);
        TRef hdr = recff_bufhdr(J);
...
static void LJ_FASTCALL recff_io_write(jit_State *J, RecordFFData *rd)
{
    ...
    for (; J->base[i]; i++) {
        ...
        if (tref_isk(len) && IR(tref_ref(len))->i == 1) {
            ...
            TRef tr = (irs->o == IR_TOSTR && irs->op2 == IRTOSTR_CHAR) ? irs->op1 : emitir(IRT(IR_XLOAD, IRT_U8), buf, IRXLOAD_READONLY);
