...
{
    global_State *g;
    MSize extra = sizeof(GCcdataVar) + sizeof(GCcdata) + (align > CT_MEMALIGN ? (1u << align) - (1u << CT_MEMALIGN) : 0);
...
void LJ_FASTCALL lj_cdata_free(global_State *g, GCcdata *cd)
{
    if (LJ_UNLIKELY(cd->marked & LJ_GC_CDATA_FIN)) {
        ...
        lua_assert(ctype_hassize(ct->info) || ctype_isfunc(ct->info) || ctype_isextern(ct->info));
...
collect_attrib:
    ...
    integer_key:
        ...
        if (ctype_isinteger(ctk->info)) {
            lj_cconv_ct_ct(cts, ctype_get(cts, CTID_INT_PSZ), ctk, (uint8_t *)&idx, cdataptr(cdk), 0);
            goto integer_key;
        }
