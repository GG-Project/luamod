...
*/
#define gcstep_barrier(J, ref) ((ref) < J->chain[IR_LOOP] && (J->chain[IR_SNEW] || J->chain[IR_XSNEW] || J->chain[IR_TNEW] || J->chain[IR_TDUP] || J->chain[IR_CNEW] || J->chain[IR_CNEWI] || J->chain[IR_BUFSTR] || J->chain[IR_TOSTR] || J->chain[IR_CALLA]))
...
LJFOLDF(kfold_numcomp) { return CONDFOLD(lj_ir_numcmp(knumleft, knumright, (IROp)fins->o)); }
...
static int32_t kfold_intop(int32_t k1, int32_t k2, IROp op)
{
    ...
    case IR_BSHL:
        ...
        break;
    case IR_BSHR:
        k1 = (int32_t)((uint32_t)k1 >> (k2 & 31));
        break;
    case IR_BSAR:
        k1 >>= (k2 & 31);
        break;
    case IR_BROL:
        k1 = (int32_t)lj_rol((uint32_t)k1, (k2 & 31));
        break;
    case IR_BROR:
        k1 = (int32_t)lj_ror((uint32_t)k1, (k2 & 31));
        break;
    case IR_MIN:
        k1 = k1 < k2 ? k1 : k2;
        break;
...
LJFOLDF(kfold_intarith) { return INTFOLD(kfold_intop(fleft->i, fright->i, (IROp)fins->o)); }
...
{
    lua_Number n = lj_vm_foldarith((lua_Number)fleft->i, (lua_Number)fright->i, fins->o - IR_ADDOV);
...
LJFOLD(BNOT KINT)
LJFOLDF(kfold_bnot) { return INTFOLD(~fleft->i); }

LJFOLD(BSWAP KINT)
LJFOLDF(kfold_bswap) { return INTFOLD((int32_t)lj_bswap((uint32_t)fleft->i)); }
...
LJFOLDF(kfold_intcomp)
{
    ...
    case IR_GT:
        return CONDFOLD(a > b);
    case IR_ULT:
        return CONDFOLD((uint32_t)a < (uint32_t)b);
    case IR_UGE:
        return CONDFOLD((uint32_t)a >= (uint32_t)b);
    case IR_ULE:
        return CONDFOLD((uint32_t)a <= (uint32_t)b);
...
static uint64_t kfold_int64arith(uint64_t k1, uint64_t k2, IROp op)
{
    ...
    case IR_BSHL:
        ...
        break;
    case IR_BSHR:
        k1 = (int32_t)((uint32_t)k1 >> (k2 & 63));
        break;
    case IR_BSAR:
        k1 >>= (k2 & 63);
        break;
    case IR_BROL:
        k1 = (int32_t)lj_rol((uint32_t)k1, (k2 & 63));
        break;
    case IR_BROR:
        k1 = (int32_t)lj_ror((uint32_t)k1, (k2 & 63));
        break;
#endif
    default:
        UNUSED(k2);
...
LJFOLDF(kfold_int64arith) { return INT64FOLD(kfold_int64arith(ir_k64(fleft)->u64, ir_k64(fright)->u64, (IROp)fins->o)); }
...
LJFOLDF(kfold_int64arith2)
{
    ...
    if (irt_isi64(fins->t)) {
        k1 = fins->o == IR_DIV ? lj_carith_divi64((int64_t)k1, (int64_t)k2) : fins->o == IR_MOD ? lj_carith_modi64((int64_t)k1, (int64_t)k2) : lj_carith_powi64((int64_t)k1, (int64_t)k2);
    } else {
        k1 = fins->o == IR_DIV ? lj_carith_divu64(k1, k2) : fins->o == IR_MOD ? lj_carith_modu64(k1, k2) : lj_carith_powu64(k1, k2);
    }
    return INT64FOLD(k1);
#else
    UNUSED(J);
...
LJFOLDF(kfold_int64comp)
{
    ...
    switch ((IROp)fins->o) {
    case IR_LT:
        return CONDFOLD((int64_t)a < (int64_t)b);
    case IR_GE:
        return CONDFOLD((int64_t)a >= (int64_t)b);
    case IR_LE:
        return CONDFOLD((int64_t)a <= (int64_t)b);
...
LJFOLDF(bufput_append)
{
    ...
    if ((J->flags & JIT_F_OPT_FWD) && !(fleft->op2 & IRBUFHDR_APPEND) && fleft->prev == fright->op2 && fleft->op1 == IR(fright->op2)->op1) {
        IRRef ref = fins->op1;
...
LJFOLDF(bufput_kgc)
{
    if (LJ_LIKELY(J->flags & JIT_F_OPT_FOLD) && fright->o == IR_KGC) {
        ...
        if (s2->len == 0) { /* Empty string? */
            ...
            if (fleft->o == IR_BUFPUT && irref_isk(fleft->op2) && !irt_isphi(fleft->t)) { /* Join two constant string puts in a row. */
...
{
    lua_assert(fleft->o == IR_BUFHDR || fleft->o == IR_BUFPUT || fleft->o == IR_CALLL);
    ...
    if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
        ...
        while (ref) {
            ...
            while (ira->o == irb->o && ira->op2 == irb->op2) {
                lua_assert(ira->o == IR_BUFHDR || ira->o == IR_BUFPUT || ira->o == IR_CALLL || ira->o == IR_CARG);
...
LJFOLDF(bufput_kfold_op)
{
    if (irref_isk(fleft->op2)) {
        ...
        sb = ((SBuf * (LJ_FASTCALL *)(SBuf *, GCstr *)) ci->func)(sb, ir_kstr(IR(fleft->op2)));
        fins->o = IR_BUFPUT;
        fins->op1 = fleft->op1;
...
LJFOLDF(bufput_kfold_fmt)
{
    ...
    if (irref_isk(fleft->op2)) {
        ...
        default: {
            const CCallInfo *ci = &lj_ir_callinfo[fins->op2];
            sb = ((SBuf * (*)(SBuf *, SFormat, lua_Number)) ci->func)(sb, sf, ir_knum(ira)->n);
            break;
        }
        }
        fins->o = IR_BUFPUT;
...
LJFOLDF(kfold_add_kgc)
{
    ...
    if (irt_iscdata(fleft->t)) {
        ...
        if (ctype_isnum(ct->info) || ctype_isenum(ct->info) || ctype_isptr(ct->info) || ctype_isfunc(ct->info) || ctype_iscomplex(ct->info) || ctype_isvector(ct->info))
...
LJFOLDF(kfold_tobit) { return INTFOLD(lj_num2bit(knumleft)); }

LJFOLD(CONV KINT IRCONV_NUM_INT)
LJFOLDF(kfold_conv_kint_num) { return lj_ir_knum(J, (lua_Number)fleft->i); }

LJFOLD(CONV KINT IRCONV_NUM_U32)
LJFOLDF(kfold_conv_kintu32_num) { return lj_ir_knum(J, (lua_Number)(uint32_t)fleft->i); }
...
LJFOLDF(kfold_conv_kint_ext)
{
    ...
    if ((fins->op2 & IRCONV_SRCMASK) == IRT_I8)
        k = (int8_t)k;
    else if ((fins->op2 & IRCONV_SRCMASK) == IRT_U8)
        k = (uint8_t)k;
    else if ((fins->op2 & IRCONV_SRCMASK) == IRT_I16)
        k = (int16_t)k;
    else
        k = (uint16_t)k;
    return INTFOLD(k);
...
LJFOLDF(kfold_conv_kint64_num_i64) { return lj_ir_knum(J, (lua_Number)(int64_t)ir_kint64(fleft)->u64); }

LJFOLD(CONV KINT64 IRCONV_NUM_U64)
LJFOLDF(kfold_conv_kint64_num_u64) { return lj_ir_knum(J, (lua_Number)ir_kint64(fleft)->u64); }

LJFOLD(CONV KINT64 IRCONV_INT_I64)
LJFOLD(CONV KINT64 IRCONV_U32_I64)
LJFOLDF(kfold_conv_kint64_int_i64) { return INTFOLD((int32_t)ir_kint64(fleft)->u64); }
...
LJFOLDF(kfold_conv_knum_i64_num) { return INT64FOLD((uint64_t)(int64_t)knumleft); }

LJFOLD(CONV KNUM IRCONV_U64_NUM)
LJFOLDF(kfold_conv_knum_u64_num) { return INT64FOLD(lj_num2u64(knumleft)); }

LJFOLD(TOSTR KNUM any)
LJFOLDF(kfold_tostr_knum) { return lj_ir_kstr(J, lj_strfmt_num(J->L, ir_knum(fleft))); }

LJFOLD(TOSTR KINT any)
LJFOLDF(kfold_tostr_kint) { return lj_ir_kstr(J, fins->op2 == IRTOSTR_INT ? lj_strfmt_int(J->L, fleft->i) : lj_strfmt_char(J->L, fleft->i)); }
...
LJFOLD(EQ KGC KGC)
LJFOLD(NE KGC KGC)
LJFOLDF(kfold_kref) { return CONDFOLD((fins->op1 == fins->op2) ^ (fins->o == IR_NE)); }
...
LJFOLDF(shortcut_left) { return LEFTFOLD; /* f(g(x)) ==> g(x) */ }
...
LJFOLDF(simplify_conv_int_num)
{
    ...
    if ((fleft->op2 & IRCONV_SRCMASK) == ((fins->op2 & IRCONV_DSTMASK) >> IRCONV_DSH))
        return fleft->op1;
    return NEXTFOLD;
}
...
{
    if ((fleft->op2 & IRCONV_SRCMASK) == IRT_INT || (fleft->op2 & IRCONV_SRCMASK) == IRT_U32)
        return LEFTFOLD;
    return NEXTFOLD;
}
...
LJFOLDF(cse_conv)
{
    if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
        ...
        while (ref > op1) {
            ...
            if (ir->op1 == op1 && (ir->op2 & IRCONV_MODEMASK) == op2 && irt_isguard(ir->t) >= guard)
                return ref;
            ref = ir->prev;
        }
    }
...
{
    int64_t k = fright->o == IR_KINT ? (int64_t)fright->i : (int64_t)ir_k64(fright)->u64;
    if (k == 0) /* i & 0 ==> 0 */
        return RIGHTFOLD;
    if (k == -1) /* i & -1 ==> i */
        return LEFTFOLD;
...
LJFOLDF(simplify_shift_andk)
{
    ...
    PHIBARRIER(fright);
    if ((fins->o < IR_BROL ? LJ_TARGET_MASKSHIFT : LJ_TARGET_MASKROT) && irk->o == IR_KINT) { /* i o (j & mask) ==> i o j */
        int32_t mask = irt_is64(fins->t) ? 63 : 31;
...
{
    int64_t k = fleft->o == IR_KINT ? (int64_t)fleft->i : (int64_t)ir_k64(fleft)->u64;
    if (k == 0) /* 0 o i ==> 0 */
        return LEFTFOLD;
    return NEXTFOLD;
}
...
{
    IRIns *irk = IR(fleft->op2);
    if (irk->o == IR_KINT && kfold_intop(-1, irk->i, (IROp)fleft->o) == fright->i)
...
LJFOLDF(simplify_andor_k64)
{
    ...
    PHIBARRIER(fleft);
    if (irk->o == IR_KINT64) {
        uint64_t k = kfold_int64arith(ir_k64(irk)->u64, ir_k64(fright)->u64, (IROp)fins->o);
        /* (i | k1) & k2 ==> i & k2, if (k1 & k2) == 0. */
        /* (i & k1) | k2 ==> i | k2, if (k1 | k2) == -1. */
...
LJFOLDF(abc_fwd)
{
    if (LJ_LIKELY(J->flags & JIT_F_OPT_ABC)) {
        if (irref_isk(fright->op2)) {
            ...
            if (add2->o == IR_ADD && irref_isk(add2->op2) && IR(fright->op2)->i == -IR(add2->op2)->i) {
...
LJFOLDF(abc_invar)
{
    ...
    if (!irt_isint(fins->t) && fins->op1 < J->chain[IR_LOOP] && !irt_isphi(IR(fins->op1)->t))
        return DROPFOLD;
    return NEXTFOLD;
}
...
static TRef kfold_xload(jit_State *J, IRIns *ir, const void *p)
{
    ...
    case IRT_NUM:
        return lj_ir_knum_u64(J, *(uint64_t *)p);
    case IRT_I8:
        k = (int32_t) * (int8_t *)p;
        break;
    case IRT_U8:
        ...
        break;
    case IRT_I16:
        k = (int32_t)(int16_t)lj_getu16(p);
        break;
    case IRT_U16:
        k = (int32_t)(uint16_t)lj_getu16(p);
        break;
    case IRT_INT:
    case IRT_U32:
        k = (int32_t)lj_getu32(p);
        break;
    case IRT_I64:
    case IRT_U64:
        return lj_ir_kint64(J, *(uint64_t *)p);
    default:
        return 0;
    }
...
#define FOLD_SNEW_TYPE8 IRT_U8 /* Prefer unsigned loads. */
    ...
    if (len <= FOLD_SNEW_MAX_LEN) {
        ...
        if (len > 0) {
            ...
            uint16_t ot = (uint16_t)(len == 1 ? IRT(IR_XLOAD, FOLD_SNEW_TYPE8) : len == 2 ? IRT(IR_XLOAD, IRT_U16) : IRTI(IR_XLOAD));
            TRef tmp = emitir(ot, strref, IRXLOAD_READONLY | (len > 1 ? IRXLOAD_UNALIGNED : 0));
            TRef val = kfold_xload(J, IR(tref_ref(tmp)), strdata(kstr));
            if (len == 3)
                tmp = emitir(IRTI(IR_BAND), tmp, lj_ir_kint(J, LJ_ENDIAN_SELECT(0x00ffffff, 0xffffff00)));
            fins->op1 = (IRRef1)tmp;
            fins->op2 = (IRRef1)val;
...
LJFOLDF(fwd_href_tdup)
{
    ...
    if (lj_tab_get(J->L, ir_ktab(IR(fleft->op1)), &keyv) == niltvg(J2G(J)) && lj_opt_fwd_href_nokey(J))
        return lj_ir_kkptr(J, niltvg(J2G(J)));
    return NEXTFOLD;
}
...
TRef LJ_FASTCALL lj_opt_fold(jit_State *J)
{
    ...
    if (LJ_UNLIKELY((J->flags & JIT_F_OPT_MASK) != JIT_F_OPT_DEFAULT)) {
        lua_assert(((JIT_F_OPT_FOLD | JIT_F_OPT_FWD | JIT_F_OPT_CSE | JIT_F_OPT_DSE) | JIT_F_OPT_DEFAULT) == JIT_F_OPT_DEFAULT);
        ...
        if ((J->flags & (JIT_F_OPT_FOLD | JIT_F_OPT_FWD | JIT_F_OPT_CSE)) != (JIT_F_OPT_FOLD | JIT_F_OPT_FWD | JIT_F_OPT_CSE) && irm_kind(lj_ir_mode[fins->o]) == IRM_L && fins->o != IR_SLOAD)
            return lj_ir_emit(J);

        /* No FOLD or DSE? Emit raw IR for stores. */
        if ((J->flags & (JIT_F_OPT_FOLD | JIT_F_OPT_DSE)) != (JIT_F_OPT_FOLD | JIT_F_OPT_DSE) && irm_kind(lj_ir_mode[fins->o]) == IRM_S)
            return lj_ir_emit(J);
    }
...
TRef LJ_FASTCALL lj_opt_cse(jit_State *J)
{
    ...
    if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
        ...
        IRRef lim = fins->op1;
        if (fins->op2 > lim)
            lim = fins->op2; /* Relies on lit < REF_BIAS. */
        while (ref > lim) {
            if (IR(ref)->op12 == op12)
