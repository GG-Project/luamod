...
static void init_mmap(void) { ntavm = (PNTAVM)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtAllocateVirtualMemory"); }
...
#define is_direct(p) (!((p)->head & PINUSE_BIT) && ((p)->prev_foot & IS_DIRECT_BIT))

/* Get the internal overhead associated with chunk p */
#define overhead_for(p) (is_direct(p) ? DIRECT_CHUNK_OVERHEAD : CHUNK_OVERHEAD)
...
/* page-align a size */
#define page_align(S) (((S) + (LJ_PAGESIZE - SIZE_T_ONE)) & ~(LJ_PAGESIZE - SIZE_T_ONE))

/* granularity-align a size */
#define granularity_align(S) (((S) + (DEFAULT_GRANULARITY - SIZE_T_ONE)) & ~(DEFAULT_GRANULARITY - SIZE_T_ONE))
...
static mchunkptr direct_resize(mchunkptr oldp, size_t nb)
{
    ...
    if (oldsize >= nb + SIZE_T_SIZE && (oldsize - nb) <= (DEFAULT_GRANULARITY >> 1)) {
        return oldp;
    } else {
        size_t offset = oldp->prev_foot & ~IS_DIRECT_BIT;
        size_t oldmmsize = oldsize + offset + DIRECT_FOOT_PAD;
        size_t newmmsize = mmap_align(nb + SIX_SIZE_T_SIZES + CHUNK_ALIGN_MASK);
        char *cp = (char *)CALL_MREMAP((char *)oldp - offset, oldmmsize, newmmsize, CALL_MREMAP_MV);
        if (cp != CMFAIL) {
...
static int alloc_trim(mstate m, size_t pad)
{
    ...
    if (pad < MAX_REQUEST && is_initialized(m)) {
        ...
        if (m->topsize > pad) {
            ...
            if (sp->size >= extra && !has_segment_link(m, sp)) { /* can't shrink if pinned */
