...
static const ASMFunction dispatch_got[] = { GOTDEF(GOTFUNC) };
#undef GOTFUNC
#endif
...
void lj_dispatch_update(global_State *g)
{
    ...
    mode |= G2J(g)->state != LJ_TRACE_IDLE ? (DISPMODE_REC | DISPMODE_INS | DISPMODE_CALL) : 0;
#endif
#if LJ_HASPROFILE
    ...
    if (oldmode != mode) { /* Mode changed? */
        ...
        if ((oldmode ^ mode) & (DISPMODE_PROF | DISPMODE_REC | DISPMODE_INS)) {
            ...
            if (!(mode & DISPMODE_INS)) { /* No ins dispatch? */
                ...
                ASMFunction f = (mode & DISPMODE_PROF) ? lj_vm_profhook : (mode & DISPMODE_REC) ? lj_vm_record : lj_vm_inshook;
                uint32_t i;
                for (i = 0; i < GG_LEN_SDISP; i++)
                    disp[i] = f;
            }
...
int luaJIT_setmode(lua_State *L, int idx, int mode)
{
    ...
    case LUAJIT_MODE_ALLSUBFUNC: {
        cTValue *tv = idx == 0 ? frame_prev(L->base - 1) - LJ_FR2 : idx > 0 ? L->base + (idx - 1) : L->top + idx;
        GCproto *pt;
...
LUA_API lua_Hook lua_gethook(lua_State *L) { return G(L)->hookf; }

LUA_API int lua_gethookmask(lua_State *L) { return G(L)->hookmask & HOOK_EVENTMASK; }

LUA_API int lua_gethookcount(lua_State *L) { return (int)G(L)->hookcstart; }
...
ASMFunction LJ_FASTCALL lj_dispatch_call(lua_State *L, const BCIns *pc)
{
    ...
    if ((uintptr_t)pc & 1) { /* Marker for hot call. */
        ...
        lua_assert(L->top - L->base == delta);
        goto out;
    } else if (J->state != LJ_TRACE_IDLE && !(g->hookmask & (HOOK_GC | HOOK_VMEVENT))) {
#ifdef LUA_USE_ASSERT
        ptrdiff_t delta = L->top - L->base;
#endif
...
out:
    ...
    if ((!(J->flags & JIT_F_ON) || J->state != LJ_TRACE_IDLE) && (op == BC_FUNCF || op == BC_FUNCV))
