...
cTValue *lj_meta_tget(lua_State *L, cTValue *o, cTValue *k)
{
    ...
    for (loop = 0; loop < LJ_MAX_IDXCHAIN; loop++) {
        ...
        if (LJ_LIKELY(tvistab(o))) {
            ...
            if (!tvisnil(tv) || !(mo = lj_meta_fast(L, tabref(t->metatable), MM_index)))
                return tv;
...
TValue *lj_meta_arith(lua_State *L, TValue *ra, cTValue *rb, cTValue *rc, BCReg op)
{
    ...
    TValue tempb, tempc;
    cTValue *b, *c;
    if ((b = str2num(rb, &tempb)) != NULL && (c = str2num(rc, &tempc)) != NULL) { /* Try coercion first. */
...
TValue *lj_meta_cat(lua_State *L, TValue *top, int left)
{
    ...
    do {
        if (!(tvisstr(top) || tvisnumber(top)) || !(tvisstr(top - 1) || tvisnumber(top - 1))) {
            cTValue *mo = lj_meta_lookup(L, top - 1, MM_concat);
            if (tvisnil(mo)) {
                mo = lj_meta_lookup(L, top, MM_concat);
                if (tvisnil(mo)) {
                    if (tvisstr(top - 1) || tvisnumber(top - 1))
                        top++;
...
void lj_meta_istype(lua_State *L, BCReg ra, BCReg tp)
{
    ...
    if (LJ_DUALNUM && tp == ~LJ_TNUMX)
        lj_lib_checkint(L, ra);
    else if (tp == ~LJ_TNUMX + 1)
        lj_lib_checknum(L, ra);
    else if (tp == ~LJ_TSTR)
        lj_lib_checkstr(L, ra);
    else
        lj_err_argtype(L, ra, lj_obj_itypename[tp]);
}
...
void LJ_FASTCALL lj_meta_for(lua_State *L, TValue *o)
{
    ...
    if (!lj_strscan_numberobj(o + 1))
        lj_err_msg(L, LJ_ERR_FORLIM);
    if (!lj_strscan_numberobj(o + 2))
    ...
    if (LJ_DUALNUM) {
        ...
        if (nint == 3) { /* Narrow to integers. */
            ...
            if (tvisint(o + 1))
                setnumV(o + 1, (lua_Number)intV(o + 1));
            if (tvisint(o + 2))
