...
static int carith_ptr(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
    ...
    if (ctype_isptr(ctp->info) || ctype_isrefarray(ctp->info)) {
        if ((mm == MM_sub || mm == MM_eq || mm == MM_lt || mm == MM_le) && (ctype_isptr(ca->ct[1]->info) || ctype_isrefarray(ca->ct[1]->info))) {
            uint8_t *pp2 = ca->p[1];
        ...
        if (!((mm == MM_add || mm == MM_sub) && ctype_isnum(ca->ct[1]->info)))
            return 0;
        lj_cconv_ct_ct(cts, ctype_get(cts, CTID_INT_PSZ), ca->ct[1], (uint8_t *)&idx, ca->p[1], 0);
        if (mm == MM_sub)
            idx = -idx;
    } else if (mm == MM_add && ctype_isnum(ctp->info) && (ctype_isptr(ca->ct[1]->info) || ctype_isrefarray(ca->ct[1]->info))) {
        /* Swap pointer and index. */
        ctp = ca->ct[1];
        pp = ca->p[1];
        lj_cconv_ct_ct(cts, ctype_get(cts, CTID_INT_PSZ), ca->ct[0], (uint8_t *)&idx, ca->p[0], 0);
    } else {
        return 0;
    }
    sz = lj_ctype_size(cts, ctype_cid(ctp->info)); /* Element size. */
...
{
    if (ctype_isnum(ca->ct[0]->info) && ca->ct[0]->size <= 8 && ctype_isnum(ca->ct[1]->info) && ca->ct[1]->size <= 8) {
        CTypeID id = (((ca->ct[0]->info & CTF_UNSIGNED) && ca->ct[0]->size == 8) || ((ca->ct[1]->info & CTF_UNSIGNED) && ca->ct[1]->size == 8)) ? CTID_UINT64 : CTID_INT64;
        ...
        case MM_eq:
            ...
            return 1;
        case MM_lt:
            setboolV(L->top - 1, id == CTID_INT64 ? ((int64_t)u0 < (int64_t)u1) : (u0 < u1));
            return 1;
        case MM_le:
            setboolV(L->top - 1, id == CTID_INT64 ? ((int64_t)u0 <= (int64_t)u1) : (u0 <= u1));
            return 1;
...
static int lj_carith_meta(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
    ...
    if (!tv) {
        ...
        lj_err_callerv(L, mm == MM_len ? LJ_ERR_FFI_BADLEN : mm == MM_concat ? LJ_ERR_FFI_BADCONCAT : mm < MM_add ? LJ_ERR_FFI_BADCOMP : LJ_ERR_FFI_BADARITH, repr[0], repr[1]);
    }
...
#if LJ_64
#define B64DEF(name) static LJ_AINLINE uint64_t lj_carith_##name(uint64_t x, int32_t sh)
#else
/* Not inlined on 32 bit archs, since some of these are quite lengthy. */
#define B64DEF(name) uint64_t LJ_NOINLINE lj_carith_##name(uint64_t x, int32_t sh)
#endif
...
{
    switch (op) {
    case IR_BSHL - IR_BSHL:
        x = lj_carith_shl64(x, sh);
        break;
    case IR_BSHR - IR_BSHL:
        x = lj_carith_shr64(x, sh);
        break;
    case IR_BSAR - IR_BSHL:
        x = lj_carith_sar64(x, sh);
        break;
    case IR_BROL - IR_BSHL:
        x = lj_carith_rol64(x, sh);
        break;
    case IR_BROR - IR_BSHL:
        x = lj_carith_ror64(x, sh);
        break;
    default:
        lua_assert(0);
        break;
...
uint64_t lj_carith_check64(lua_State *L, int narg, CTypeID *id)
{
    ...
    err:
        ...
        if (ctype_isenum(s->info))
            s = ctype_child(cts, s);
        if ((s->info & (CTMASK_NUM | CTF_BOOL | CTF_FP | CTF_UNSIGNED)) == CTINFO(CT_NUM, CTF_UNSIGNED) && s->size == 8)
            *id = CTID_UINT64; /* Use uint64_t, since it has the highest rank. */
        else if (!*id)
            *id = CTID_INT64; /* Use int64_t, unless already set. */
        lj_cconv_ct_ct(cts, ctype_get(cts, *id), s, (uint8_t *)&x, sp, CCF_ARG(narg));
        return x;
...
int64_t lj_carith_mul64(int64_t a, int64_t b) { return a * b; }
#endif
