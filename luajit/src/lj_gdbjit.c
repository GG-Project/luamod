...
GDBJITdesc __jit_debug_descriptor = { 1, GDBJIT_NOACTION, NULL, NULL };

/* GDB sets a breakpoint at this function. */
void LJ_NOINLINE __jit_debug_register_code() { __asm__ __volatile__(""); };
...
enum { ELFSECT_TYPE_PROGBITS = 1, ELFSECT_TYPE_SYMTAB = 2, ELFSECT_TYPE_STRTAB = 3, ELFSECT_TYPE_NOBITS = 8 };
...
enum { DW_CFA_nop = 0x0, DW_CFA_offset_extended = 0x5, DW_CFA_def_cfa = 0xc, DW_CFA_def_cfa_offset = 0xe, DW_CFA_offset_extended_sf = 0x11, DW_CFA_advance_loc = 0x40, DW_CFA_offset = 0x80 };
...
enum { DW_AT_name = 0x03, DW_AT_stmt_list = 0x10, DW_AT_low_pc = 0x11, DW_AT_high_pc = 0x12 };

enum { DW_FORM_addr = 0x01, DW_FORM_data4 = 0x06, DW_FORM_string = 0x08 };

enum { DW_LNS_extended_op = 0, DW_LNS_copy = 1, DW_LNS_advance_pc = 2, DW_LNS_advance_line = 3 };
...
enum { GDBJIT_SECT_NULL, GDBJIT_SECT_text, GDBJIT_SECT_eh_frame, GDBJIT_SECT_shstrtab, GDBJIT_SECT_strtab, GDBJIT_SECT_symtab, GDBJIT_SECT_debug_info, GDBJIT_SECT_debug_abbrev, GDBJIT_SECT_debug_line, GDBJIT_SECT__MAX };
...
static const ELFheader elfhdr_template = { .emagic = { 0x7f, 'E', 'L', 'F' },
    .eclass = LJ_64 ? 2 : 1,
...
#define DSECT(name, stmt)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
    {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
        ...
        p += 4;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
        stmt *szp_##name = (uint32_t)((p - (uint8_t *)szp_##name) - 4);                                                                                                                                                                                                                                                                                                                                                                                                                                            \
    }
...
static void LJ_FASTCALL gdbjit_symtab(GDBJITctx *ctx)
{
    ...
    *ctx->p++ = '\0'; /* Empty string at start of string table. */
    ...
    sym->name = gdbjit_strz(ctx, "TRACE_");
    ctx->p--;
...
static void LJ_FASTCALL gdbjit_ehframe(GDBJITctx *ctx)
{
    ...
    /* Emit DWARF EH FDE. */
    DSECT(
        FDE, DU32((uint32_t)(p - framep)); /* Offset to CIE. */
        DU32(0); /* Machine code offset relative to .text. */
        DU32(ctx->szmcode); /* Machine code length. */
        DB(0); /* Augmentation data. */
    /* Registers saved in CFRAME. */
#if LJ_TARGET_X86
        DB(DW_CFA_offset | DW_REG_BP); DUV(2); DB(DW_CFA_offset | DW_REG_DI); DUV(3); DB(DW_CFA_offset | DW_REG_SI); DUV(4); DB(DW_CFA_offset | DW_REG_BX); DUV(5);
#elif LJ_TARGET_X64
        DB(DW_CFA_offset | DW_REG_BP); DUV(2); DB(DW_CFA_offset | DW_REG_BX); DUV(3); DB(DW_CFA_offset | DW_REG_15); DUV(4); DB(DW_CFA_offset | DW_REG_14); DUV(5);
        /* Extra registers saved for JIT-compiled code. */
        DB(DW_CFA_offset | DW_REG_13); DUV(LJ_GC64 ? 10 : 9); DB(DW_CFA_offset | DW_REG_12); DUV(LJ_GC64 ? 11 : 10);
#elif LJ_TARGET_ARM
        {
            int i;
            for (i = 11; i >= 4; i--) {
...
#error "Unsupported target architecture"
        ...
        DUV(ctx->spadj); /* Trace stack frame size. */
        DALIGNNOP(sizeof(uintptr_t));)
...
static void LJ_FASTCALL gdbjit_debuginfo(GDBJITctx *ctx)
{
    ...
    DSECT(info, DU16(2); /* DWARF version. */
...
static void LJ_FASTCALL gdbjit_debugline(GDBJITctx *ctx)
{
    ...
    DSECT(
        line, DU16(2); /* DWARF version. */
        DSECT(header, DB(1); /* Minimum instruction length. */
              DB(1); /* is_stmt. */
              ...
              DB(0);
              /* File name table. */
              DSTR(ctx->filename); DUV(0); DUV(0); DUV(0); DB(0);)

            DLNE(DW_LNE_set_address, sizeof(uintptr_t));
        DADDR(ctx->mcaddr); if (ctx->lineno) {
            DB(DW_LNS_advance_line);
            DSV(ctx->lineno - 1);
        } DB(DW_LNS_copy);
        DB(DW_LNS_advance_pc); DUV(ctx->szmcode); DLNE(DW_LNE_end_sequence, 0);)
...
#define SECTALIGN(p, a) ((p) = (uint8_t *)(((uintptr_t)(p) + ((a)-1)) & ~(uintptr_t)((a)-1)))
...
static void gdbjit_lock_release() { __sync_lock_release(&gdbjit_lock); }
...
void lj_gdbjit_addtrace(jit_State *J, GCtrace *T)
{
    ...
    ctx.spadjp = CFRAME_SIZE_JIT + (MSize)(parent ? traceref(J, parent)->spadjust : 0);
