...
LJ_NORET LJ_NOINLINE static void err_syntax(LexState *ls, ErrMsg em) { lj_lex_error(ls, ls->tok, em); }

LJ_NORET LJ_NOINLINE static void err_token(LexState *ls, LexToken tok) { lj_lex_error(ls, ls->tok, LJ_ERR_XTOKEN, lj_lex_token2str(ls, tok)); }
...
#define checklimit(fs, v, l, m)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
    if ((v) >= (l))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
    err_limit(fs, l, m)
#define checklimitgt(fs, v, l, m)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
    if ((v) > (l))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
    err_limit(fs, l, m)
#define checkcond(ls, c, em)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
    {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
        if (!(c))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
            err_syntax(ls, em);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
    }
...
static void bcemit_nil(FuncState *fs, BCReg from, BCReg n)
{
    ...
    bcemit_INS(fs, n == 1 ? BCINS_AD(BC_KPRI, from, VKNIL) : BCINS_AD(BC_KNIL, from, from + n - 1));
}
...
static void expr_toreg_nobranch(FuncState *fs, ExpDesc *e, BCReg reg)
{
    ...
    if (e->k == VKSTR) {
        ...
        fs->flags |= PROTO_FFI;
        ins = BCINS_AD(BC_KCDATA, reg, const_gc(fs, obj2gco(cdataV(&e->u.nval)), LJ_TCDATA));
#endif
...
static void bcemit_store(FuncState *fs, ExpDesc *var, ExpDesc *e)
{
    ...
    if (var->k == VLOCAL) {
        ...
        if ((int32_t)rc < 0) {
            ...
            lua_assert(e->k != VNONRELOC || ra < fs->nactvar || rc < ra || (bcreg_free(fs, rc), 1));
...
static void bcemit_arith(FuncState *fs, BinOpr opr, ExpDesc *e1, ExpDesc *e2)
{
    ...
    if (opr == OPR_POW) {
        ...
        if (expr_isnumk(e1) && !expr_isnumk(e2) && (t = const_num(fs, e1)) <= BCMAX_B) {
            rb = rc;
            rc = t;
...
static void var_new(LexState *ls, BCReg n, GCstr *name)
{
    ...
    lua_assert((uintptr_t)name < VARNAME__MAX || lj_tab_getstr(fs->kt, name) != NULL);
...
static size_t fs_prep_line(FuncState *fs, BCLine numline) { return (fs->pc - 1) << (numline < 256 ? 0 : numline < 65536 ? 1 : 2); }
...
#define fs_fixup_line(fs, pt, li, numline) pt->firstline = pt->numline = 0, setmref((pt)->lineinfo, NULL)
#define fs_prep_var(ls, fs, ofsvar) (UNUSED(ofsvar), 0)
#define fs_fixup_var(ls, pt, p, ofsvar) setmref((pt)->uvinfo, NULL), setmref((pt)->varinfo, NULL)
...
static GCproto *fs_finish(LexState *ls, BCLine line)
{
    ...
    sizept += ((fs->nuv + 1) & ~1) * 2;
    ofsli = sizept;
    sizept += fs_prep_line(fs, numline);
    ofsdbg = sizept;
    sizept += fs_prep_var(ls, fs, &ofsvar);
...
static void expr_table(LexState *ls, ExpDesc *e)
{
    ...
    while (ls->tok != '}') {
        ...
        expr(ls, &val);
        if (expr_isk(&key) && key.k != VKNIL && (key.k == VKSTR || expr_isk_nojump(&val))) {
            TValue k, *v;
    ...
    if (vcall) {
        ...
        ExpDesc en;
        lua_assert(bc_a(ilp->ins) == freg && bc_op(ilp->ins) == (narr > 256 ? BC_TSETV : BC_TSETB));
...
static void parse_body(LexState *ls, ExpDesc *e, int needself, BCLine line)
{
    ...
    expr_init(e, VRELOCABLE, bcemit_AD(pfs, BC_FNEW, 0, const_gc(pfs, obj2gco(pt), LJ_TPROTO)));
#if LJ_HASFFI
...
/* Parse expression. */
static void expr(LexState *ls, ExpDesc *v) { expr_binop(ls, v, 0); /* Priority 0: parse whole expression. */ }
...
static int predict_next(LexState *ls, FuncState *fs, BCPos pc)
{
    ...
    return (name->len == 5 && !strcmp(strdata(name), "pairs")) || (name->len == 4 && !strcmp(strdata(name), "next"));
}
...
static void parse_chunk(LexState *ls)
{
    ...
    while (!islast && !parse_isend(ls->tok)) {
        ...
        lex_opt(ls, ';');
        lua_assert(ls->fs->framesize >= ls->fs->freereg && ls->fs->freereg >= ls->fs->nactvar);
