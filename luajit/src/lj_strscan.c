...
/* Parse octal number. */
static StrScanFmt strscan_oct(const uint8_t *p, TValue *o, StrScanFmt fmt, int32_t neg, uint32_t dig)
{
    uint64_t x = 0;
        ...
        if (LJ_UNLIKELY(*p >= 'A')) { /* Parse "inf", "infinity" or "nan". */
            ...
            if (casecmp(p[0], 'i') && casecmp(p[1], 'n') && casecmp(p[2], 'f')) {
                if (neg)
                    ...
                    setpinfV(&tmp);
                p += 3;
                if (casecmp(p[0], 'i') && casecmp(p[1], 'n') && casecmp(p[2], 'i') && casecmp(p[3], 't') && casecmp(p[4], 'y'))
                    p += 5;
    ...
    /* Parse regular number. */
    {
        ...
        if (*p) {
            ...
            if (casecmp(*p, 'i')) {
                ...
                if ((fmt == STRSCAN_U32 && !(opt & STRSCAN_OPT_C)) || (fmt >= STRSCAN_I64 && !(opt & STRSCAN_OPT_LL)))
                    return STRSCAN_ERROR;
            }
            while (lj_char_isspace(*p))
                p++;
...
{
    StrScanFmt fmt = lj_strscan_scan((const uint8_t *)strdata(str), o, STRSCAN_OPT_TONUM);
...
{
    StrScanFmt fmt = lj_strscan_scan((const uint8_t *)strdata(str), o, STRSCAN_OPT_TOINT);
