...
{
    IRIns *ir = IR(ref);
    if (ir->o == IR_TNEW && ir->op1 <= LJ_MAX_COLOSIZE && !neverfuse(as) && noconflict(as, ref, IR_NEWREF))
...
static void asm_fusexref(ASMState *as, MIPSIns mi, Reg rt, IRRef ref, RegSet allow, int32_t ofs)
{
    IRIns *ir = IR(ref);
    Reg base;
    if (ra_noreg(ir->r) && canfuse(as, ir)) {
        if (ir->o == IR_ADD) {
            intptr_t ofs2;
            if (irref_isk(ir->op2) && (ofs2 = ofs + get_kval(IR(ir->op2)), checki16(ofs2))) {
                ref = ir->op1;
                ofs = (int32_t)ofs2;
            }
...
static void asm_gencall(ASMState *as, const CCallInfo *ci, IRRef *args)
{
    ...
    for (n = 0; n < nargs; n++) { /* Setup args. */
        ...
        if (ref) {
            IRIns *ir = IR(ref);
#if !LJ_SOFTFP
            if (irt_isfp(ir->t) && fpr <= REGARG_LASTFPR && !(ci->flags & CCI_VARARG)) {
...
static void asm_callround(ASMState *as, IRIns *ir, IRCallID id)
{
    ...
    RegSet drop = RID2RSET(RID_R1) | RID2RSET(RID_R12) | RID2RSET(RID_FPRET) | RID2RSET(RID_F2) | RID2RSET(RID_F4) | RID2RSET(REGARG_FIRSTFPR);
    if (ra_hasreg(ir->r))
...
static void asm_retf(ASMState *as, IRIns *ir)
{
    ...
    asm_guard(as, MIPSI_BNE, RID_TMP, ra_allock(as, igcptr(pc), rset_exclude(RSET_GPR, base)));
...
static void asm_conv(ASMState *as, IRIns *ir)
{
    ...
    IRRef lref = ir->op1;
#if LJ_32
    lua_assert(!(irt_isint64(ir->t) || (st == IRT_I64 || st == IRT_U64))); /* Handled by SPLIT. */
#endif
#if LJ_32 && LJ_SOFTFP
    ...
    if (irt_isfp(ir->t)) {
        ...
        if (stfp) { /* FP to FP conversion. */
            emit_fg(as, st == IRT_NUM ? MIPSI_CVT_S_D : MIPSI_CVT_D_S, dest, ra_alloc1(as, lref, RSET_FPR));
            ...
            emit_lsptr(as, MIPSI_LDC1, (tmp & 31), (void *)&as->J->k64[LJ_K64_2P31], RSET_GPR);
            ...
            if (irt_isfloat(ir->t)) {
                ...
                emit_lsptr(as, MIPSI_LWC1, (tmp & 31), (void *)&as->J->k32[LJ_K32_2P63], rset_exclude(RSET_GPR, left));
                emit_fg(as, MIPSI_CVT_S_L, dest, dest);
            } else {
                emit_fgh(as, MIPSI_ADD_D, dest, dest, tmp);
                emit_lsptr(as, MIPSI_LDC1, (tmp & 31), (void *)&as->J->k64[LJ_K64_2P63], rset_exclude(RSET_GPR, left));
            ...
            Reg left = ra_alloc1(as, lref, RSET_GPR);
#if LJ_32
            emit_fg(as, irt_isfloat(ir->t) ? MIPSI_CVT_S_W : MIPSI_CVT_D_W, dest, dest);
            emit_tg(as, MIPSI_MTC1, left, dest);
#else
            MIPSIns mi = irt_isfloat(ir->t) ? (st64 ? MIPSI_CVT_S_L : MIPSI_CVT_S_W) : (st64 ? MIPSI_CVT_D_L : MIPSI_CVT_D_W);
        ...
        if (irt_isguard(ir->t)) {
            ...
            if (irt_isu32(ir->t)) { /* FP to U32 conversion. */
                ...
                emit_ti(as, MIPSI_LUI, RID_TMP, 0x8000);
                emit_tg(as, MIPSI_MFC1, dest, tmp);
                emit_fg(as, st == IRT_FLOAT ? MIPSI_FLOOR_W_S : MIPSI_FLOOR_W_D, tmp, tmp);
                emit_fgh(as, st == IRT_FLOAT ? MIPSI_SUB_S : MIPSI_SUB_D, tmp, left, tmp);
                if (st == IRT_FLOAT)
                    emit_lsptr(as, MIPSI_LWC1, (tmp & 31), (void *)&as->J->k32[LJ_K32_2P31], RSET_GPR);
                else
                    emit_lsptr(as, MIPSI_LDC1, (tmp & 31), (void *)&as->J->k64[LJ_K64_2P31], RSET_GPR);
#if LJ_64
                ...
                if (st == IRT_NUM) {
                    ...
                    emit_lsptr(as, MIPSI_LDC1, (tmp & 31), (void *)&as->J->k64[LJ_K64_M2P64], rset_exclude(RSET_GPR, dest));
                    emit_fg(as, MIPSI_TRUNC_L_D, tmp, left); /* Delay slot. */
                    emit_branch(as, MIPSI_BC1T, 0, 0, l_end);
                    emit_fgh(as, MIPSI_C_OLT_D, 0, left, tmp);
                    emit_lsptr(as, MIPSI_LDC1, (tmp & 31), (void *)&as->J->k64[LJ_K64_2P63], rset_exclude(RSET_GPR, dest));
                } else {
                    emit_fg(as, MIPSI_TRUNC_L_S, tmp, tmp);
                    emit_fgh(as, MIPSI_ADD_S, tmp, left, tmp);
                    emit_lsptr(as, MIPSI_LWC1, (tmp & 31), (void *)&as->J->k32[LJ_K32_M2P64], rset_exclude(RSET_GPR, dest));
                    emit_fg(as, MIPSI_TRUNC_L_S, tmp, left); /* Delay slot. */
                    emit_branch(as, MIPSI_BC1T, 0, 0, l_end);
                    emit_fgh(as, MIPSI_C_OLT_S, 0, left, tmp);
                    emit_lsptr(as, MIPSI_LWC1, (tmp & 31), (void *)&as->J->k32[LJ_K32_2P63], rset_exclude(RSET_GPR, dest));
                }
#endif
            } else {
#if LJ_32
                emit_tg(as, MIPSI_MFC1, dest, tmp);
                emit_fg(as, st == IRT_FLOAT ? MIPSI_TRUNC_W_S : MIPSI_TRUNC_W_D, tmp, left);
#else
                MIPSIns mi = irt_is64(ir->t) ? (st == IRT_NUM ? MIPSI_TRUNC_L_D : MIPSI_TRUNC_L_S) : (st == IRT_NUM ? MIPSI_TRUNC_W_D : MIPSI_TRUNC_W_S);
...
#endif
    {
        ...
        if (st >= IRT_I8 && st <= IRT_U16) { /* Extend to 32 bit integer. */
            ...
            if ((ir->op2 & IRCONV_SEXT)) {
                ...
                emit_tsi(as, MIPSI_ANDI, dest, left, (int32_t)(st == IRT_U8 ? 0xff : 0xffff));
            }
...
static void asm_strto(ASMState *as, IRIns *ir)
{
    ...
    if (ra_used(ir)) {
        if (ra_hasspill(ir->s) && ra_hasspill((ir + 1)->s) && (ir->s & 1) == LJ_BE && (ir->s ^ 1) == (ir + 1)->s) {
            int i;
            for (i = 0; i < 2; i++) {
                Reg r = (ir + i)->r;
                if (ra_hasreg(r)) {
    ...
    RegSet drop = RSET_SCRATCH;
    if (ra_hasreg(ir->r))
        rset_set(drop, ir->r); /* Spill dest reg (if any). */
    ra_evictset(as, drop);
    ofs = sps_scale(ir->s);
#endif
    ...
    emit_tsi(as, MIPSI_AADDIU, ra_releasetmp(as, ASMREF_TMP1), RID_SP, ofs);
}
...
static void asm_tvstore64(ASMState *as, Reg base, int32_t ofs, IRRef ref)
{
    ...
    if (irref_isk(ref)) {
        ...
        Reg type = ra_allock(as, (int64_t)irt_toitype(ir->t) << 47, rset_exclude(allow, src));
...
static void asm_tvptr(ASMState *as, Reg dest, IRRef ref)
{
    ...
    if (irt_isnum(ir->t)) {
        ...
        emit_tsi(as, MIPSI_DADDIU, dest, RID_JGL, (int32_t)(offsetof(global_State, tmptv) - 32768));
#endif
    }
}
...
nolo:
    ...
    if (ra_hasreg(key))
        emit_tsi(as, MIPSI_LW, key, idx, kofs + (LJ_BE ? 4 : 0));
...
static void asm_uref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ir->op1)) {
        ...
        emit_tsi(as, MIPSI_AL, uv, func, (int32_t)offsetof(GCfuncL, uvptr) + (int32_t)sizeof(MRef) * (int32_t)(ir->op2 >> 8));
    }
}
...
static void asm_strref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ref)) {
        ...
        if (ra_hasreg(irr->r)) {
            ra_noweak(as, irr->r);
            right = irr->r;
        } else if (mayfuse(as, irr->op2) && irr->o == IR_ADD && irref_isk(irr->op2) && checki16(ofs + IR(irr->op2)->i)) {
            ofs += IR(irr->op2)->i;
    ...
    if (checki16(ofs))
        ...
        emit_dst(as, MIPSI_ADDU, dest, r, ra_allock(as, ofs, rset_exclude(RSET_GPR, r)));
#else
    RegSet allow = RSET_GPR;
    Reg dest = ra_dest(as, ir, allow);
...
static MIPSIns asm_fxloadins(IRIns *ir)
{
    ...
    case IRT_FLOAT:
        if (!LJ_SOFTFP)
            return MIPSI_LWC1;
    default:
        return (LJ_64 && irt_is64(ir->t)) ? MIPSI_LD : MIPSI_LW;
    }
}
...
static MIPSIns asm_fxstoreins(IRIns *ir)
{
    ...
    case IRT_FLOAT:
        if (!LJ_SOFTFP)
            return MIPSI_SWC1;
    default:
        return (LJ_64 && irt_is64(ir->t)) ? MIPSI_SD : MIPSI_SW;
    }
}
...
{
    Reg dest = ra_dest(as, ir, (!LJ_SOFTFP && irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR);
...
{
    if (ir->r != RID_SINK) {
        Reg src = ra_alloc1z(as, ir->op2, (!LJ_SOFTFP && irt_isfp(ir->t)) ? RSET_FPR : RSET_GPR);
        asm_fusexref(as, asm_fxstoreins(ir), src, ir->op1, rset_exclude(RSET_GPR, src), ofs);
    }
}
...
static void asm_ahuvload(ASMState *as, IRIns *ir)
{
    ...
    if (ra_used(ir)) {
        lua_assert((LJ_SOFTFP ? 0 : irt_isnum(ir->t)) || irt_isint(ir->t) || irt_isaddr(ir->t));
    ...
    if (irt_isnum(t)) {
        ...
        asm_guard(as, MIPSI_BNE, type, ra_allock(as, (int32_t)irt_toitype(t), allow));
    }
#if LJ_32
    if (ra_hasreg(dest)) {
...
dotypecheck:
    ...
    if ((ir->op2 & IRSLOAD_TYPECHECK)) {
        ...
        if (irt_ispri(t)) {
            asm_guard(as, MIPSI_BNE, type, ra_allock(as, ~((int64_t)~irt_toitype(t) << 47), allow));
        } else {
            if (irt_isnum(t)) {
                asm_guard(as, MIPSI_BEQ, RID_TMP, RID_ZERO);
                ...
                asm_guard(as, MIPSI_BNE, RID_TMP, ra_allock(as, (int32_t)irt_toitype(t), allow));
            }
        ...
        if (irt_isnum(t))
            ...
            emit_tsi(as, irt_isint(t) ? MIPSI_LW : MIPSI_LD, dest, base, ofs ^ ((LJ_BE && irt_isint(t)) ? 4 : 0));
    }
#endif
}
...
#else
        emit_tsi(as, MIPSI_SD, ra_alloc1(as, ir->op2, allow), RID_RET, sizeof(GCcdata));
#endif
...
static void asm_obar(ASMState *as, IRIns *ir)
{
    ...
    emit_tsi(as, MIPSI_LBU, tmp, obj, (int32_t)offsetof(GCupval, marked) - (int32_t)offsetof(GCupval, tv));
...
static void asm_add(ASMState *as, IRIns *ir)
{
...
#endif
    {
        ...
        if (irref_isk(ir->op2)) {
            ...
            if (checki16(k)) {
                emit_tsi(as, (LJ_64 && irt_is64(t)) ? MIPSI_DADDIU : MIPSI_ADDIU, dest, left, k);
                return;
            }
        }
        right = ra_alloc1(as, ir->op2, rset_exclude(RSET_GPR, left));
        emit_dst(as, (LJ_64 && irt_is64(t)) ? MIPSI_DADDU : MIPSI_ADDU, dest, left, right);
    }
}
...
static void asm_sub(ASMState *as, IRIns *ir)
{
...
#endif
    {
        ...
        right = (left >> 8);
        left &= 255;
        emit_dst(as, (LJ_64 && irt_is64(ir->t)) ? MIPSI_DSUBU : MIPSI_SUBU, dest, left, right);
    }
}
...
static void asm_neg(ASMState *as, IRIns *ir)
{
...
#endif
    {
        ...
        emit_dst(as, (LJ_64 && irt_is64(ir->t)) ? MIPSI_DSUBU : MIPSI_SUBU, dest, RID_ZERO, left);
    }
}
...
static void asm_arithov(ASMState *as, IRIns *ir)
{
    ...
    right = (left >> 8);
    left &= 255;
    tmp = ra_scratch(as, rset_exclude(rset_exclude(rset_exclude(RSET_GPR, left), right), dest));
...
static void asm_bitshift(ASMState *as, IRIns *ir, MIPSIns mi, MIPSIns mik)
{
    ...
    if (irref_isk(ir->op2)) { /* Constant shifts. */
        ...
        if (LJ_64 && irt_is64(ir->t))
            mik |= (shift & 32) ? MIPSI_D32 : MIPSI_D;
        emit_dta(as, mik, dest, ra_hintalloc(as, ir->op1, dest, RSET_GPR), (shift & 31));
    } else {
        Reg right, left = ra_alloc2(as, ir, RSET_GPR);
        right = (left >> 8);
        left &= 255;
...
static void asm_min_max(ASMState *as, IRIns *ir, int ismax)
{
    if (!LJ_SOFTFP && irt_isnum(ir->t)) {
        ...
        if (dest == left) {
            ...
            if (dest != right)
                emit_move(as, dest, right);
        }
        emit_dst(as, MIPSI_SLT, RID_TMP, ismax ? left : right, ismax ? right : left);
    }
}
...
static void asm_sfpcomp(ASMState *as, IRIns *ir)
{
    ...
    Reg r;
    IRRef args[4];
    args[LJ_LE ? 0 : 1] = ir->op1;
    args[LJ_LE ? 1 : 0] = (ir + 1)->op1;
    args[LJ_LE ? 2 : 3] = ir->op2;
    args[LJ_LE ? 3 : 2] = (ir + 1)->op2;

    for (r = REGARG_FIRSTGPR; r <= REGARG_FIRSTGPR + 3; r++) {
        if (!rset_test(as->freeset, r) && regcost_ref(as->cost[r]) == args[r - REGARG_FIRSTGPR])
            rset_clear(drop, r);
    }
    ra_evictset(as, drop);
...
static void asm_comp(ASMState *as, IRIns *ir)
{
    ...
    if (!LJ_SOFTFP && irt_isnum(ir->t)) {
        ...
        if ((op & 4) == 0 && irref_isk(ir->op2) && get_kval(IR(ir->op2)) == 0) {
            MIPSIns mi = (op & 2) ? ((op & 1) ? MIPSI_BLEZ : MIPSI_BGTZ) : ((op & 1) ? MIPSI_BLTZ : MIPSI_BGEZ);
            ...
            if (irref_isk(ir->op2)) {
                ...
                if (checki16(k)) {
                    ...
                    emit_tsi(as, (op & 4) ? MIPSI_SLTIU : MIPSI_SLTI, RID_TMP, left, k);
                    return;
                }
            }
            right = ra_alloc1(as, ir->op2, rset_exclude(RSET_GPR, left));
            asm_guard(as, ((op ^ (op >> 1)) & 1) ? MIPSI_BNE : MIPSI_BEQ, RID_TMP, RID_ZERO);
            emit_dst(as, (op & 4) ? MIPSI_SLTU : MIPSI_SLT, RID_TMP, (op & 2) ? right : left, (op & 2) ? left : right);
        }
    }
}
...
static void asm_comp64(ASMState *as, IRIns *ir)
{
    ...
    righthi = (lefthi >> 8);
    lefthi &= 255;
    leftlo = ra_alloc2(as, ir - 1, rset_exclude(rset_exclude(RSET_GPR, lefthi), righthi));
    rightlo = (leftlo >> 8);
    leftlo &= 255;
    asm_guard(as, ((op ^ (op >> 1)) & 1) ? MIPSI_BNE : MIPSI_BEQ, RID_TMP, RID_ZERO);
    l_end = emit_label(as);
    if (lefthi != righthi)
        emit_dst(as, (op & 4) ? MIPSI_SLTU : MIPSI_SLT, RID_TMP, (op & 2) ? righthi : lefthi, (op & 2) ? lefthi : righthi);
    emit_dst(as, MIPSI_SLTU, RID_TMP, (op & 2) ? rightlo : leftlo, (op & 2) ? leftlo : rightlo);
    if (lefthi != righthi)
...
static void asm_prof(ASMState *as, IRIns *ir)
{
    ...
    emit_lsglptr(as, MIPSI_LBU, RID_TMP, (int32_t)offsetof(global_State, hookmask));
}
...
#if LJ_32
    tmp = allow ? rset_pickbot(allow) : (pbase == RID_RETHI ? RID_RETLO : RID_RETHI);
#else
...
static void asm_stack_restore(ASMState *as, SnapShot *snap)
{
    ...
    for (n = 0; n < nent; n++) {
        ...
        if (irt_isnum(ir->t)) {
            ...
            if (rset_test(as->freeset, tmp + 1))
                allow = RID2RSET(tmp + 1);
            ...
            if ((sn & (SNAP_CONT | SNAP_FRAME))) {
                if (s == 0)
                    continue; /* Do not overwrite link to previous frame. */
...
static Reg asm_setup_call_slots(ASMState *as, IRIns *ir, const CCallInfo *ci)
{
    ...
    for (i = 0; i < nargs; i++) {
#if LJ_32
        if (!LJ_SOFTFP && args[i] && irt_isfp(IR(args[i])->t) && nfpr > 0 && !(ci->flags & CCI_VARARG)) {
            nfpr--;
