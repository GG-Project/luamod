...
GCtrace *LJ_FASTCALL lj_trace_alloc(lua_State *L, GCtrace *T)
{
    ...
    size_t sz = sztr + szins + T->nsnap * sizeof(SnapShot) + T->nsnapmap * sizeof(SnapEntry);
...
void LJ_FASTCALL lj_trace_free(global_State *g, GCtrace *T)
{
    ...
    lj_mem_free(g, T, ((sizeof(GCtrace) + 7) & ~7) + (T->nins - T->nk) * sizeof(IRIns) + T->nsnap * sizeof(SnapShot) + T->nsnapmap * sizeof(SnapEntry));
}
...
static void trace_start(jit_State *J)
{
    ...
    if ((J->pt->flags & PROTO_NOJIT)) { /* JIT disabled for this proto? */
        if (J->parent == 0 && J->exitno == 0) {
            ...
            lua_assert(bc_op(*J->pc) == BC_FORL || bc_op(*J->pc) == BC_ITERL || bc_op(*J->pc) == BC_LOOP || bc_op(*J->pc) == BC_FUNCF);
    ...
    L = J->L;
    lj_vmevent_send(
        L, TRACE, setstrV(L, L->top++, lj_str_newlit(L, "start")); setintV(L->top++, traceno); setfuncV(L, L->top++, J->fn); setintV(L->top++, proto_bcpos(J->pt, J->pc)); if (J->parent) {
...
static void trace_stop(jit_State *J)
{
    ...
    L = J->L;
    lj_vmevent_send(L, TRACE, setstrV(L, L->top++, lj_str_newlit(L, "stop")); setintV(L->top++, traceno); setfuncV(L, L->top++, J->fn););
}
...
static int trace_abort(jit_State *J)
{
    ...
    if (traceno) {
        ...
        lj_vmevent_send(
            L, TRACE, TValue * frame; const BCIns *pc; GCfunc * fn; setstrV(L, L->top++, lj_str_newlit(L, "abort")); setintV(L->top++, traceno);
            /* Find original Lua function call to generate a better error message. */
            frame = J->L->base - 1;
            pc = J->pc; while (!isluafunc(frame_func(frame))) {
                pc = (frame_iscont(frame) ? frame_contpc(frame) : frame_pc(frame)) - 1;
                frame = frame_prev(frame);
            } fn
            = frame_func(frame);
            setfuncV(L, L->top++, fn); setintV(L->top++, proto_bcpos(funcproto(fn), pc)); copyTV(L, L->top++, restorestack(L, errobj)); copyTV(L, L->top++, &J->errinfo););
    ...
    retry:
        ...
        case LJ_TRACE_RECORD:
            ...
            lj_vmevent_send_(L, RECORD,
                             ...
                             TValue savetv2 = J2G(J)->tmptv2; setintV(L->top++, J->cur.traceno); setfuncV(L, L->top++, J->fn); setintV(L->top++, J->pt ? (int32_t)proto_bcpos(J->pt, J->pc) : -1); setintV(L->top++, J->framedepth);, J2G(J)->tmptv = savetv; J2G(J)->tmptv2 = savetv2;);
            lj_record_ins(J);
            break;
        ...
        case LJ_TRACE_END:
            trace_pendpatch(J, 1);
            J->loopref = 0;
            if ((J->flags & JIT_F_OPT_LOOP) && J->cur.link == J->cur.traceno && J->framedepth + J->retdepth == 0) {
...
void LJ_FASTCALL lj_trace_hot(jit_State *J, const BCIns *pc)
{
    ...
    if (J->state == LJ_TRACE_IDLE && !(J2G(J)->hookmask & (HOOK_GC | HOOK_VMEVENT))) {
...
static void trace_hotside(jit_State *J, const BCIns *pc)
{
    ...
    if (!(J2G(J)->hookmask & (HOOK_GC | HOOK_VMEVENT)) && isluafunc(curr_func(J->L)) && snap->count != SNAPCOUNT_DONE && ++snap->count >= J->param[JIT_P_hotexit]) {
...
int LJ_FASTCALL lj_trace_exit(jit_State *J, void *exptr)
{
    ...
    if (!(LJ_HASPROFILE && (G(L)->hookmask & HOOK_PROFILE)))
        lj_vmevent_send(L, TEXIT, lj_state_checkstack(L, 4 + RID_NUM_GPR + RID_NUM_FPR + LUA_MINSTACK); setintV(L->top++, J->parent); setintV(L->top++, J->exitno); trace_exit_regs(L, ex););
