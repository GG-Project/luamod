...
static void asm_fusexref(ASMState *as, PPCIns pi, Reg rt, IRRef ref, RegSet allow, int32_t ofs)
{
    IRIns *ir = IR(ref);
    Reg base;
...
static void asm_fusexrefx(ASMState *as, PPCIns pi, Reg rt, IRRef ref, RegSet allow)
{
    IRIns *ira = IR(ref);
    Reg right, left;
...
static int asm_fusemadd(ASMState *as, IRIns *ir, PPCIns pi, PPCIns pir)
{
    ...
    IRIns *irm;
    if (lref != rref && ((mayfuse(as, lref) && (irm = IR(lref), irm->o == IR_MUL) && ra_noreg(irm->r)) || (mayfuse(as, rref) && (irm = IR(rref), irm->o == IR_MUL) && (rref = lref, pi = pir, ra_noreg(irm->r))))) {
...
static void asm_retf(ASMState *as, IRIns *ir)
{
    ...
    emit_ab(as, PPCI_CMPW, RID_TMP, ra_allock(as, i32ptr(pc), rset_exclude(RSET_GPR, base)));
...
static void asm_tointg(ASMState *as, IRIns *ir, Reg left)
{
    ...
    emit_lsptr(as, PPCI_LFS, (fbias & 31), (void *)&as->J->k32[LJ_K32_2P52_2P31], RSET_GPR);
...
static void asm_conv(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isfp(ir->t)) {
        ...
        if (stfp) { /* FP to FP conversion. */
            ...
            if (irt_isfloat(ir->t))
                emit_fb(as, PPCI_FRSP, dest, dest);
            emit_fab(as, PPCI_FSUB, dest, dest, fbias);
            emit_fai(as, PPCI_LFD, dest, RID_SP, SPOFS_TMP);
            emit_lsptr(as, PPCI_LFS, (fbias & 31), &as->J->k32[st == IRT_U32 ? LJ_K32_2P52 : LJ_K32_2P52_2P31], rset_clear(allow, hibias));
            emit_tai(as, PPCI_STW, st == IRT_U32 ? left : RID_TMP, RID_SP, SPOFS_TMPLO);
            emit_tai(as, PPCI_STW, hibias, RID_SP, SPOFS_TMPHI);
            if (st != IRT_U32)
                emit_asi(as, PPCI_XORIS, RID_TMP, left, 0x8000);
        }
        ...
        if (irt_isguard(ir->t)) {
            ...
            if (irt_isu32(ir->t)) {
                ...
                emit_fai(as, PPCI_STFD, tmp, RID_SP, SPOFS_TMP);
                emit_tai(as, PPCI_LWZ, dest, RID_SP, SPOFS_TMPLO); /* dest = (int)(x-2^31) */
                emit_fb(as, PPCI_FCTIWZ, tmp, left);
                emit_fai(as, PPCI_STFD, tmp, RID_SP, SPOFS_TMP);
                emit_fb(as, PPCI_FCTIWZ, tmp, tmp);
                emit_fab(as, PPCI_FSUB, tmp, left, tmp);
                emit_lsptr(as, PPCI_LFS, (tmp & 31), (void *)&as->J->k32[LJ_K32_2P31], RSET_GPR);
            } else {
                emit_tai(as, PPCI_LWZ, dest, RID_SP, SPOFS_TMPLO);
                emit_fai(as, PPCI_STFD, tmp, RID_SP, SPOFS_TMP);
                emit_fb(as, PPCI_FCTIWZ, tmp, left);
...
static void asm_href(ASMState *as, IRIns *ir, IROp merge)
{
    ...
    *l_loop = PPCI_BC | PPCF_Y | PPCF_CC(CC_NE) | (((char *)as->mcp - (char *)l_loop) & 0xffffu);
...
static void asm_hrefk(ASMState *as, IRIns *ir)
{
    ...
    if (ra_hasreg(key))
        emit_tai(as, PPCI_LWZ, key, idx, kofs + 4);
...
static void asm_uref(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ir->op1)) {
        ...
        emit_tai(as, PPCI_LWZ, uv, func, (int32_t)offsetof(GCfuncL, uvptr) + 4 * (int32_t)(ir->op2 >> 8));
    }
}
...
static void asm_strref(ASMState *as, IRIns *ir)
{
    ...
    if (checki16(ofs))
        ...
        emit_tab(as, PPCI_ADD, dest, r, ra_allock(as, ofs, rset_exclude(RSET_GPR, r)));
}
...
static PPCIns asm_fxloadins(IRIns *ir)
{
    ...
    case IRT_I8:
        return PPCI_LBZ; /* Needs sign-extension. */
    case IRT_U8:
        return PPCI_LBZ;
    case IRT_I16:
        return PPCI_LHA;
...
static void asm_xstore_(ASMState *as, IRIns *ir, int32_t ofs)
{
    ...
    if (ir->r == RID_SINK)
        return;
    if (ofs == 0 && mayfuse(as, ir->op2) && (irb = IR(ir->op2))->o == IR_BSWAP && ra_noreg(irb->r) && (irt_isint(ir->t) || irt_isu32(ir->t))) {
...
static void asm_ahuvload(ASMState *as, IRIns *ir)
{
    ...
    if (irt_isnum(t)) {
        ...
        if (ra_hasreg(dest)) {
            if (ofs == AHUREF_LSX) {
                tmp = ra_scratch(as, rset_exclude(rset_exclude(RSET_GPR, (idx & 255)), (idx >> 8)));
        ...
        if (ra_hasreg(dest))
            emit_tai(as, PPCI_LWZ, dest, idx, ofs + 4);
    }
    if (ofs == AHUREF_LSX) {
    ...
    if ((ir->op2 & IRSLOAD_CONVERT) && irt_isguard(t) && irt_isint(t)) {
        ...
        if ((ir->op2 & IRSLOAD_CONVERT)) {
            if (irt_isint(t)) {
                ...
                emit_fab(as, PPCI_FSUB, dest, dest, fbias);
                emit_fai(as, PPCI_LFD, dest, RID_SP, SPOFS_TMP);
                emit_lsptr(as, PPCI_LFS, (fbias & 31), (void *)&as->J->k32[LJ_K32_2P52_2P31], rset_clear(allow, hibias));
...
dotypecheck:
    if (irt_isnum(t)) {
        ...
        if (ra_hasreg(dest))
            emit_fai(as, PPCI_LFD, dest, base, ofs - 4);
    } else {
        if ((ir->op2 & IRSLOAD_TYPECHECK)) {
            asm_guardcc(as, CC_NE);
    ...
    if (ra_hasreg(type))
        emit_tai(as, PPCI_LWZ, type, base, ofs - 4);
}
...
static void asm_obar(ASMState *as, IRIns *ir)
{
    ...
    emit_tai(as, PPCI_LBZ, tmp, obj, (int32_t)offsetof(GCupval, marked) - (int32_t)offsetof(GCupval, tv));
...
static void asm_bswap(ASMState *as, IRIns *ir)
{
    Reg dest = ra_dest(as, ir, RSET_GPR);
    IRIns *irx;
    if (mayfuse(as, ir->op1) && (irx = IR(ir->op1))->o == IR_XLOAD && ra_noreg(irx->r) && (irt_isint(irx->t) || irt_isu32(irx->t))) {
...
static void asm_fuseandsh(ASMState *as, PPCIns pi, int32_t mask, IRRef ref)
{
    IRIns *ir;
    Reg left;
    if (mayfuse(as, ref) && (ir = IR(ref), ra_noreg(ir->r)) && irref_isk(ir->op2) && ir->o >= IR_BSHL && ir->o <= IR_BROR) {
...
static void asm_band(ASMState *as, IRIns *ir)
{
    ...
    if (irref_isk(ir->op2)) {
        ...
        if (k) {
            ...
            if ((k1 & (k1 + 1)) == 0) {
                asm_fuseandsh(as, PPCI_RLWINM | dot | PPCF_A(dest) | PPCF_MB(31 - lj_fls((uint32_t)k)) | PPCF_ME(31 - s1), k, lref);
                return;
            }
            if (~(uint32_t)k) {
                ...
                if ((k2 & (k2 + 1)) == 0) {
                    asm_fuseandsh(as, PPCI_RLWINM | dot | PPCF_A(dest) | PPCF_MB(32 - s2) | PPCF_ME(30 - lj_fls(~(uint32_t)k)), k, lref);
                    return;
                }
            }
        }
...
#define asm_brol(as, ir) asm_bitshift(as, ir, PPCI_RLWNM | PPCF_MB(0) | PPCF_ME(31), PPCI_RLWINM | PPCF_MB(0) | PPCF_ME(31))
...
static void asm_min_max(ASMState *as, IRIns *ir, int ismax)
{
    if (irt_isnum(ir->t)) {
        ...
        if (tmp == left || tmp == right)
            tmp = ra_scratch(as, rset_exclude(rset_exclude(rset_exclude(RSET_FPR, dest), left), right));
        emit_facb(as, PPCI_FSEL, dest, tmp, ismax ? left : right, ismax ? right : left);
        ...
        if (tmp2 == left || tmp2 == right)
            tmp2 = ra_scratch(as, rset_exclude(rset_exclude(rset_exclude(RSET_GPR, dest), left), right));
...
static void asm_comp64(ASMState *as, IRIns *ir)
{
    ...
    if ((cc & 3) == (CC_EQ & 3)) {
        asm_guardcc(as, cc);
        emit_tab(as, (cc & 4) ? PPCI_CRAND : PPCI_CROR, (CC_EQ & 3), (CC_EQ & 3), 4 + (CC_EQ & 3));
    } else {
        asm_guardcc(as, CC_EQ);
        emit_tab(as, PPCI_CROR, (CC_EQ & 3), (CC_EQ & 3), ((cc ^ ~(cc >> 2)) & 1));
        emit_tab(as, (cc & 4) ? PPCI_CRAND : PPCI_CRANDC, (CC_EQ & 3), (CC_EQ & 3), 4 + (cc & 3));
    }
    /* Loword comparison sets cr1 and is unsigned, except for equality. */
    asm_intcomp_(as, (ir - 1)->op1, (ir - 1)->op2, 4, cc | ((cc & 3) == (CC_EQ & 3) ? 0 : CC_UNSIGNED));
...
static void asm_prof(ASMState *as, IRIns *ir)
{
    ...
    emit_lsglptr(as, PPCI_LBZ, RID_TMP, (int32_t)offsetof(global_State, hookmask));
}
...
static void asm_setup_target(ASMState *as) { asm_exitstub_setup(as, as->T->nsnap + (as->parent ? 1 : 0)); }
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
    ...
    for (; p < pe; p++) {
        ...
        uint32_t ins = *p;
        if ((ins & 0xfc000000u) == 0x40000000u && ((ins ^ ((char *)px - (char *)p)) & 0xffffu) == 0) {
            ...
            if (((delta + 0x8000) >> 16) == 0) {
                *p = (ins & 0xffdf0000u) | ((uint32_t)delta & 0xffffu) | ((delta & 0x8000) * (PPCF_Y / 0x8000));
                if (!cstart)
                    cstart = p;
            }
        } else if ((ins & 0xfc000000u) == PPCI_B && ((ins ^ ((char *)px - (char *)p)) & 0x03ffffffu) == 0) {
            ptrdiff_t delta = (char *)target - (char *)p;
