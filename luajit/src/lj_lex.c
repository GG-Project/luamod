...
/* Get next character. */
static LJ_AINLINE LexChar lex_next(LexState *ls) { return (ls->c = ls->p < ls->pe ? (LexChar)(uint8_t)*ls->p++ : lex_more(ls)); }

/* Save character. */
static LJ_AINLINE void lex_save(LexState *ls, LexChar c) { lj_buf_putb(&ls->sb, c); }
...
static void lex_number(LexState *ls, TValue *tv)
{
    ...
    lex_save(ls, '\0');
    fmt = lj_strscan_scan((const uint8_t *)sbufB(&ls->sb), tv, (LJ_DUALNUM ? STRSCAN_OPT_TOINT : STRSCAN_OPT_TONUM) | (LJ_HASFFI ? (STRSCAN_OPT_LL | STRSCAN_OPT_IMAG) : 0));
...
static void lex_longstring(LexState *ls, TValue *tv, int sep)
{
    ...
    for (;;) {
        ...
        case '\r':
            ...
            lex_newline(ls);
            if (!tv)
                lj_buf_reset(&ls->sb); /* Don't waste space for comments. */
            break;
        default:
            lex_savenext(ls);
            break;
...
endloop:
    if (tv) {
        GCstr *str = lj_parse_keepstr(ls, sbufB(&ls->sb) + (2 + (MSize)sep), sbuflen(&ls->sb) - 2 * (2 + (MSize)sep));
        setstrV(ls->L, tv, str);
    }
}
...
static void lex_string(LexState *ls, TValue *tv)
{
    ...
    setstrV(ls->L, tv, lj_parse_keepstr(ls, sbufB(&ls->sb) + 1, sbuflen(&ls->sb) - 2));
}
...
int lj_lex_setup(lua_State *L, LexState *ls)
{
    ...
    if (ls->c == 0xef && ls->p + 2 <= ls->pe && (uint8_t)ls->p[0] == 0xbb && (uint8_t)ls->p[1] == 0xbf) { /* Skip UTF-8 BOM (if buffered). */
        ls->p += 2;
        lex_next(ls);
        header = 1;
    }
