...
static LJ_AINLINE void lj_buf_reset(SBuf *sb) { setmrefr(sb->p, sb->b); }
...
static LJ_AINLINE void lj_buf_free(global_State *g, SBuf *sb) { lj_mem_free(g, sbufB(sb), sbufsz(sb)); }
...
static LJ_AINLINE char *lj_buf_wmem(char *p, const void *q, MSize len) { return (char *)memcpy(p, q, len) + len; }
...
LJ_FUNC SBuf *lj_buf_puttab(SBuf *sb, GCtab *t, GCstr *sep, int32_t i, int32_t e);
...
static LJ_AINLINE GCstr *lj_buf_str(lua_State *L, SBuf *sb) { return lj_str_new(L, sbufB(sb), sbuflen(sb)); }
