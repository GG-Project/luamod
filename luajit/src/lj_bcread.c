...
#define bcread_swap(ls) ((bcread_flags(ls) & BCDUMP_F_BE) != LJ_BE * BCDUMP_F_BE)
#define bcread_oldtop(L, ls) restorestack(L, ls->lastline)
#define bcread_savetop(L, ls, top) ls->lastline = (BCLine)savestack(L, (top))
...
static void bcread_block(LexState *ls, void *q, MSize len) { memcpy(q, bcread_mem(ls, len), len); }
...
static void bcread_kgc(LexState *ls, GCproto *pt, MSize sizekgc)
{
    ...
    for (i = 0; i < sizekgc; i++, kr++) {
        ...
        if (tp >= BCDUMP_KGC_STR) {
            ...
            CTypeID id = tp == BCDUMP_KGC_COMPLEX ? CTID_COMPLEX_DOUBLE : tp == BCDUMP_KGC_I64 ? CTID_INT64 : CTID_UINT64;
...
static void bcread_bytecode(LexState *ls, GCproto *pt, MSize sizebc)
{
    ...
    bc[0] = BCINS_AD((pt->flags & PROTO_VARARG) ? BC_FUNCV : BC_FUNCF, pt->framesize, 0);
...
GCproto *lj_bcread_proto(LexState *ls)
{
    ...
    ofsk = sizept;
    sizept += sizekn * (MSize)sizeof(TValue);
    ofsuv = sizept;
    sizept += ((sizeuv + 1) & ~1) * 2;
    ofsdbg = sizept;
    sizept += sizedbg;
...
static int bcread_header(LexState *ls)
{
    ...
    if (bcread_byte(ls) != BCDUMP_HEAD2 || bcread_byte(ls) != BCDUMP_HEAD3 || bcread_byte(ls) != BCDUMP_VERSION)
        return 0;
...
GCproto *lj_bcread(LexState *ls)
{
    ...
    if ((int32_t)(2 * (uint32_t)(ls->pe - ls->p)) > 0 || L->top - 1 != bcread_oldtop(L, ls))
        bcread_error(ls, LJ_ERR_BCBAD);
