...
static void loop_emit_phi(jit_State *J, IRRef1 *subst, IRRef1 *phi, IRRef nphi, SnapNo onsnap)
{
    int passx = 0;
    IRRef i, j, nslots;
    ...
    if (passx) {
        ...
        for (i = J->cur.nins - 1; i > invar; i--) {
            IRIns *ir = IR(i);
            if (!irref_isk(ir->op2))
                irt_clearmark(IR(ir->op2)->t);
            if (!irref_isk(ir->op1)) {
                irt_clearmark(IR(ir->op1)->t);
                if (ir->op1 < invar && ir->o >= IR_CALLN && ir->o <= IR_CARG) { /* ORDER IR */
                    ir = IR(ir->op1);
                    while (ir->o == IR_CARG) {
                        if (!irref_isk(ir->op2))
                            irt_clearmark(IR(ir->op2)->t);
...
static void loop_unroll(LoopState *lps)
{
    ...
    *psentinel = SNAP(255, 0, 0); /* Replace PC with temporary sentinel. */
    ...
    for (ins = REF_FIRST; ins < invar; ins++) {
        ...
        if (!irref_isk(op2))
            op2 = subst[op2];
        if (irm_kind(lj_ir_mode[ir->o]) == IRM_N && op1 == ir->op1 && op2 == ir->op2) { /* Regular invariant ins? */
            ...
            if (ref != ins) {
                ...
                } else if (ref != REF_DROP && irr->o == IR_CONV && ref > invar && irr->op1 < invar) {
