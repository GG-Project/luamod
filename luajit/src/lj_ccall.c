...
#define CCALL_HANDLE_STRUCTARG ngpr = maxgpr; /* Pass all structs by value on the stack. */

#define CCALL_HANDLE_COMPLEXARG isfp = 1; /* Pass complex by value on stack. */
...
#define CCALL_HANDLE_STRUCTARG                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
    ...
    if (!ccall_classify_struct(cts, d, rcl, 0)) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
        ...
        cc->ngpr = ngpr;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
        cc->nfpr = nfpr;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
        if (ccall_struct_arg(cc, cts, d, rcl, o, narg))                                                                                                                                                                                                                                                                                                                                                                                                                                                            \
            goto err_nyi;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
        nsp = cc->nsp;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
        ngpr = cc->ngpr;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
        nfpr = cc->nfpr;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
...
#define CCALL_HANDLE_COMPLEXARG isfp = 2; /* Pass complex in FPRs or on stack. Needs postprocessing. */
...
#define CCALL_HANDLE_COMPLEXRET2 UNUSED(dp); /* Nothing to do. */

#define CCALL_HANDLE_STRUCTARG /* Pass all structs by value in registers and/or on the stack. */

#define CCALL_HANDLE_COMPLEXARG /* Pass complex by value in 2 or 4 GPRs. */
...
#define CCALL_HANDLE_STRUCTRET2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
    if (ccall_classify_struct(cts, ctr, ct) > 1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
        sp = (uint8_t *)&cc->fpr[0];                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
    memcpy(dp, sp, ctr->size);

#define CCALL_HANDLE_COMPLEXRET                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
    if (!(ct->info & CTF_VARARG))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
        cc->retref = 0; /* Return complex in FPRs. */

#define CCALL_HANDLE_COMPLEXRET2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
    if (!(ct->info & CTF_VARARG))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
        memcpy(dp, &cc->fpr[0], ctr->size);

#define CCALL_HANDLE_STRUCTARG isfp = (ccall_classify_struct(cts, d, ct) > 1);
/* Pass all structs by value in registers and/or on the stack. */

#define CCALL_HANDLE_COMPLEXARG isfp = 1; /* Pass complex by value in FPRs or on stack. */
...
#define CCALL_HANDLE_COMPLEXRET2 memcpy(dp, sp, ctr->size); /* Copy complex from GPRs. */
...
#define CCALL_HANDLE_STRUCTRET2 ccall_copy_struct(cc, ctr, dp, sp, ccall_classify_struct(cts, ctr, ct));
...
#define CCALL_HANDLE_STRUCTRET2 memcpy(dp, sp, ctr->size); /* Copy struct return value from GPRs. */
#endif
...
{
    CTSize sz = ct->size;
    if (!(sz == sizeof(float) || sz == sizeof(double)))
        return 0;
    if ((ct->info & CTF_UNION))
        return 0;
    while (ct->sib) {
...
{
    if (ct->size > 16)
        return CCALL_RCL_MEM; /* Too big, gets memory class. */
    while (ct->sib) {
        CTSize fofs;
        ct = ctype_get(cts, ct->sib);
        fofs = ofs + ct->size;
...
static int ccall_struct_arg(CCallState *cc, CTState *cts, CType *d, int *rcl, TValue *o, int narg)
{
    GPRArg dp[2];
    dp[0] = dp[1] = 0;
...
static unsigned int ccall_classify_struct(CTState *cts, CType *ct, CType *ctf)
{
    ...
    while (ct->sib) {
        ...
        } else if (ctype_isbitfield(ct->info) || ctype_isxattrib(ct->info, CTA_SUBTYPE)) {
            goto noth;
        }
    }
    if (n <= 2)
...
{
    if (LJ_ABI_SOFTFP ? ft : ((ft & 3) == FTYPE_FLOAT || (ft >> 2) == FTYPE_FLOAT)) {
        int i, ofs = 0;
        for (i = 0; ft != 0; i++, ft >>= 2) {
            if ((ft & 3) == FTYPE_FLOAT) {
#if LJ_ABI_SOFTFP
                /* The 2nd FP struct result is in CARG1 (gpr[2]) and not CRET2. */
                memcpy((uint8_t *)dp + ofs, (uint8_t *)&cc->gpr[2 * i] + LJ_ENDIAN_SELECT(0, 4), 4);
#else
...
CTypeID lj_ccall_ctid_vararg(CTState *cts, cTValue *o)
{
    if (tvisnumber(o)) {
        ...
        if (ctype_isrefarray(s->info)) {
            return lj_ctype_intern(cts, CTINFO(CT_PTR, CTALIGN_PTR | ctype_cid(s->info)), CTSIZE_PTR);
    ...
    for (o = L->base + 1, narg = 1; o < top; o++, narg++) {
        ...
        if (ctype_isinteger_or_bool(d->info) && d->size < 4) {
            if (d->info & CTF_UNSIGNED)
                *(uint32_t *)dp = d->size == 1 ? (uint32_t) * (uint8_t *)dp : (uint32_t) * (uint16_t *)dp;
            else
                *(int32_t *)dp = d->size == 1 ? (int32_t) * (int8_t *)dp : (int32_t) * (int16_t *)dp;
        }
...
static int ccall_get_results(lua_State *L, CTState *cts, CType *ct, CCallState *cc, int *ret)
{
    ...
    *ret = 1; /* One result. */
    ...
    if (LJ_BE && ctr->size < CTSIZE_PTR && (ctype_isinteger_or_bool(ctr->info) || ctype_isenum(ctr->info)))
