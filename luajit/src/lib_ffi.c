...
static int32_t ffi_checkint(lua_State *L, int narg)
{
    ...
    if (o >= L->top)
        lj_err_arg(L, narg, LJ_ERR_NOVAL);
    lj_cconv_ct_tv(cts, ctype_get(cts, CTID_INT32), (uint8_t *)&i, o, CCF_ARG(narg));
    return i;
}
...
static int ffi_index_meta(lua_State *L, CTState *cts, CType *ct, MMS mm)
{
    ...
    err_index:
        ...
        if (tvisstr(L->base + 1)) {
            ...
            const char *key = tviscdata(L->base + 1) ? strdata(lj_ctype_repr(L, cdataV(L->base + 1)->ctypeid, NULL)) : lj_typename(L->base + 1);
...
LJLIB_CF(ffi_meta___eq) LJLIB_REC(cdata_arith MM_eq) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___len) LJLIB_REC(cdata_arith MM_len) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___lt) LJLIB_REC(cdata_arith MM_lt) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___le) LJLIB_REC(cdata_arith MM_le) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___concat) LJLIB_REC(cdata_arith MM_concat) { return ffi_arith(L); }
...
LJLIB_CF(ffi_meta___add) LJLIB_REC(cdata_arith MM_add) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___sub) LJLIB_REC(cdata_arith MM_sub) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___mul) LJLIB_REC(cdata_arith MM_mul) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___div) LJLIB_REC(cdata_arith MM_div) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___mod) LJLIB_REC(cdata_arith MM_mod) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___pow) LJLIB_REC(cdata_arith MM_pow) { return ffi_arith(L); }

LJLIB_CF(ffi_meta___unm) LJLIB_REC(cdata_arith MM_unm) { return ffi_arith(L); }
...
LJLIB_CF(ffi_meta___tostring)
{
    ...
    if (id == CTID_CTYPEID) {
        ...
        if (ctype_iscomplex(ct->info)) {
            ...
            setstrV(L, L->top - 1, lj_ctype_repr_int64(L, *(uint64_t *)cdataptr(cd), (ct->info & CTF_UNSIGNED)));
            goto checkgc;
...
static int ffi_pairs(lua_State *L, MMS mm)
{
    ...
    tv = lj_ctype_meta(cts, id, mm);
    if (!tv)
        lj_err_callerv(L, LJ_ERR_FFI_BADMM, strdata(lj_ctype_repr(L, id, NULL)), strdata(mmname_str(G(L), mm)));
    return lj_meta_tailcall(L, tv);
}

LJLIB_CF(ffi_meta___pairs) { return ffi_pairs(L, MM_pairs); }

LJLIB_CF(ffi_meta___ipairs) { return ffi_pairs(L, MM_ipairs); }

LJLIB_PUSH("ffi")
LJLIB_SET(__metatable)
...
LJLIB_CF(ffi_callback_free) { return ffi_callback_set(L, NULL); }
...
LJLIB_CF(ffi_new) LJLIB_REC(.)
{
    ...
    lj_cconv_ct_init(cts, ct, sz, cdataptr(cd), o, (MSize)(L->top - o)); /* Initialize cdata. */
...
LJLIB_CF(ffi_istype) LJLIB_REC(.)
{
    ...
    if (tviscdata(o)) {
        GCcdata *cd = cdataV(o);
        CTypeID id2 = cd->ctypeid == CTID_CTYPEID ? *(CTypeID *)cdataptr(cd) : cd->ctypeid;
        CType *ct1 = lj_ctype_rawref(cts, id1);
        CType *ct2 = lj_ctype_rawref(cts, id2);
        if (ct1 == ct2) {
            b = 1;
        } else if (ctype_type(ct1->info) == ctype_type(ct2->info) && ct1->size == ct2->size) {
        ...
        } else if (ctype_isstruct(ct1->info) && ctype_isptr(ct2->info) && ct1 == ctype_rawchild(cts, ct2)) {
            b = 1;
        }
    }
    setboolV(L->top - 1, b);
...
LJLIB_CF(ffi_string) LJLIB_REC(.)
{
    ...
    if (o + 1 < L->top && !tvisnil(o + 1)) {
        ...
        lj_cconv_ct_tv(cts, ctype_get(cts, CTID_P_CVOID), (uint8_t *)&p, o, CCF_ARG(1));
    } else {
        lj_cconv_ct_tv(cts, ctype_get(cts, CTID_P_CCHAR), (uint8_t *)&p, o, CCF_ARG(1));
        len = strlen(p);
    }
    L->top = o + 1; /* Make sure this is the last item on the stack. */
...
LJLIB_CF(ffi_abi) LJLIB_REC(.)
{
    ...
    switch (s->hash) {
#if LJ_64
    case H_(849858eb, ad35fd06):
        b = 1;
        break; /* 64bit */
#else
    case H_(662d3c79, d0e22477):
        b = 1;
        break; /* 32bit */
#endif
#if LJ_ARCH_HASFPU
    case H_(e33ee463, e33ee463):
        b = 1;
        break; /* fpu */
#endif
#if LJ_ABI_SOFTFP
    case H_(61211a23, c2e8c81c):
        b = 1;
        break; /* softfp */
#else
    case H_(539417a8, 8ce0812f):
        b = 1;
        break; /* hardfp */
#endif
#if LJ_ABI_EABI
    case H_(2182df8f, f2ed1152):
        b = 1;
        break; /* eabi */
#endif
#if LJ_ABI_WIN
    case H_(4ab624a8, 4ab624a8):
        b = 1;
        break; /* win */
#endif
    case H_(3af93066, 1f001464):
        b = 1;
        break; /* le/be */
#if LJ_GC64
    case H_(9e89d2c9, 13c83c92):
        b = 1;
        break; /* gc64 */
#endif
    default:
    ...
    LJLIB_CF(ffi_metatype)
{
    ...
    TValue *tv;
    GCcdata *cd;
    if (!(ctype_isstruct(ct->info) || ctype_iscomplex(ct->info) || ctype_isvector(ct->info)))
        lj_err_arg(L, 1, LJ_ERR_FFI_INVTYPE);
    ...
    LJLIB_CF(ffi_gc) LJLIB_REC(.)
{
    ...
    CTState *cts = ctype_cts(L);
    CType *ct = ctype_raw(cts, cd->ctypeid);
    if (!(ctype_isptr(ct->info) || ctype_isstruct(ct->info) || ctype_isrefarray(ct->info)))
        lj_err_arg(L, 1, LJ_ERR_FFI_INVTYPE);
...
LJLIB_PUSH(top - 4)
LJLIB_SET(C) LJLIB_PUSH(top - 3) LJLIB_SET(os) LJLIB_PUSH(top - 2) LJLIB_SET(arch)
    ...
    static GCtab *ffi_finalizer(lua_State *L)
{
    ...
    setstrV(L, lj_tab_setstr(L, t, lj_str_newlit(L, "__mode")), lj_str_newlit(L, "k"));
