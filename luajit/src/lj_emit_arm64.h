...
static uint32_t emit_isk13(uint64_t n, int is64)
{
    ...
    if (n & 1) {
        ...
        w = 64;
        inv = 1;
    } /* Avoid wrap-around of ones. */
    if (!n)
        return 0; /* Neither all-zero nor all-ones are allowed. */
    do { /* Find the repeat width. */
        if (is64 && (uint32_t)(n ^ (n >> 32)))
            break;
        n = (uint32_t)n;
        if (!n)
            return 0; /* Ditto when passing n=0xffffffff and is64=0. */
        w = 32;
        if ((n ^ (n >> 16)) & 0xffff)
            break;
        n = n & 0xffff;
...
static void emit_dnma(ASMState *as, A64Ins ai, Reg rd, Reg rn, Reg rm, Reg ra) { *--as->mcp = ai | A64F_D(rd) | A64F_N(rn) | A64F_M(rm) | A64F_A(ra); }

static void emit_dnm(ASMState *as, A64Ins ai, Reg rd, Reg rn, Reg rm) { *--as->mcp = ai | A64F_D(rd) | A64F_N(rn) | A64F_M(rm); }

static void emit_dm(ASMState *as, A64Ins ai, Reg rd, Reg rm) { *--as->mcp = ai | A64F_D(rd) | A64F_M(rm); }

static void emit_dn(ASMState *as, A64Ins ai, Reg rd, Reg rn) { *--as->mcp = ai | A64F_D(rd) | A64F_N(rn); }

static void emit_nm(ASMState *as, A64Ins ai, Reg rn, Reg rm) { *--as->mcp = ai | A64F_N(rn) | A64F_M(rm); }

static void emit_d(ASMState *as, A64Ins ai, Reg rd) { *--as->mcp = ai | A64F_D(rd); }

static void emit_n(ASMState *as, A64Ins ai, Reg rn) { *--as->mcp = ai | A64F_N(rn); }
...
static void emit_lso(ASMState *as, A64Ins ai, Reg rd, Reg rn, int64_t ofs)
{
    ...
    if ((sc == 2 || sc == 3) && (!(ai & 0x400000) || rd != rn) && as->mcp != as->mcloop) {
        uint32_t prev = *as->mcp & ~A64F_D(31);
        int ofsm = ofs - (1 << sc), ofsp = ofs + (1 << sc);
        A64Ins aip;
        if (prev == (ai | A64F_N(rn) | A64F_U12(ofsm >> sc)) || prev == ((ai ^ A64I_LS_U) | A64F_N(rn) | A64F_S9(ofsm & 0x1ff))) {
            aip = (A64F_A(rd) | A64F_D(*as->mcp & 31));
        } else if (prev == (ai | A64F_N(rn) | A64F_U12(ofsp >> sc)) || prev == ((ai ^ A64I_LS_U) | A64F_N(rn) | A64F_S9(ofsp & 0x1ff))) {
        ...
        if (ofsm >= (int)((unsigned int)-64 << sc) && ofsm <= (63 << sc)) {
            *as->mcp = aip | A64F_N(rn) | ((ofsm >> sc) << 15) | (ai ^ ((ai == A64I_LDRx || ai == A64I_STRx) ? 0x50000000 : 0x90000000));
            return;
        }
    }
nopair:
...
static int emit_kdelta(ASMState *as, Reg rd, uint64_t k, int lim)
{
    ...
    if (lim <= 1)
        return 0; /* Can't beat that. */
    while (work) {
        Reg r = rset_picktop(work);
        IRRef ref = regcost_ref(as->cost[r]);
        lua_assert(r != rd);
        if (ref < REF_TRUE) {
            uint64_t kx = ra_iskref(ref) ? (uint64_t)ra_krefk(as, ref) : get_k64val(IR(ref));
...
static void emit_loadk(ASMState *as, Reg rd, uint64_t u64, int is64)
{
    ...
    if (k13) { /* Can the constant be represented as a bitmask immediate? */
        ...
        if (!is64)
            u64 = (int64_t)(int32_t)u64; /* Sign-extend. */
        ...
        if (!emit_kdelta(as, rd, u64, 4 - (neg ? ones : zeros))) {
            ...
            emit_d(as, (neg ? A64I_MOVNx : A64I_MOVZx) | A64F_U16((n64 >> lshift) & 0xffff) | A64F_LSL16(lshift), rd);
        }
    }
}
...
#define glofs(as, k) ((intptr_t)((uintptr_t)(k) - (uintptr_t)&J2GG(as->J)->g))
#define mcpofs(as, k) ((intptr_t)((uintptr_t)(k) - (uintptr_t)(as->mcp - 1)))
#define checkmcpofs(as, k) ((((mcpofs(as, k) >> 2) + 0x00040000) >> 19) == 0)
...
static void emit_loadk64(ASMState *as, Reg r, IRIns *ir)
{
    ...
    if (emit_checkofs(A64I_LDRx, ofs)) {
        emit_lso(as, r >= RID_MAX_GPR ? A64I_LDRd : A64I_LDRx, (r & 31), RID_GL, ofs);
    } else {
        if (r >= RID_MAX_GPR) {
...
#define emit_getgl(as, r, field) emit_lsptr(as, A64I_LDRx, (r), (void *)&J2G(as->J)->field)
#define emit_setgl(as, r, field) emit_lsptr(as, A64I_STRx, (r), (void *)&J2G(as->J)->field)
...
static void emit_call(ASMState *as, void *target)
{
    ...
    if ((((delta >> 2) + 0x02000000) >> 26) == 0) {
        *p = A64I_BL | ((uint32_t)(delta >> 2) & 0x03ffffffu);
        ...
        Reg r = ra_allock(as, i64ptr(target), RSET_RANGE(RID_X8, RID_MAX_GPR) - RSET_FIXED);
        *p = A64I_BLR | A64F_N(r);
    }
}
...
{
    if (dst >= RID_MAX_GPR) {
        emit_dn(as, irt_isnum(ir->t) ? A64I_FMOV_D : A64I_FMOV_S, (dst & 31), (src & 31));
        return;
    }
...
static void emit_opk(ASMState *as, A64Ins ai, Reg dest, Reg src, int32_t i, RegSet allow)
{
...
{
    if (ofs)
        emit_opk(as, ofs < 0 ? A64I_SUBx : A64I_ADDx, r, r, ofs < 0 ? -ofs : ofs, rset_exclude(RSET_GPR, r));
}
