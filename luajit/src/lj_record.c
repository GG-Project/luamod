...
static void rec_check_ir(jit_State *J)
{
    ...
    for (i = nk; i < nins; i++) {
        ...
        case IRMref:
            lua_assert(op1 >= nk);
            lua_assert(i >= REF_BIAS ? op1 < i : op1 > i);
            break;
        case IRMlit:
            break;
        case IRMcst:
            lua_assert(i < REF_BIAS);
        ...
        case IRMref:
            lua_assert(op2 >= nk);
            lua_assert(i >= REF_BIAS ? op2 < i : op2 > i);
            break;
        case IRMlit:
            break;
        case IRMcst:
    ...
    for (s = 0; s < nslots; s++) {
        ...
        if (tr) {
            ...
            if (s == 0) {
                ...
                lua_assert(tref_isfunc(tr));
                if (tref_isk(tr))
                    lua_assert(fn == ir_kfunc(ir));
                lua_assert(s > delta + LJ_FR2 ? (J->slot[s - delta] & TREF_FRAME) : (s == delta + LJ_FR2));
                depth++;
...
static TRef sload(jit_State *J, int32_t slot)
{
    ...
    TRef ref = emitir_raw(IRTG(IR_SLOAD, t), (int32_t)J->baseslot + slot, IRSLOAD_TYPECHECK);
    if (irtype_ispri(t))
        ref = TREF_PRI(t); /* Canonicalize primitive refs. */
    J->base[slot] = ref;
    return ref;
}
...
static TRef fori_load(jit_State *J, BCReg slot, IRType t, int mode)
{
    ...
    return sloadt(J, (int32_t)slot, t + (((mode & IRSLOAD_TYPECHECK) || (conv && t == IRT_INT && !(mode >> 16))) ? IRT_GUARD : 0), mode + conv);
}
...
*/
static int rec_for_direction(cTValue *o) { return (tvisint(o) ? intV(o) : (int32_t)o->u32.hi) >= 0; }
...
static void rec_for_loop(jit_State *J, const BCIns *fori, ScEvEntry *scev, int init)
{
    ...
    IRType t = idx ? tref_type(idx) : (init || LJ_DUALNUM) ? lj_opt_narrow_forl(J, tv) : IRT_NUM;
    int mode = IRSLOAD_INHERIT + ((!LJ_DUALNUM || tvisint(tv) == (t == IRT_INT)) ? IRSLOAD_READONLY : 0);
    ...
    tc = (LJ_DUALNUM && !(scev->start && irref_isk(scev->stop) && irref_isk(scev->step) && tvisint(&tv[FORL_IDX]) == (t == IRT_INT))) ? IRSLOAD_TYPECHECK : 0;
    if (tc) {
        J->base[ra + FORL_STOP] = stop;
        J->base[ra + FORL_STEP] = step;
    }
    if (!idx)
        idx = fori_load(J, ra + FORL_IDX, t, IRSLOAD_INHERIT + tc + (J->scev.start << 16));
    if (!init)
...
static LoopEvent rec_for(jit_State *J, const BCIns *fori, int isforl)
{
    ...
    if (isforl) { /* Handle FORL/JFORL opcodes. */
        ...
        BCReg i;
        lj_meta_for(J->L, tv);
        t = (LJ_DUALNUM || tref_isint(tr[FORL_IDX])) ? lj_opt_narrow_forl(J, tv) : IRT_NUM;
...
static int innerloopleft(jit_State *J, const BCIns *pc)
{
    ...
    for (i = 0; i < PENALTY_SLOTS; i++)
        if (mref(J->penalty[i].pc, const BCIns) == pc) {
            if ((J->penalty[i].reason == LJ_TRERR_LLEAVE || J->penalty[i].reason == LJ_TRERR_LINNER) && J->penalty[i].val >= 2 * PENALTY_MIN)
                return 1;
            break;
        }
    return 0;
    ...
    if (J->parent == 0 && J->exitno == 0) {
        if (pc == J->startpc && J->framedepth + J->retdepth == 0) {
            ...
            */
            ...
            if ((ev != LOOPEV_ENTERLO && J->loopref && J->cur.nins - J->loopref > 24) || --J->loopunroll < 0)
...
void lj_record_ret(jit_State *J, BCReg rbase, ptrdiff_t gotresults)
{
    ...
    if (J->framedepth == 0 && J->pt && bc_isret(bc_op(*J->pc)) && (!frame_islua(frame) || (J->parent == 0 && J->exitno == 0 && !bc_isret(bc_op(J->cur.startins))))) {
    ...
    if (frame_islua(frame)) { /* Return to Lua frame. */
        ...
        } else if (J->parent == 0 && J->exitno == 0 && !bc_isret(bc_op(J->cur.startins))) {
...
int lj_record_mm_lookup(jit_State *J, RecordIndex *ix, MMS mm)
{
    ...
    if (tref_istab(ix->tab)) {
        ...
        ix->mt = mix.tab = lj_ir_ggfload(J, IRT_TAB, GG_OFS(g.gcroot[GCROOT_BASEMT + itypemap(&ix->tabv)]));
#else
...
static TRef rec_mm_arith(jit_State *J, RecordIndex *ix, MMS mm)
{
    ...
    base[1 + LJ_FR2] = ix->tab;
    base[2 + LJ_FR2] = ix->key;
...
static TRef rec_mm_len(jit_State *J, TRef tr, TValue *tv)
{
    ...
    if (lj_record_mm_lookup(J, &ix, MM_len)) {
        ...
        basev += LJ_FR2;
        base[1] = tr;
        copyTV(J->L, basev + 1, tv);
#if LJ_52
        base[2] = tr;
        copyTV(J->L, basev + 2, tv);
#else
        base[2] = TREF_NIL;
        setnilV(basev + 2);
#endif
...
static void rec_mm_equal(jit_State *J, RecordIndex *ix, int op)
{
    ...
    if (lj_record_mm_lookup(J, ix, MM_eq)) { /* Lookup mm on 1st operand. */
        ...
        if (tvistab(bv) && tabref(tabV(bv)->metatable) == ix->mtv) {
            ...
            if (!lj_record_mm_lookup(J, ix, MM_eq) || lj_record_objcmp(J, mo1, ix->mobj, &mo1v, &ix->mobjv))
                return;
        }
...
static void rec_mm_comp(jit_State *J, RecordIndex *ix, int op)
{
    ...
    while (1) {
        ...
        if (lj_record_mm_lookup(J, ix, mm)) { /* Lookup mm on 1st operand. */
            ...
            if (tvistab(bv) && tabref(tabV(bv)->metatable) == ix->mtv) {
                ...
                ix->tab = ix->key;
                copyTV(J->L, &ix->tabv, bv);
                if (!lj_record_mm_lookup(J, ix, mm) || lj_record_objcmp(J, mo1, ix->mobj, &mo1v, &ix->mobjv))
                    goto nomatch;
            }
            rec_mm_callcomp(J, ix, op);
            return;
    ...
    nomatch:
        ...
        if (!(op & 2))
            break; /* Already at __lt. Interpreter will throw. */
        ix->tab = ix->key;
        ix->key = ix->val;
        ix->val = ix->tab;
        copyTV(J->L, &ix->tabv, &ix->keyv);
...
static void rec_idx_bump(jit_State *J, RecordIndex *ix)
{
    ...
    if (tref_ref(ix->tab) == rbc->ref) {
        ...
        if (ir->o == IR_TNEW) {
            ...
            if ((tb->asize > tpl->asize && (1u << nhbits) - 1 == tpl->hmask) || (tb->asize == tpl->asize && (1u << nhbits) - 1 > tpl->hmask)) {
...
static void rec_idx_abc(jit_State *J, TRef asizeref, TRef ikey, uint32_t asize)
{
    ...
    if ((J->flags & (JIT_F_OPT_LOOP | JIT_F_OPT_ABC)) == (JIT_F_OPT_LOOP | JIT_F_OPT_ABC)) {
        ...
        if (ref == J->scev.idx) {
            ...
            if ((uint64_t)stop + ofs < (uint64_t)asize) {
                ...
                emitir(IRTG(IR_ABC, IRT_P32), asizeref, ofs == 0 ? J->scev.stop : emitir(IRTI(IR_ADD), J->scev.stop, ofsref));
                /* Emit invariant bounds check for start, if not const or negative. */
                if (!(J->scev.dir && J->scev.start && (int64_t)IR(J->scev.start)->i + ofs >= 0))
...
static TRef rec_idx_key(jit_State *J, RecordIndex *ix, IRRef *rbref, IRType1 *rbguard)
{
    ...
    *rbref = 0;
    ...
    if (tref_isk(key)) {
        ...
        if (t->hmask > 0 && hslot <= t->hmask * (MSize)sizeof(Node) && hslot <= 65535 * (MSize)sizeof(Node)) {
            TRef node, kslot, hm;
...
TRef lj_record_idx(jit_State *J, RecordIndex *ix)
{
    ...
    if (ix->val == 0) { /* Indexed load */
        ...
        if (t == IRT_NIL && ix->idxchain && lj_record_mm_lookup(J, ix, MM_index))
            goto handlemm;
        if (irtype_ispri(t))
            res = TREF_PRI(t); /* Canonicalize primitives. */
        return res;
    } else { /* Indexed store. */
        ...
        if (tvisnil(oldv)) { /* Previous value was nil? */
            ...
            if (hasmm)
                ...
                emitir(IRTG(oldv == niltvg(J2G(J)) ? IR_EQ : IR_NE, IRT_PGC), xref, lj_ir_kkptr(J, niltvg(J2G(J))));
...
noconstify:
    ...
    if (!uvp->closed) {
        ...
        if (uvval(uvp) >= tvref(J->L->stack) && uvval(uvp) < tvref(J->L->maxstack)) {
            ...
            if (slot >= 0) { /* Aliases an SSA slot? */
                emitir(IRTG(IR_EQ, IRT_PGC), REF_BASE, emitir(IRT(IR_ADD, IRT_PGC), uref, lj_ir_kint(J, (slot - 1 - LJ_FR2) * -8)));
                slot -= (int32_t)J->baseslot; /* Note: slot number may be negative! */
                if (val == 0) {
                    return getslot(J, slot);
                } else {
                    J->base[slot] = val;
                    if (slot >= (int32_t)J->maxslot)
                        J->maxslot = (BCReg)(slot + 1);
                    return 0;
                }
            }
        }
        emitir(IRTG(IR_UGT, IRT_PGC), emitir(IRT(IR_SUB, IRT_PGC), uref, REF_BASE), lj_ir_kint(J, (J->baseslot + J->maxslot) * 8));
    } else {
        needbarrier = 1;
    ...
    if (val == 0) { /* Upvalue load */
        ...
        if (irtype_ispri(t))
            res = TREF_PRI(t); /* Canonicalize primitive refs. */
        return res;
    } else { /* Upvalue store. */
        /* Convert int to number before storing. */
...
static void check_call_unroll(jit_State *J, TraceNo lnk)
{
    ...
    int32_t count = 0;
    if ((J->pt->flags & PROTO_VARARG))
        depth--; /* Vararg frame still missing. */
        ...
        if (nresults >= 0) { /* Known fixed number of results. */
            ...
            if (nvararg > 0) {
                ...
                if (nvararg >= nresults)
                    ...
                    emitir(IRTGI(IR_EQ), fr, lj_ir_kint(J, (int32_t)frame_ftsz(J->L->base - 1)));
                vbase = emitir(IRT(IR_SUB, IRT_IGC), REF_BASE, fr);
                vbase = emitir(IRT(IR_ADD, IRT_PGC), vbase, lj_ir_kint(J, frofs - 8));
                for (i = 0; i < nload; i++) {
                    IRType t = itype2irt(&J->L->base[i - 1 - LJ_FR2 - nvararg]);
                    TRef aref = emitir(IRT(IR_AREF, IRT_PGC), vbase, lj_ir_kint(J, (int32_t)i));
                    TRef tr = emitir(IRTG(IR_VLOAD, t), aref, 0);
                    if (irtype_ispri(t))
                        tr = TREF_PRI(t); /* Canonicalize primitives. */
                    J->base[dst + i] = tr;
                }
            } else {
            ...
            if (idx != 0 && tref_isk(tridx)) {
                emitir(IRTGI(idx <= nvararg ? IR_GE : IR_LT), fr, lj_ir_kint(J, frofs + 8 * (int32_t)idx));
            ...
            if (idx != 0 && idx <= nvararg) {
                ...
                vbase = emitir(IRT(IR_ADD, IRT_PGC), vbase, lj_ir_kint(J, frofs - (8 << LJ_FR2)));
    ...
    if (tref_isnumber_str(top[0]) && tref_isnumber_str(top[-1])) {
        ...
        for (trp = top; trp >= base; trp--) {
            if (tref_isnumber(*trp))
                *trp = emitir(IRT(IR_TOSTR, IRT_STR), *trp, tref_isnum(*trp) ? IRTOSTR_NUM : IRTOSTR_INT);
        ...
        if (xbase == base)
            return tr; /* Return simple concatenation result. */
        /* Pass partial result. */
        topslot = J->maxslot--;
        *xbase = tr;
        top = xbase;
...
#define rcv (&ix.keyv)
    ...
    switch (bcmode_a(op)) {
    case BCMvar:
        copyTV(J->L, rav, &lbase[ra]);
        ix.val = ra = getslot(J, ra);
        break;
    default:
        break; /* Handled later. */
    }
    ...
    case BCMvar:
        copyTV(J->L, rbv, &lbase[rb]);
        ix.tab = rb = getslot(J, rb);
        break;
    default:
        break; /* Handled later. */
    }
    switch (bcmode_c(op)) {
    case BCMvar:
        copyTV(J->L, rcv, &lbase[rc]);
        ix.key = rc = getslot(J, rc);
        break;
    case BCMpri:
        setpriV(rcv, ~rc);
        ix.key = rc = TREF_PRI(IRT_NIL + rc);
        break;
    case BCMnum: {
        cTValue *tv = proto_knumtv(J->pt, rc);
        copyTV(J->L, rcv, tv);
        ix.key = rc = tvisint(tv) ? lj_ir_kint(J, intV(tv)) : lj_ir_knumint(J, numV(tv));
    } break;
    case BCMstr: {
        GCstr *s = gco2str(proto_kgc(J->pt, ~(ptrdiff_t)rc));
        setstrV(J->L, rcv, s);
        ix.key = rc = lj_ir_kstr(J, s);
    } break;
    default:
        break; /* Handled later. */
    }
    ...
    case BC_ISGT:
        ...
        if (!(tref_isk2(ra, rc) && tref_isnumber_str(ra) && tref_isnumber_str(rc))) {
            ...
            if (ta != tc) {
                ...
                } else if (!((ta == IRT_FALSE || ta == IRT_TRUE) && (tc == IRT_FALSE || tc == IRT_TRUE))) {
            ...
            if (ta == IRT_NUM) {
                if ((irop & 1))
                    irop ^= 4; /* ISGE/ISGT are unordered. */
                ...
                if (!lj_ir_strcmp(strV(rav), strV(rcv), (IROp)irop))
                    irop ^= 1;
    ...
    case BC_ISFC:
        ...
        /* fallthrough */
    case BC_IST:
    case BC_ISF: /* Type specialization suffices. */
    ...
    case BC_DIVVV: {
        ...
        if (tref_isnumber_str(rb) && tref_isnumber_str(rc))
            rc = lj_opt_narrow_arith(J, rb, rc, rbv, rcv, (int)mm - (int)MM_add + (int)IR_ADD);
        else
    ...
    case BC_TNEW:
        ...
        break;
    case BC_TDUP:
        rc = emitir(IRTG(IR_TDUP, IRT_TAB), lj_ir_ktab(J, gco2tab(proto_kgc(J->pt, ~(ptrdiff_t)rc))), 0);
#ifdef LUAJIT_ENABLE_TABLE_BUMP
...
void lj_record_setup(jit_State *J)
{
    ...
    if (J->parent) { /* Side trace. */
        ...
        if (J->exitno == 0 && T->snap[0].nent == 0) {
            ...
            if (J->pc > proto_bc(J->pt) && bc_op(J->pc[-1]) == BC_JFORI && bc_d(J->pc[bc_j(J->pc[-1]) - 1]) == root) {
                lj_snap_add(J);
        ...
        lj_snap_replay(J, T);
    sidecheck:
        if (traceref(J, J->cur.root)->nchild >= J->param[JIT_P_maxside] || T->snap[J->exitno].count >= J->param[JIT_P_hotexit] + J->param[JIT_P_tryside]) {
