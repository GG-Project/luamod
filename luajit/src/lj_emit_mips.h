...
static void emit_dst(ASMState *as, MIPSIns mi, Reg rd, Reg rs, Reg rt) { *--as->mcp = mi | MIPSF_D(rd) | MIPSF_S(rs) | MIPSF_T(rt); }

static void emit_dta(ASMState *as, MIPSIns mi, Reg rd, Reg rt, uint32_t a) { *--as->mcp = mi | MIPSF_D(rd) | MIPSF_T(rt) | MIPSF_A(a); }

#define emit_ds(as, mi, rd, rs) emit_dst(as, (mi), (rd), (rs), 0)
#define emit_tg(as, mi, rt, rg) emit_dst(as, (mi), (rg)&31, 0, (rt))

static void emit_tsi(ASMState *as, MIPSIns mi, Reg rt, Reg rs, int32_t i) { *--as->mcp = mi | MIPSF_T(rt) | MIPSF_S(rs) | (i & 0xffff); }

#define emit_ti(as, mi, rt, i) emit_tsi(as, (mi), (rt), 0, (i))
#define emit_hsi(as, mi, rh, rs, i) emit_tsi(as, (mi), (rh)&31, (rs), (i))

static void emit_fgh(ASMState *as, MIPSIns mi, Reg rf, Reg rg, Reg rh) { *--as->mcp = mi | MIPSF_F(rf & 31) | MIPSF_G(rg & 31) | MIPSF_H(rh & 31); }
...
#if LJ_64
static void emit_tsml(ASMState *as, MIPSIns mi, Reg rt, Reg rs, uint32_t msb, uint32_t lsb) { *--as->mcp = mi | MIPSF_T(rt) | MIPSF_S(rs) | MIPSF_M(msb) | MIPSF_L(lsb); }
#endif
...
static int emit_kdelta1(ASMState *as, Reg t, intptr_t i)
{
    ...
    while (work) {
        ...
        lua_assert(r != t);
        if (ref < ASMREF_L) {
            intptr_t delta = (intptr_t)((uintptr_t)i - (uintptr_t)(ra_iskref(ref) ? ra_krefk(as, ref) : get_kval(IR(ref))));
            if (checki16(delta)) {
...
#else
#define emit_loadk64(as, r, ir) emit_lsptr(as, MIPSI_LDC1, ((r)&31), (void *)&ir_knum((ir))->u64, RSET_GPR)
#endif

/* Get/set global_State fields. */
static void emit_lsglptr(ASMState *as, MIPSIns mi, Reg r, int32_t ofs) { emit_tsi(as, mi, r, RID_JGL, ofs - 32768); }

#define emit_getgl(as, r, field) emit_lsglptr(as, MIPSI_AL, (r), (int32_t)offsetof(global_State, field))
#define emit_setgl(as, r, field) emit_lsglptr(as, MIPSI_AS, (r), (int32_t)offsetof(global_State, field))
...
static void emit_call(ASMState *as, void *target, int needcfa)
{
    ...
    *--p = MIPSI_NOP;
    if ((((uintptr_t)target ^ (uintptr_t)p) >> 28) == 0) {
        *--p = (((uintptr_t)target & 1) ? MIPSI_JALX : MIPSI_JAL) | (((uintptr_t)target >> 2) & 0x03ffffffu);
...
#define emit_move(as, dst, src) emit_ds(as, MIPSI_MOVE, (dst), (src))
...
static void emit_loadofs(ASMState *as, IRIns *ir, Reg r, Reg base, int32_t ofs)
{
    if (r < RID_MAX_GPR)
        ...
        emit_tsi(as, irt_isnum(ir->t) ? MIPSI_LDC1 : MIPSI_LWC1, (r & 31), base, ofs);
}
...
static void emit_storeofs(ASMState *as, IRIns *ir, Reg r, Reg base, int32_t ofs)
{
    if (r < RID_MAX_GPR)
        ...
        emit_tsi(as, irt_isnum(ir->t) ? MIPSI_SDC1 : MIPSI_SWC1, (r & 31), base, ofs);
}
