...
LJLIB_CF(jit_on) { return setjitmode(L, LUAJIT_MODE_ON); }

LJLIB_CF(jit_off) { return setjitmode(L, LUAJIT_MODE_OFF); }
...
LJLIB_PUSH(top - 5)
LJLIB_SET(os) LJLIB_PUSH(top - 4) LJLIB_SET(arch) LJLIB_PUSH(top - 3) LJLIB_SET(version_num) LJLIB_PUSH(top - 2) LJLIB_SET(version)
...
static void setintfield(lua_State *L, GCtab *t, const char *name, int32_t val) { setintV(lj_tab_setstr(L, t, lj_str_newz(L, name)), val); }
...
static const char *const jit_trlinkname[] = { "none", "root", "loop", "tail-recursion", "up-recursion", "down-recursion", "interpreter", "return", "stitch" };
...
LJLIB_CF(jit_opt_start)
{
    ...
    if (nargs == 0) {
        ...
        for (i = 1; i <= nargs; i++) {
            ...
            if (!jitopt_level(J, str) && !jitopt_flag(J, str) && !jitopt_param(J, str))
...
LJLIB_CF(jit_profile_start)
{
    ...
    luaJIT_profile_start(L, mode ? strdata(mode) : "", (luaJIT_profile_callback)jit_profile_callback, L2);
    return 0;
}
...
#endif
    flags |= ver >= 70 ? JIT_F_ARMV7 : ver >= 61 ? JIT_F_ARMV6T2_ : ver >= 60 ? JIT_F_ARMV6_ : 0;
...
#endif
        if (x)
            flags |= JIT_F_MIPSXXR2; /* Either 0x80000000 (R2) or 0 (R1). */
    }
#endif
#endif
#else
...
#if LJ_HASPROFILE
    lj_lib_prereg(L, LUA_JITLIBNAME ".profile", luaopen_jit_profile, tabref(L->env));
#endif
