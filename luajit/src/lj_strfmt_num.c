...
#define RESCALE_EXPONENTS(P, N) P(308), P(289), P(270), P(250), P(231), P(212), P(193), P(173), P(154), P(135), P(115), P(96), P(77), P(58), P(38), P(0), P(0), P(0), N(39), N(58), N(77), N(96), N(116), N(135), N(154), N(174), N(193), N(212), N(231), N(251), N(270), N(289)
...
*/
static const int8_t four_ulp_m_e[] = { 34, -21, 68, -21, 14, -20, 28, -20, 55, -20, 2, -19, 3, -19, 5, -19, 9, -19, -82, -18, 35, -18, 7, -17, -117, -17, 28, -17, 56, -17, 112, -16, -33, -16, 45, -16, 89, -16, -78, -15, 36, -15, 72, -15, -113, -14, 29, -14, 57, -14, 114, -13, -28, -13, 46, -13, 91, -12, -74, -12, 37, -12, 73, -12, 15, -11, 3, -11, 59, -11, 2, -10, 3, -10, 5, -10, 1, -9, -69, -9, 38, -9, 75, -9, 15, -7, 3, -7, 6, -7, 12, -6, -17, -7, 48, -7, 96, -7, -65, -6, 39, -6, 77, -6, -103,
    -5, 31, -5, 62, -5, 123, -4, -11, -4, 49, -4, 98, -4, -60, -3, 4, -2, 79, -3, 16, -2, 32, -2, 63, -2, 2, -1, 25, 0, 5, 1, 1, 2, 2, 2, 4, 2, 8, 2, 16, 2, 32, 2, 64, 2, -128, 2, 26, 2, 52, 2, 103, 3, -51, 3, 41, 4, 82, 4, -92, 4, 33, 4, 66, 4, -124, 5, 27, 5, 53, 5, 105, 6, 21, 6, 42, 6, 84, 6, 17, 7, 34, 7, 68, 7, 2, 8, 3, 8, 6, 8, 108, 9, -41, 9, 43, 10, 86, 9, -84, 10, 35, 10, 69, 10, -118, 11, 28, 11, 55, 12, 11, 13, 22, 13, 44, 13, 88, 13, -80, 13, 36, 13, 71, 13, -115, 14, 29, 14, 57,
    14, 113, 15, -30, 15, 46, 15, 91, 15, 19, 16, 37, 16, 73, 16, 2, 17, 3, 17, 6, 17 };

/* min(2^32-1, 10^e-1) for e in range 0 through 10 */
static uint32_t ndigits_dec_threshold[] = { 0, 9U, 99U, 999U, 9999U, 99999U, 999999U, 9999999U, 99999999U, 999999999U, 0xffffffffU };
...
static uint32_t nd_mul2k(uint32_t *nd, uint32_t ndhi, uint32_t k, uint32_t carry_in, SFormat sf)
{
...
static int nd_similar(uint32_t *nd, uint32_t ndhi, uint32_t *ref, MSize hilen, MSize prec)
{
    char nd9[9], ref9[9];
    if (hilen <= prec) {
        if (LJ_UNLIKELY(nd[ndhi] != *ref))
            return 0;
        prec -= hilen;
        ref--;
        ndhi = (ndhi - 1) & 0x3f;
...
static char *lj_strfmt_wfnum(SBuf *sb, SFormat sf, lua_Number n, char *p)
{
    ...
    if (LJ_UNLIKELY((t.u32.hi << 1) >= 0xffe00000)) {
        ...
        *p++ = (char)ch;
        ...
        /* %a */
        const char *hexdig = (sf & STRFMT_F_UPPER) ? "0123456789ABCDEFPX" : "0123456789abcdefpx";
...
#error "ND_MUL2K_MAX_SHIFT too small"
        ...
        if ((sf & STRFMT_T_FP_E)) {
            ...
            if (ndebias) {
                ...
                */
                int32_t eidx = e + 70 + (ND_MUL2K_MAX_SHIFT < 29) + (t.u32.lo >= 0xfffffffe && !(~t.u32.hi << 12));
            ...
            if ((sf & STRFMT_T_FP_F)) {
                ...
                if ((int32_t)prec >= nde && nde >= -4) {
                    ...
                    if (prec >= maxprec)
                        ...
                        ndlo = (ndhi - (((int32_t)(prec - hilen) + 9) / 9)) & 0x3f;
            ...
            len = 3 + prec + (prefix != 0) + ndigits_dec((uint32_t)nde) + (nde < 10) + ((prec | (sf & STRFMT_F_ALT)) != 0);
            if (!p)
                p = lj_buf_more(sb, (width > len ? width : len) + 5);
            if (!(sf & (STRFMT_F_LEFT | STRFMT_F_ZERO))) {
                while (width-- > len)
                    *p++ = ' ';
            }
        ...
        g_format_like_f:
            ...
            len = ndhi * 9 + ndigits_dec(nd[ndhi]) + prec + (prefix != 0) + ((prec | (sf & STRFMT_F_ALT)) != 0);
            if (!p)
                p = lj_buf_more(sb, (width > len ? width : len) + 8);
            if (!(sf & (STRFMT_F_LEFT | STRFMT_F_ZERO))) {
                while (width-- > len)
                    *p++ = ' ';
            }
