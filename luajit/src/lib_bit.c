...
LJLIB_REC(bit_shift IR_BSHR) LJLIB_ASM_(bit_arshift) LJLIB_REC(bit_shift IR_BSAR) LJLIB_ASM_(bit_rol) LJLIB_REC(bit_shift IR_BROL) LJLIB_ASM_(bit_ror) LJLIB_REC(bit_shift IR_BROR)
...
LJLIB_ASM_(bit_bor)
LJLIB_REC(bit_nary IR_BOR) LJLIB_ASM_(bit_bxor) LJLIB_REC(bit_nary IR_BXOR)
    ...
    LJLIB_CF(bit_tohex) LJLIB_REC(.)
{
    ...
    int32_t n = L->base + 1 >= L->top ? (id ? 16 : 8) : (int32_t)lj_carith_check64(L, 2, &id2);
#else
