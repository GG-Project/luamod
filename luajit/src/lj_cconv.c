...
/* Bad conversion. */
LJ_NORET static void cconv_err_conv(CTState *cts, CType *d, CType *s, CTInfo flags)
{
    const char *dst = strdata(lj_ctype_repr(cts->L, ctype_typeid(cts, d), NULL));
    const char *src;
    if ((flags & CCF_FROMTV))
        src = lj_obj_typename[1 + (ctype_isnum(s->info) ? LUA_TNUMBER : ctype_isarray(s->info) ? LUA_TSTRING : LUA_TNIL)];
    else
...
LJ_NORET static void cconv_err_convtv(CTState *cts, CType *d, TValue *o, CTInfo flags)
{
    const char *dst = strdata(lj_ctype_repr(cts->L, ctype_typeid(cts, d), NULL));
...
void lj_cconv_ct_ct(CTState *cts, CType *d, CType *s, uint8_t *dp, uint8_t *sp, CTInfo flags)
{
    ...
    case CCX(P, P):
        if (!lj_cconv_compatptr(cts, d, s, flags))
            goto err_conv;
    ...
    case CCX(A, A):
        if ((flags & CCF_CAST) || (d->info & CTF_VLA) || dsize != ssize || d->size == CTSIZE_INVALID || !lj_cconv_compatptr(cts, d, s, flags))
            goto err_conv;
        goto copyval;
...
int lj_cconv_tv_ct(CTState *cts, CType *s, CTypeID sid, TValue *o, uint8_t *sp)
{
    ...
    if (ctype_isnum(sinfo)) {
        if (!ctype_isbool(sinfo)) {
            if (ctype_isinteger(sinfo) && s->size > 4)
                goto copyval;
            if (LJ_DUALNUM && ctype_isinteger(sinfo)) {
                int32_t i;
                lj_cconv_ct_ct(cts, ctype_get(cts, CTID_INT32), s, (uint8_t *)&i, sp, 0);
                if ((sinfo & CTF_UNSIGNED) && i < 0)
                    setnumV(o, (lua_Number)(uint32_t)i);
                else
                    setintV(o, i);
            } else {
                lj_cconv_ct_ct(cts, ctype_get(cts, CTID_DOUBLE), s, (uint8_t *)&o->n, sp, 0);
...
static void cconv_array_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t, CTInfo flags)
{
    int32_t i;
    ...
    for (i = 0;; i++) {
        ...
        if (!tv || tvisnil(tv)) {
            if (i == 0)
                continue; /* Try again for 1-based tables. */
...
static void cconv_substruct_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t, int32_t *ip, CTInfo flags)
{
    CTypeID id = d->sib;
    while (id) {
        ...
        if (ctype_isfield(df->info) || ctype_isbitfield(df->info)) {
            ...
            cconv_substruct_tab(cts, ctype_rawchild(cts, df), dp + df->size, t, ip, flags);
        } /* Ignore all other entries in the chain. */
    }
}

/* Convert table to struct/union. */
static void cconv_struct_tab(CTState *cts, CType *d, uint8_t *dp, GCtab *t, CTInfo flags)
{
    int32_t i = 0;
...
static void cconv_array_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp, TValue *o, MSize len)
{
    CType *dc = ctype_rawchild(cts, d); /* Array element type. */
...
static void cconv_substruct_init(CTState *cts, CType *d, uint8_t *dp, TValue *o, MSize len, MSize *ip)
{
    CTypeID id = d->sib;
    while (id) {
        CType *df = ctype_get(cts, id);
        ...
        if (ctype_isfield(df->info) || ctype_isbitfield(df->info)) {
            ...
            *ip = i + 1;
            ...
            cconv_substruct_init(cts, ctype_rawchild(cts, df), dp + df->size, o, len, ip);
            if ((d->info & CTF_UNION))
                break;
        } /* Ignore all other entries in the chain. */
    }
...
static void cconv_struct_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp, TValue *o, MSize len)
{
    MSize i = 0;
