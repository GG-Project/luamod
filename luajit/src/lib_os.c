...
{
    lua_pushnumber(L, difftime((time_t)(luaL_checknumber(L, 1)), (time_t)(luaL_optnumber(L, 2, (lua_Number)0))));
    return 1;
}
...
LJLIB_CF(os_setlocale)
{
    ...
    int opt = lj_lib_checkopt(L, 2, 6, "\5ctype\7numeric\4time\7collate\10monetary\1\377\3all");
    if (opt == 0)
        opt = LC_CTYPE;
    else if (opt == 1)
        opt = LC_NUMERIC;
