...
static AliasRet aa_escape(jit_State *J, IRIns *ir, IRIns *stop)
{
    ...
    for (ir++; ir < stop; ir++)
        if (ir->op2 == ref && (ir->o == IR_ASTORE || ir->o == IR_HSTORE || ir->o == IR_USTORE || ir->o == IR_FSTORE))
...
static AliasRet aa_ahref(jit_State *J, IRIns *refa, IRIns *refb)
{
    ...
    if (refa->o == IR_AREF) {
        ...
        lua_assert((refa->o == IR_HREF || refa->o == IR_HREFK || refa->o == IR_NEWREF) && (refb->o == IR_HREF || refb->o == IR_HREFK || refb->o == IR_NEWREF));
...
static TRef fwd_ahload(jit_State *J, IRRef xref)
{
    ...
    while (ref > xref) {
        ...
        case ALIAS_MAY:
            lim = ref;
            goto cselim; /* Limit search for load. */
        case ALIAS_MUST:
    ...
    /* No conflicting store (yet): const-fold loads from allocations. */
    {
        ...
        if (ir->o == IR_TNEW || (ir->o == IR_TDUP && irref_isk(xr->op2))) {
            ...
            */
            ...
            } else if (irt_isnum(fins->t) || (LJ_DUALNUM && irt_isint(fins->t)) || irt_isstr(fins->t)) {
                TValue keyv;
                cTValue *tv;
                IRIns *key = IR(xr->op2);
                if (key->o == IR_KSLOT)
...
static TRef fwd_aload_reassoc(jit_State *J)
{
    ...
    if (key->o == IR_ADD && irref_isk(key->op2)) {
        ...
        if (add2->o == IR_ADD && irref_isk(add2->op2) && IR(key->op2)->i == -IR(add2->op2)->i) {
...
{
    IRRef ref;
    if ((ref = fwd_ahload(J, fins->op1)) || (ref = fwd_aload_reassoc(J)))
        return ref;
    return EMITFOLD;
}
...
static int fwd_aa_tab_clear(jit_State *J, IRRef lim, IRRef ta)
{
    ...
    while (ref > lim) {
        IRIns *calls = IR(ref);
        if (calls->op2 == IRCALL_lj_tab_clear && (ta == calls->op1 || aa_table(J, ta, calls->op1) != ALIAS_NO))
            return 0; /* Conflict. */
        ref = calls->prev;
    }
...
cselim:
    ...
    while (ref > lim) {
        IRIns *ir = IR(ref);
        if (ir->op1 == uref || (IR(ir->op1)->op12 == IR(uref)->op12 && IR(ir->op1)->o == IR(uref)->o))
        ...
        case ALIAS_MUST: /* Store to the same location. */
            ...
            if (ref > J->chain[IR_LOOP]) { /* Quick check to avoid crossing LOOP. */
                ...
                *refp = store->prev;
                ...
                store->op1 = store->op2 = 0;
                store->prev = 0;
                if (ref + 1 < J->cur.nins && store[1].o == IR_OBAR && store[1].op1 == xref) {
...
static AliasRet aa_xref(jit_State *J, IRIns *refa, IRIns *xa, IRIns *xb)
{
    ...
    if (refa->o == IR_ADD && irref_isk(refa->op2)) {
        ...
        basea = IR(refa->op1);
        ofsa = (LJ_64 && irk->o == IR_KINT64) ? (ptrdiff_t)ir_k64(irk)->u64 : (ptrdiff_t)irk->i;
    }
    if (refb->o == IR_ADD && irref_isk(refb->op2)) {
        IRIns *irk = IR(refb->op2);
        baseb = IR(refb->op1);
        ofsb = (LJ_64 && irk->o == IR_KINT64) ? (ptrdiff_t)ir_k64(irk)->u64 : (ptrdiff_t)irk->i;
    }
    ...
    */
    ...
    if (!irt_sametype(xa->t, xb->t) && !(irt_typerange(xa->t, IRT_I8, IRT_U64) && ((xa->t.irt - IRT_I8) ^ (xb->t.irt - IRT_I8)) == 1))
        return ALIAS_NO;
...
static IRRef reassoc_xref(jit_State *J, IRIns *ir)
{
    ...
    if (ir->o == IR_ADD && irref_isk(ir->op2)) { /* Get constant offset. */
        IRIns *irk = IR(ir->op2);
        ofs = (LJ_64 && irk->o == IR_KINT64) ? (ptrdiff_t)ir_k64(irk)->u64 : (ptrdiff_t)irk->i;
        ir = IR(ir->op1);
    }
    if (ir->o == IR_ADD) { /* Add of base + index. */
        ...
        if (ir1 != ir && !(idxref = reassoc_trycse(J, ir1->o, idxref, ir1->o == IR_BSHL ? ir1->op2 : idxref)))
            return 0;
...
retry:
    if (J->chain[IR_CALLXS] > lim)
        lim = J->chain[IR_CALLXS];
    if (J->chain[IR_XBAR] > lim)
        lim = J->chain[IR_XBAR];
    while (ref > lim) {
        IRIns *store = IR(ref);
...
cselim:
    ...
    if (!(fins->op2 & IRXLOAD_READONLY) && J->chain[IR_LOOP] && xref == fins->op1 && (xref = reassoc_xref(J, xr)) != 0) {
        ref = J->chain[IR_XSTORE];
...
TRef LJ_FASTCALL lj_opt_dse_xstore(jit_State *J)
{
    ...
    if (J->chain[IR_XBAR] > lim)
        lim = J->chain[IR_XBAR];
    if (J->chain[IR_XSNEW] > lim)
        lim = J->chain[IR_XSNEW];
...
TRef LJ_FASTCALL lj_opt_fwd_tab_len(jit_State *J)
{
    ...
    if (J->chain[IR_ASTORE] > lim)
        lim = J->chain[IR_ASTORE];
