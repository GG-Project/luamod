...
#define CALLBACK_SLOT2OFS(slot) (CALLBACK_MCODE_HEAD + CALLBACK_MCODE_GROUP * ((slot) / 32) + 4 * (slot))
...
#define CALLBACK_MAX_SLOT (((CALLBACK_MCODE_SIZE - CALLBACK_MCODE_HEAD) / (CALLBACK_MCODE_GROUP + 4 * 32)) * 32)
...
static void *callback_slot2ptr(CTState *cts, MSize slot) { return (uint8_t *)cts->cb.mcode + CALLBACK_SLOT2OFS(slot); }
    ...
    for (slot = 0; slot < CALLBACK_MAX_SLOT; slot++) {
        ...
        *p++ = (uint8_t)slot;
        if ((slot & 31) == 31 || slot == CALLBACK_MAX_SLOT - 1) {
            ...
            *p++ = XI_GROUP5;
            *p++ = XM_OFS0 + (XOg_JMP << 3) + RID_EBP;
            *(int32_t *)p = (int32_t)(page - (p + 4));
            p += 4;
#else
            /* jmp lj_vm_ffi_callback. */
            *p++ = XI_JMP;
            *(int32_t *)p = target - (p + 4);
            p += 4;
#endif
        } else {
            *p++ = XI_JMPs;
            *p++ = (uint8_t)((2 + 2) * (31 - (slot & 31)) - 2);
        }
    }
    ...
    while (fid) {
        ...
        if (!ctype_isattrib(ctf->info)) {
            ...
            CALLBACK_HANDLE_REGARG /* Handle register arguments. */
                ...
                if (CCALL_ALIGN_STACKARG && LJ_32 && sz == 8) nsp
                = (nsp + 1) & ~1u; /* Align 64 bit argument on stack. */
            sp = &stack[nsp];
            nsp += n;
    ...
    if (!ctype_isvoid(ctr->info)) {
        ...
        if (ctype_isinteger_or_bool(ctr->info) && ctr->size < 4) {
            if (ctr->info & CTF_UNSIGNED)
                *(uint32_t *)dp = ctr->size == 1 ? (uint32_t) * (uint8_t *)dp : (uint32_t) * (uint16_t *)dp;
            else
                *(int32_t *)dp = ctr->size == 1 ? (int32_t) * (int8_t *)dp : (int32_t) * (int16_t *)dp;
        }
#if LJ_TARGET_MIPS64 || (LJ_TARGET_ARM64 && LJ_BE)
        /* Always sign-extend results to 64 bits. Even a soft-fp 'float'. */
        if (ctr->size <= 4 && (LJ_ABI_SOFTFP || ctype_isinteger_or_bool(ctr->info)))
...
static CType *callback_checkfunc(CTState *cts, CType *ct)
{
    ...
    if (ctype_isfunc(ct->info)) {
        ...
        CTypeID fid = ct->sib;
        if (!(ctype_isvoid(ctr->info) || ctype_isenum(ctr->info) || ctype_isptr(ctr->info) || (ctype_isnum(ctr->info) && ctr->size <= 8)))
            return NULL;
        ...
        while (fid) {
            ...
            if (!ctype_isattrib(ctf->info)) {
                ...
                lua_assert(ctype_isfield(ctf->info));
                cta = ctype_rawchild(cts, ctf);
                if (!(ctype_isenum(cta->info) || ctype_isptr(cta->info) || (ctype_isnum(cta->info) && cta->size <= 8)) || ++narg >= LUA_MINSTACK - 3)
                    return NULL;
            }
            fid = ctf->sib;
        }
