...
static IRIns *sink_checkalloc(jit_State *J, IRIns *irs)
{
    ...
    if (ir->o == IR_HREFK || ir->o == IR_AREF)
        ir = IR(ir->op1);
    else if (!(ir->o == IR_HREF || ir->o == IR_NEWREF || ir->o == IR_FREF || ir->o == IR_ADD))
...
static int sink_phidep(jit_State *J, IRRef ref)
{
    ...
    if (irt_isphi(ir->t))
        return 1;
    if (ir->op1 >= REF_FIRST && sink_phidep(J, ir->op1))
        return 1;
    if (ir->op2 >= REF_FIRST && sink_phidep(J, ir->op2))
        return 1;
    return 0;
}
...
static int sink_checkphi(jit_State *J, IRIns *ira, IRRef ref)
{
    if (ref >= REF_FIRST) {
        IRIns *ir = IR(ref);
        if (irt_isphi(ir->t) || (ir->o == IR_CONV && ir->op2 == IRCONV_NUM_INT && irt_isphi(IR(ir->op1)->t))) {
            ira->prev++;
...
static void sink_mark_ins(jit_State *J)
{
    ...
    for (ir = irlast;; ir--) {
...
#if LJ_HASFFI
        case IR_CNEWI:
            if (irt_isphi(ir->t) && (!sink_checkphi(J, ir, ir->op2) || (LJ_32 && ir + 1 < irlast && (ir + 1)->o == IR_HIOP && !sink_checkphi(J, ir, (ir + 1)->op2))))
        ...
        case IR_PHI: {
            ...
            if (irl->o == irr->o && (irl->o == IR_TNEW || irl->o == IR_TDUP || (LJ_HASFFI && (irl->o == IR_CNEW || irl->o == IR_CNEWI))))
                break;
            irt_setmark(irl->t);
            irt_setmark(irr->t);
            break;
...
static void sink_sweep_ins(jit_State *J)
{
    ...
    for (ir = IR(J->cur.nins - 1); ir >= irbase; ir--) {
        ...
        case IR_PHI: {
            IRIns *ira = IR(ir->op2);
            if (!irt_ismarked(ira->t) && (ira->o == IR_TNEW || ira->o == IR_TDUP || (LJ_HASFFI && (ira->o == IR_CNEW || ira->o == IR_CNEWI)))) {
                ir->prev = REGSP(RID_SINK, 0);
            } else {
                ir->prev = REGSP_INIT;
            }
...
{
    const uint32_t need = (JIT_F_OPT_SINK | JIT_F_OPT_FWD | JIT_F_OPT_DCE | JIT_F_OPT_CSE | JIT_F_OPT_FOLD);
    if ((J->flags & need) == need && (J->chain[IR_TNEW] || J->chain[IR_TDUP] || (LJ_HASFFI && (J->chain[IR_CNEW] || J->chain[IR_CNEWI])))) {
        if (!J->loopref)
