...
/* End-of-line? */
static LJ_AINLINE int cp_iseol(CPChar c) { return (c == '\n' || c == '\r'); }

/* Peek next raw character. */
static LJ_AINLINE CPChar cp_rawpeek(CPState *cp) { return (CPChar)(uint8_t)(*cp->p); }
...
static LJ_AINLINE void cp_save(CPState *cp, CPChar c) { lj_buf_putb(&cp->sb, c); }
...
LJ_NORET static void cp_errmsg(CPState *cp, CPToken tok, ErrMsg em, ...)
{
    ...
    if (tok == 0) {
        tokstr = NULL;
    } else if (tok == CTOK_IDENT || tok == CTOK_INTEGER || tok == CTOK_STRING || tok >= CTOK_FIRSTDECL) {
        if (sbufP(&cp->sb) == sbufB(&cp->sb))
            cp_save(cp, '$');
        cp_save(cp, '\0');
        tokstr = sbufB(&cp->sb);
    } else {
...
LJ_NORET LJ_NOINLINE static void cp_err_token(CPState *cp, CPToken tok) { cp_errmsg(cp, cp->tok, LJ_ERR_XTOKEN, cp_tok2str(cp, tok)); }
...
LJ_NORET LJ_NOINLINE static void cp_err(CPState *cp, ErrMsg em) { cp_errmsg(cp, 0, em); }
...
static LJ_NOINLINE CPToken cp_next(CPState *cp) { return (cp->tok = cp_next_(cp)); }

/* -- C parser ------------------------------------------------------------ */

/* Namespaces for resolving identifiers. */
#define CPNS_DEFAULT ((1u << CT_KW) | (1u << CT_TYPEDEF) | (1u << CT_FUNC) | (1u << CT_EXTERN) | (1u << CT_CONSTVAL))
...
static void cp_expr_prefix(CPState *cp, CPValue *k)
{
    if (cp->tok == CTOK_INTEGER) {
        *k = cp->val;
        ...
        cp_expr_unary(cp, k);
        k->id = lj_ctype_intern(cp->cts, CTINFO(CT_PTR, CTALIGN_PTR + k->id), CTSIZE_PTR);
...
static void cp_expr_postfix(CPState *cp, CPValue *k)
{
    for (;;) {
        ...
        if (cp_opt(cp, '[')) { /* Array/pointer index. */
            ...
            if (!ctype_isstruct(ct->info) || ct->size == CTSIZE_INVALID || !(fct = lj_ctype_getfield(cp->cts, ct, cp->str, &ofs)) || ctype_isbitfield(fct->info)) {
                GCstr *s = lj_ctype_repr(cp->cts->L, ctype_typeid(cp->cts, ct), NULL);
    ...
    for (;;) {
        ...
        case 10:
            if (cp_opt(cp, '*')) {
                ...
                if (k2.id > k->id)
                    k->id = k2.id; /* Trivial promotion to unsigned. */
                if (k2.u32 == 0 || (k->id == CTID_INT32 && k->u32 == 0x80000000u && k2.i32 == -1))
...
static CPDeclIdx cp_push(CPDecl *decl, CTInfo info, CTSize size) { return (decl->pos = cp_add(decl, info, size)); }
...
static void cp_push_attributes(CPDecl *decl)
{
    ...
    if (ctype_isfunc(ct->info)) { /* Ok to modify in-place. */
        ...
        if ((decl->fattr & CTFP_CCONV))
            ct->info = (ct->info & (CTMASK_NUM | CTF_VARARG | CTMASK_CID)) + (decl->fattr & ~CTMASK_CID);
#endif
    } else {
        if ((decl->attr & CTFP_ALIGNED) && !(decl->mode & CPARSE_MODE_FIELD))
            cp_push(decl, CTINFO(CT_ATTRIB, CTATTRIB(CTA_ALIGN)), ctype_align(decl->attr));
    }
}
...
static void cp_push_type(CPDecl *decl, CTypeID id)
{
    ...
    case CT_ENUM:
        ...
        if ((decl->attr & CTF_QUAL)) { /* Push unmerged qualifiers. */
            cp_push(decl, CTINFO(CT_ATTRIB, CTATTRIB(CTA_QUAL)), (decl->attr & CTF_QUAL));
            decl->attr &= ~CTF_QUAL;
        }
        break;
    case CT_ATTRIB:
                ...
                if (!(info & CTF_BOOL)) {
                    ...
                    if (vsize) { /* Vector size set? */
                        ...
                        if (vsize >= esize) {
                            ...
                            info = CTINFO(CT_ARRAY, (info & CTF_QUAL) + CTF_VECTOR + CTALIGN(vsize));
                        }
                    }
                }
...
static void cp_decl_mode(CPState *cp, CPDecl *decl)
{
    ...
    if (cp->tok == CTOK_IDENT) {
        ...
        if (*s == 'I' || *s == 'F') {
            ...
            if (vlen)
                CTF_INSERT(decl->attr, VSIZEP, lj_fls(vlen * sz));
        }
    bad_size:
        cp_next(cp);
    }
...
static void cp_decl_gccattribute(CPState *cp, CPDecl *decl)
{
    ...
    while (cp->tok != ')') {
        if (cp->tok == CTOK_IDENT) {
            ...
            cp_next(cp);
            switch (attrstr->hash) {
            case H_(64a9208e, 8ce14319):
            case H_(8e6331b2, 95a282af): /* aligned */
                cp_decl_align(cp, decl);
                break;
            case H_(42eb47de, f0ede26c):
            case H_(29f48a09, cf383e0c): /* packed */
                decl->attr |= CTFP_PACKED;
                break;
            case H_(0a84eef6, 8dfab04c):
            case H_(995cf92c, d5696591): /* mode */
                cp_decl_mode(cp, decl);
                break;
            case H_(0ab31997, 2d5213fa):
            case H_(bf875611, 200e9990): /* vector_size */
            {
                CTSize vsize = cp_decl_sizeattr(cp);
                if (vsize)
                    CTF_INSERT(decl->attr, VSIZEP, lj_fls(vsize));
            } break;
#if LJ_TARGET_X86
            case H_(5ad22db8, c689b848):
            case H_(439150fa, 65ea78cb): /* regparm */
                CTF_INSERT(decl->fattr, REGPARM, cp_decl_sizeattr(cp));
                decl->fattr |= CTFP_CCONV;
                break;
            case H_(18fc0b98, 7ff4c074):
            case H_(4e62abed, 0a747424): /* cdecl */
                CTF_INSERT(decl->fattr, CCONV, CTCC_CDECL);
                decl->fattr |= CTFP_CCONV;
                break;
            case H_(72b2e41b, 494c5a44):
            case H_(f2356d59, f25fc9bd): /* thiscall */
                CTF_INSERT(decl->fattr, CCONV, CTCC_THISCALL);
                decl->fattr |= CTFP_CCONV;
                break;
            case H_(0d0ffc42, ab746f88):
            case H_(21c54ba1, 7f0ca7e3): /* fastcall */
                CTF_INSERT(decl->fattr, CCONV, CTCC_FASTCALL);
                decl->fattr |= CTFP_CCONV;
                break;
            case H_(ef76b040, 9412e06a):
            case H_(de56697b, c750e6e1): /* stdcall */
                CTF_INSERT(decl->fattr, CCONV, CTCC_STDCALL);
                decl->fattr |= CTFP_CCONV;
                break;
            case H_(ea78b622, f234bd8e):
            case H_(252ffb06, 8d50f34b): /* sseregparm */
...
static void cp_struct_layout(CPState *cp, CTypeID sid, CTInfo sattr)
{
    ...
    while (fieldid) {
        ...
        if (ctype_isfield(ct->info) || (ctype_isxattrib(ct->info, CTA_SUBTYPE) && attr)) {
            ...
            if (sz >= 0x20000000u || bofs + csz < bofs || (info & CTF_VLA)) {
                if (!(sz == CTSIZE_INVALID && ctype_isarray(info) && !(sinfo & CTF_UNION)))
                    cp_err(cp, LJ_ERR_FFI_INVSIZE);
                csz = sz = 0; /* Treat a[] and a[?] as zero-sized. */
            }
            align = cp_field_align(cp, ct, info);
            if (((attr | sattr) & CTFP_PACKED) || ((attr & CTFP_ALIGNED) && ctype_align(attr) > align))
            ...
            if (bsz == CTBSZ_FIELD || !ctype_isfield(ct->info)) {
                ...
                if (bsz == 0 || (attr & CTFP_ALIGNED) || (!((attr | sattr) & CTFP_PACKED) && (bofs & amask) + bsz > csz))
                    bofs = (bofs + amask) & ~amask; /* Start new aligned field. */
                ...
                if (bsz == csz && (bofs & amask) == 0) {
                    ...
                    ct->info = CTINFO(CT_BITFIELD, (info & (CTF_QUAL | CTF_UNSIGNED | CTF_BOOL)) + (csz << (CTSHIFT_BITCSZ - 3)) + (bsz << CTSHIFT_BITBSZ));
#if LJ_BE
            ...
            for (;;) {
                ...
                if ((scl & CDF_STATIC)) { /* Static constant in struct namespace. */
                    ...
                    if (decl.bits == CTSIZE_INVALID) { /* Regular field. */
                        ...
                        if (!decl.name) {
                            if (!((ctype_isstruct(tct->info) && !(tct->info & CTF_VLA)) || ctype_isenum(tct->info)))
                                cp_err_token(cp, CTOK_IDENT);
                        ...
                        bsz = decl.bits;
                        if (!ctype_isinteger_or_bool(tct->info) || (bsz == 0 && decl.name) || 8 * tct->size > CTBSZ_MAX || bsz > ((tct->info & CTF_BOOL) ? 1 : 8 * tct->size))
    ...
    if (cp->tok != ')') {
        do {
            ...
            if (ctype_isvoid(ct->info))
                ...
                ctypeid = lj_ctype_intern(cp->cts, CTINFO(CT_PTR, CTALIGN_PTR | ctypeid), CTSIZE_PTR);
            /* Add new parameter. */
...
static void cp_declarator(CPState *cp, CPDecl *decl)
{
    ...
    if (cp_opt(cp, '(')) { /* Inner declarator. */
        ...
        if ((decl->mode & CPARSE_MODE_ABSTRACT) && (cp->tok == ')' || cp_istypedecl(cp)))
            goto func_decl;
        pos = decl->pos;
        cp_declarator(cp, decl);
        cp_check(cp, ')');
...
{
    cp_next(cp);
    if (cp->tok == CTOK_IDENT && cp->str->hash == H_(e79b999f, 42ca3e85)) { /* pack */
        cp_next(cp);
        cp_check(cp, '(');
        if (cp->tok == CTOK_IDENT) {
    ...
    while (cp->tok != CTOK_EOF) {
        ...
        if (cp->tok == '#') { /* Workaround, since we have no preprocessor, yet. */
            ...
            if (tok == CTOK_INTEGER) {
                cp_line(cp, hashline);
                continue;
            } else if (tok == CTOK_IDENT && cp->str->hash == H_(187aab88, fcb60b42)) { /* line */
                if (cp_next(cp) != CTOK_INTEGER)
                    cp_err_token(cp, tok);
                cp_line(cp, hashline);
                continue;
            } else if (tok == CTOK_IDENT && cp->str->hash == H_(f5e6b4f8, 1d509107)) { /* pragma */
