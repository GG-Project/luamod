...
#define emitconv(a, dt, st, flags) emitir(IRT(IR_CONV, (dt)), (a), (st) | ((dt) << 5) | (flags))
...
static CTypeID argv2ctype(jit_State *J, TRef tr, cTValue *o)
{
    if (tref_isstr(tr)) {
        ...
        return cd->ctypeid == CTID_CTYPEID ? crec_constructor(J, cd, tr) : cd->ctypeid;
    }
}
...
static void crec_fill(jit_State *J, TRef trdst, TRef trlen, TRef trfill, CTSize step)
{
    if (tref_isk(trlen)) { /* Length must be constant. */
        ...
        if (LJ_TARGET_UNALIGNED || step >= CTSIZE_PTR)
            step = CTSIZE_PTR;
        if (step * CREC_FILL_MAXUNROLL < len)
            goto fallback;
        ...
        if (ml[0].tp != IRT_U8) { /* Scatter U8 to U16/U32/U64. */
            if (CTSIZE_PTR == 8 && ml[0].tp == IRT_U64) {
                ...
                trfill = emitir(IRT(IR_MUL, IRT_U64), trfill, lj_ir_kint64(J, U64x(01010101, 01010101)));
            } else {
                trfill = emitir(IRTI(IR_MUL), trfill, lj_ir_kint(J, ml[0].tp == IRT_U16 ? 0x0101 : 0x01010101));
            }
        }
    ...
    */
    ...
    case CCX(B, F):
        if (st != IRT_CDATA) {
            ...
            TRef zero = (st == IRT_NUM || st == IRT_FLOAT) ? lj_ir_knum(J, 0) : (st == IRT_I64 || st == IRT_U64) ? lj_ir_kint64(J, 0) : lj_ir_kint(J, 0);
    ...
    conv_F_F:
        if (dt == IRT_CDATA || st == IRT_CDATA)
            goto err_nyi;
        if (dt != st)
            sp = emitconv(sp, dt, st, 0);
        goto xstore;

    /* Destination is a complex number. */
    case CCX(C, I):
    case CCX(C, F): { /* Clear im. */
    ...
    case CCX(P, F):
        ...
        sp = emitconv(sp, (LJ_64 && dsize == 8) ? IRT_I64 : IRT_U32, st, IRCONV_ANY);
        goto xstore;
...
#if LJ_64
        sp = emitir(IRT(IR_BAND, IRT_P64), sp, lj_ir_kint64(J, U64x(00007fff, ffffffff)));
#endif
...
static TRef crec_reassoc_ofs(jit_State *J, TRef tr, ptrdiff_t *ofsp, MSize sz)
{
    ...
    if (LJ_LIKELY(J->flags & JIT_F_OPT_FOLD) && irref_isk(ir->op2) && (ir->o == IR_ADD || ir->o == IR_ADDOV || ir->o == IR_SUBOV)) {
        IRIns *irk = IR(ir->op2);
        ptrdiff_t k;
...
static void crec_index_bf(jit_State *J, RecordFFData *rd, TRef ptr, CTInfo info)
{
    ...
    if (rd->data == 0) { /* __index metamethod. */
        ...
        CTState *cts = ctype_ctsG(J2G(J));
        CType *ct = ctype_get(cts, (info & CTF_BOOL) ? CTID_BOOL : (info & CTF_UNSIGNED) ? CTID_UINT32 : CTID_INT32);
        ...
        if (ctype_isstruct(ct->info)) {
            ...
            if (fct) {
                ...
                if (ctype_isconstval(fct->info)) {
                    if (fct->size >= 0x80000000u && (ctype_child(cts, fct)->info & CTF_UNSIGNED)) {
            ...
            if (name->len == 2 && ((strdata(name)[0] == 'r' && strdata(name)[1] == 'e') || (strdata(name)[0] == 'i' && strdata(name)[1] == 'm'))) {
                /* Always specialize to the field name. */
                emitir(IRTG(IR_EQ, IRT_STR), idx, lj_ir_kstr(J, name));
                if (strdata(name)[0] == 'i')
                    ofs += (ct->size >> 1);
...
static void crec_finalizer(jit_State *J, TRef trcd, TRef trfin, cTValue *fin)
{
    if (tvisgcv(fin)) {
        if (!trfin)
            trfin = lj_ir_kptr(J, gcval(fin));
    } else if (tvisnil(fin)) {
        trfin = lj_ir_kptr(J, NULL);
    } else {
        lj_trace_err(J, LJ_TRERR_BADTYPE);
    }
    lj_ir_call(J, IRCALL_lj_cdata_setfin, trcd, trfin, lj_ir_kint(J, (int32_t)itype(fin)));
    J->needsnap = 1;
}
...
static void crec_alloc(jit_State *J, RecordFFData *rd, CTypeID id)
{
    ...
    if (ctype_isptr(info) || (ctype_isinteger(info) && (sz == 4 || sz == 8))) {
        TRef sp = J->base[1] ? crec_ct_tv(J, d, 0, J->base[1], &rd->argv[1]) : ctype_isptr(info) ? lj_ir_kptr(J, NULL) : sz == 4 ? lj_ir_kint(J, 0) : (lj_needsplit(J), lj_ir_kint64(J, 0));
        ...
        if ((info & CTF_VLA)) { /* Calculate VLA/VLS size at runtime. */
            ...
            trsz = crec_ct_tv(J, ctype_get(cts, CTID_INT32), 0, J->base[1], &rd->argv[1]);
        ...
        special: /* Only handle bulk zero-fill for large/VLA/VLS types. */
        ...
        } else if (J->base[1] && !J->base[2] && !lj_cconv_multi_init(cts, d, &rd->argv[1])) {
            goto single_init;
            ...
            MSize i;
            tv.u64 = 0;
            if (!(ctype_isnum(dc->info) || ctype_isptr(dc->info)) || esize * CREC_FILL_MAXUNROLL < sz)
                goto special;
            for (i = 1, ofs = 0; ofs < sz; ofs += esize) {
                TRef dp = emitir(IRT(IR_ADD, IRT_PTR), trcd, lj_ir_kintp(J, ofs + sizeof(GCcdata)));
                if (J->base[i]) {
                    sp = J->base[i];
                    sval = &rd->argv[i];
                    i++;
            ...
            while (fid) {
                ...
                if (ctype_isfield(df->info)) {
                    ...
                    if (!(ctype_isnum(dc->info) || ctype_isptr(dc->info) || ctype_isenum(dc->info)))
                    ...
                    dp = emitir(IRT(IR_ADD, IRT_PTR), trcd, lj_ir_kintp(J, df->size + sizeof(GCcdata)));
                    crec_ct_tv(J, dc, dp, sp, sval);
                } else if (!ctype_isconstval(df->info)) {
...
static void crec_snap_caller(jit_State *J)
{
    ...
    if (!frame_islua(base - 1) || J->framedepth <= 0)
        lj_trace_err(J, LJ_TRERR_NYICALL);
    J->pc = frame_pc(base - 1);
    delta = 1 + LJ_FR2 + bc_a(J->pc[-1]);
    L->top = base;
    L->base = base - delta;
    J->base[-1 - LJ_FR2] = TREF_FALSE;
    J->base -= delta;
    J->baseslot -= (BCReg)delta;
    J->maxslot = (BCReg)delta - LJ_FR2;
    J->framedepth--;
    lj_snap_add(J);
    L->base = base;
    L->top = top;
...
static int crec_call(jit_State *J, RecordFFData *rd, GCcdata *cd)
{
    ...
    if (ctype_isfunc(ct->info)) {
        ...
        setlightudV(&tv, cdata_getptr(cdataptr(cd), (LJ_64 && tp == IRT_P64) ? 8 : 4));
        if (tvistrue(lj_tab_get(J->L, cts->miscmap, &tv)))
            lj_trace_err(J, LJ_TRERR_BLACKL);
        if (ctype_isvoid(ctr->info)) {
            t = IRT_NIL;
            rd->nres = 0;
        } else if (!(ctype_isnum(ctr->info) || ctype_isptr(ctr->info) || ctype_isenum(ctr->info)) || t == IRT_CDATA) {
            lj_trace_err(J, LJ_TRERR_NYICALL);
        }
        if ((ct->info & CTF_VARARG)
#if LJ_TARGET_X86
            ...
            func = emitir(IRT(IR_CARG, IRT_NIL), func, lj_ir_kint(J, ctype_typeid(cts, ct)));
        ...
        } else if (t == IRT_PTR || (LJ_64 && t == IRT_P32) || t == IRT_I64 || t == IRT_U64 || ctype_isenum(ctr->info)) {
...
static TRef crec_arith_int64(jit_State *J, TRef *sp, CType **s, MMS mm)
{
    if (sp[0] && sp[1] && ctype_isnum(s[0]->info) && ctype_isnum(s[1]->info)) {
        ...
        IROp op;
        lj_needsplit(J);
        if (((s[0]->info & CTF_UNSIGNED) && s[0]->size == 8) || ((s[1]->info & CTF_UNSIGNED) && s[1]->size == 8)) {
            dt = IRT_U64;
            id = CTID_UINT64;
        } else {
            dt = IRT_I64;
            id = CTID_INT64;
            if (mm < MM_add && !((s[0]->info | s[1]->info) & CTF_FP) && s[0]->size == 4 && s[1]->size == 4) { /* Try to narrow comparison. */
                if (!((s[0]->info ^ s[1]->info) & CTF_UNSIGNED) || (tref_isk(sp[1]) && IR(tref_ref(sp[1]))->i >= 0)) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (st == IRT_NUM || st == IRT_FLOAT)
                ...
                sp[i] = emitconv(sp[i], dt, IRT_INT, (s[i]->info & CTF_UNSIGNED) ? 0 : IRCONV_SEXT);
        }
        if (mm < MM_add) {
        comp:
...
static TRef crec_arith_ptr(jit_State *J, TRef *sp, CType **s, MMS mm)
{
    ...
    if (ctype_isptr(ctp->info) || ctype_isrefarray(ctp->info)) {
        if ((mm == MM_sub || mm == MM_eq || mm == MM_lt || mm == MM_le) && (ctype_isptr(s[1]->info) || ctype_isrefarray(s[1]->info))) {
        ...
        if (!((mm == MM_add || mm == MM_sub) && ctype_isnum(s[1]->info)))
            return 0;
    } else if (mm == MM_add && ctype_isnum(ctp->info) && (ctype_isptr(s[1]->info) || ctype_isrefarray(s[1]->info))) {
        TRef tr = sp[0];
        sp[0] = sp[1];
        sp[1] = tr; /* Swap pointer and index. */
        ctp = s[1];
    } else {
        return 0;
    }
    {
        ...
        if (t == IRT_NUM || t == IRT_FLOAT)
            ...
            tr = emitconv(tr, IRT_INTP, IRT_INT, ((t - IRT_I8) & 1) ? 0 : IRCONV_SEXT);
#else
        if (!tref_typerange(sp[1], IRT_I8, IRT_U32)) {
            tr = emitconv(tr, IRT_INTP, t, (t == IRT_NUM || t == IRT_FLOAT) ? IRCONV_ANY : 0);
        }
#endif
        tr = emitir(IRT(IR_MUL, IRT_INTP), tr, lj_ir_kintp(J, sz));
        tr = emitir(IRT(mm + (int)IR_ADD - (int)MM_add, IRT_PTR), sp[0], tr);
        id = lj_ctype_intern(cts, CTINFO(CT_PTR, CTALIGN_PTR | ctype_cid(ctp->info)), CTSIZE_PTR);
        return emitir(IRTG(IR_CNEWI, IRT_CDATA), lj_ir_kint(J, id), tr);
    }
}
...
void LJ_FASTCALL recff_cdata_arith(jit_State *J, RecordFFData *rd)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (!tr) {
            ...
            if (ctype_isptr(ct->info)) { /* Resolve pointer or reference. */
                ...
                ct = ctype_get(cts, lj_ctype_intern(cts, CTINFO(CT_PTR, CTALIGN_PTR | id), CTSIZE_PTR));
                goto ok;
            } else {
    ...
    {
        TRef tr;
        if (!(tr = crec_arith_int64(J, sp, s, (MMS)rd->data)) && !(tr = crec_arith_ptr(J, sp, s, (MMS)rd->data)) && !(tr = crec_arith_meta(J, sp, s, cts, rd)))
            return;
        J->base[0] = tr;
        /* Fixup cdata comparisons, too. Avoids some cdata escapes. */
        if (J->postproc == LJ_POST_FIXGUARD && frame_iscont(J->L->base - 1) && !irt_isguard(J->guardemit)) {
...
{
    CTState *cts = ctype_ctsG(J2G(J));
    if (tref_isudata(J->base[0]) && tref_isstr(J->base[1]) && udataV(&rd->argv[0])->udtype == UDTYPE_FFI_CLIB) {
        ...
        if (id && tv && !tvisnil(tv)) {
            ...
            if (ctype_isconstval(ct->info)) {
                if (ct->size >= 0x80000000u && (ctype_child(cts, ct)->info & CTF_UNSIGNED))
...
static TRef crec_toint(jit_State *J, CTState *cts, TRef sp, TValue *sval) { return crec_ct_tv(J, ctype_get(cts, CTID_INT32), 0, sp, sval); }

void LJ_FASTCALL recff_ffi_new(jit_State *J, RecordFFData *rd) { crec_alloc(J, rd, argv2ctype(J, J->base[0], &rd->argv[0])); }
...
void LJ_FASTCALL recff_ffi_typeof(jit_State *J, RecordFFData *rd)
{
    if (tref_iscdata(J->base[0])) {
        ...
        J->base[0] = emitir(IRTG(IR_CNEWI, IRT_CDATA), lj_ir_kint(J, CTID_CTYPEID), trid);
    } else {
...
static CTypeID crec_bit64_type(CTState *cts, cTValue *tv)
{
    if (tviscdata(tv)) {
        ...
        if (ctype_isenum(ct->info))
            ct = ctype_child(cts, ct);
        if ((ct->info & (CTMASK_NUM | CTF_BOOL | CTF_FP | CTF_UNSIGNED)) == CTINFO(CT_NUM, CTF_UNSIGNED) && ct->size == 8)
...
{
    CTState *cts = ctype_ctsG(J2G(J));
    TRef tr = crec_ct_tv(J, ctype_get(cts, CTID_INT64), 0, J->base[0], &rd->argv[0]);
...
int LJ_FASTCALL recff_bit64_nary(jit_State *J, RecordFFData *rd)
{
    ...
    for (i = 0; J->base[i] != 0; i++) {
        ...
        if (id < aid)
            id = aid; /* Determine highest type rank of all arguments. */
    }
    if (id) {
...
int LJ_FASTCALL recff_bit64_shift(jit_State *J, RecordFFData *rd)
{
    ...
    if (J->base[0] && tref_iscdata(J->base[1])) {
        tsh = crec_ct_tv(J, ctype_get(cts, CTID_INT64), 0, J->base[1], &rd->argv[1]);
    ...
    if (id) {
        ...
        uint32_t op = rd->data;
        if (!tsh)
            tsh = lj_opt_narrow_tobit(J, J->base[1]);
        if (!(op < IR_BROL ? LJ_TARGET_MASKSHIFT : LJ_TARGET_MASKROT) && !tref_isk(tsh))
...
TRef recff_bit64_tohex(jit_State *J, RecordFFData *rd, TRef hdr)
{
    ...
    if (id) {
        ...
        if (n < 16)
            tr = emitir(IRT(IR_BAND, IRT_U64), tr, lj_ir_kint64(J, ((uint64_t)1 << 4 * n) - 1));
    } else {
...
void LJ_FASTCALL lj_crecord_tonumber(jit_State *J, RecordFFData *rd)
{
    ...
    if (ctype_isnum(ct->info) || ctype_iscomplex(ct->info)) {
        if (ctype_isinteger_or_bool(ct->info) && ct->size <= 4 && !(ct->size == 4 && (ct->info & CTF_UNSIGNED)))
