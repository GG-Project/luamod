...
void		(*pfnClientCommand)			(edict_t* pEdict, const
char* szFmt, ...);
 */

/*
void		(*pfnMessageBegin)			(int msg_dest, int
msg_type, const float *pOrigin, edict_t *ed); void		(*pfnMessageEnd)
(void); void		(*pfnWriteByte)				(int iValue);
...
void		(*pfnCVarSetFloat)			(const char *szVarName,
float flValue); void		(*pfnCVarSetString)
...
/*
void		(*pfnClientPrintf)			( edict_t* pEdict,
PRINT_TYPE ptype, const char *szMsg );
 */
...
int lu_engfuncs::l_pfnClientPrintf(lua_State *L)
{
    ...
    if (print_type < 0 || print_type > 3)
        luaL_error(L, "Print type %i invalid!!!", print_type);
