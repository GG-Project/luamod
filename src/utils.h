...
inline char *ENTITY_KEYVALUE(edict_t *entity, const char *key) { return (INFOKEY_VALUE(GET_INFOKEYBUFFER(entity), key)); }

// Set a setinfo value for a player entity.
inline void ENTITY_SET_KEYVALUE(edict_t *entity, const char *key, char *value) { SET_CLIENT_KEYVALUE(ENTINDEX(entity), GET_INFOKEYBUFFER(entity), key, value); }

// Get a "serverinfo" value.
inline char *SERVERINFO(const char *key) { return (ENTITY_KEYVALUE(INDEXENT(0), key)); }

// Set a "serverinfo" value.
inline void SET_SERVERINFO(const char *key, char *value) { SET_SERVER_KEYVALUE(GET_INFOKEYBUFFER(INDEXENT(0)), key, value); }

// Get a "localinfo" value.
inline char *LOCALINFO(const char *key) { return (ENTITY_KEYVALUE(NULL, key)); }

// Set a "localinfo" value.
inline void SET_LOCALINFO(const char *key, char *value) { SET_SERVER_KEYVALUE(GET_INFOKEYBUFFER(NULL), key, value); }
