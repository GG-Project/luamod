...
char *Q_pretifymem(float value, int digitsafterdecimal)
{
    ...
    while (*i) {
        // if pos is still valid then insert a comma every third digit, except if we
        // would be putting one in the first spot
        if (pos >= 0 && !(pos % 3)) {
            // never in first spot
            if (o != out)
                *o++ = ',';
...
typedef struct memheader_s {
    ...
    size_t size; // size of the memory after the header (excluding header and
                 // sentinel2)
...
void *_Mem_Alloc(byte *poolptr, size_t size, qboolean clear, const char *filename, int fileline)
{
    ...
    if (size <= 0)
        return NULL;
    if (poolptr == NULL)
        PRINT_CONSOLE("Mem_Alloc: pool == NULL (alloc at %s:%i)\n", filename, fileline);
    pool->totalsize += size;
    ...
    if (mem == NULL)
        PRINT_CONSOLE("Mem_Alloc: out of memory (alloc at %s:%i)\n", filename,
            fileline); // SYS_ERROR HERE
    ...
    // we have to use only a single byte for this sentinel, because it may not be
    // aligned and some platforms can't use unaligned accesses
...
{
    if (data == NULL)
        PRINT_CONSOLE("Mem_Free: data == NULL (called at %s:%i)\n", filename, fileline);
...
byte *_Mem_AllocPool(const char *name, const char *filename, int fileline)
{
    ...
    if (pool == NULL)
        PRINT_CONSOLE("Mem_AllocPool: out of memory (allocpool at %s:%i)\n", filename, fileline);
...
void _Mem_FreePool(byte **poolptr, const char *filename, int fileline)
{
    ...
    if (pool) {
        ...
        if (*chainaddress != pool)
            PRINT_CONSOLE("Mem_FreePool: pool already free (freepool at %s:%i)\n", filename, fileline);
        if (pool->sentinel1 != MEMHEADER_SENTINEL1)
            PRINT_CONSOLE("Mem_FreePool: trashed pool sentinel 1 (allocpool at "
                          "%s:%i, freepool at %s:%i)\n",
                pool->filename, pool->fileline, filename, fileline);
        if (pool->sentinel2 != MEMHEADER_SENTINEL1)
            PRINT_CONSOLE("Mem_FreePool: trashed pool sentinel 2 (allocpool at "
                          "%s:%i, freepool at %s:%i)\n",
                pool->filename, pool->fileline, filename, fileline);
...
void _Mem_EmptyPool(byte *poolptr, const char *filename, int fileline)
{
    mempool_t *pool = (mempool_t *)poolptr;
    if (poolptr == NULL)
        PRINT_CONSOLE("Mem_EmptyPool: pool == NULL (emptypool at %s:%i)\n", filename, fileline);

    if (pool->sentinel1 != MEMHEADER_SENTINEL1)
        PRINT_CONSOLE("Mem_EmptyPool: trashed pool sentinel 1 (allocpool at %s:%i, "
                      "emptypool at %s:%i)\n",
            pool->filename, pool->fileline, filename, fileline);
    if (pool->sentinel2 != MEMHEADER_SENTINEL1)
        PRINT_CONSOLE("Mem_EmptyPool: trashed pool sentinel 2 (allocpool at %s:%i, "
                      "emptypool at %s:%i)\n",
            pool->filename, pool->fileline, filename, fileline);
...
void Mem_CheckHeaderSentinels(void *data, const char *filename, int fileline)
{
    ...
    if (mem->sentinel1 != MEMHEADER_SENTINEL1) {
        mem->filename = Mem_CheckFilename(mem->filename); // make sure what we don't crash var_args
        PRINT_CONSOLE("Mem_CheckSentinels: trashed header sentinel 1 (block "
                      "allocated at %s:%i, sentinel check at %s:%i)\n",
            mem->filename, mem->fileline, filename, fileline);
    }

    if (*((byte *)mem + sizeof(memheader_t) + mem->size) != MEMHEADER_SENTINEL2) {
        mem->filename = Mem_CheckFilename(mem->filename); // make sure what we don't crash var_args
        PRINT_CONSOLE("Mem_CheckSentinels: trashed header sentinel 2 (block "
                      "allocated at %s:%i, sentinel check at %s:%i)\n",
            mem->filename, mem->fileline, filename, fileline);
    }
}
...
void _Mem_Check(const char *filename, int fileline)
{
    ...
    for (pool = poolchain; pool; pool = pool->next) {
        if (pool->sentinel1 != MEMHEADER_SENTINEL1)
            PRINT_CONSOLE("Mem_CheckSentinelsGlobal: trashed pool sentinel 1 "
                          "(allocpool at %s:%i, sentinel check at %s:%i)\n",
                pool->filename, pool->fileline, filename, fileline);
        if (pool->sentinel2 != MEMHEADER_SENTINEL1)
            PRINT_CONSOLE("Mem_CheckSentinelsGlobal: trashed pool sentinel 2 "
                          "(allocpool at %s:%i, sentinel check at %s:%i)\n",
                pool->filename, pool->fileline, filename, fileline);
    }
...
void Mem_PrintList(size_t minallocationsize)
{
    ...
    for (pool = poolchain; pool; pool = pool->next) {
        ...
        if (changed_size != 0) {
            ...
            PRINT_CONSOLE("%10s (%10s actual) %s (^7%c%s change)\n", Q_memprint(pool->totalsize), Q_memprint(pool->realsize), pool->name, sign, Q_memprint(abs(changed_size)));
        } else {
