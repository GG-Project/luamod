...
void *_Mem_Alloc( byte *poolptr, size_t size, qboolean clear, const char *filename, int fileline )
{
	...
	if( poolptr == NULL ) PRINT_CONSOLE( "Mem_Alloc: pool == NULL (alloc at %s:%i)\n", filename, fileline );
	pool->totalsize += size;

	// big allocations are not clumped
	pool->realsize += sizeof( memheader_t ) + size + sizeof( int );
	mem = (memheader_t *)malloc( sizeof( memheader_t ) + size + sizeof( int ));
	if( mem == NULL ) PRINT_CONSOLE( "Mem_Alloc: out of memory (alloc at %s:%i)\n", filename, fileline ); //SYS_ERROR HERE
...
static void Mem_FreeBlock( memheader_t *mem, const char *filename, int fileline )
{
	...
	if( mem->sentinel1 != MEMHEADER_SENTINEL1 )
	{
		...
		PRINT_CONSOLE( "Mem_Free: trashed header sentinel 1 (alloc at %s:%i, free at %s:%i)\n", mem->filename, mem->fileline, filename, fileline );
	}

	if( *((byte *)mem + sizeof( memheader_t ) + mem->size ) != MEMHEADER_SENTINEL2 )
	{	
		mem->filename = Mem_CheckFilename( mem->filename ); // make sure what we don't crash var_args
		PRINT_CONSOLE( "Mem_Free: trashed header sentinel 2 (alloc at %s:%i, free at %s:%i)\n", mem->filename, mem->fileline, filename, fileline );
	}

	pool = mem->pool;
	// unlink memheader from doubly linked list
	if(( mem->prev ? mem->prev->next != mem : pool->chain != mem ) || ( mem->next && mem->next->prev != mem ))
		PRINT_CONSOLE( "Mem_Free: not allocated or double freed (free at %s:%i)\n", filename, fileline );
...
{
	if( data == NULL ) PRINT_CONSOLE( "Mem_Free: data == NULL (called at %s:%i)\n", filename, fileline );
...
void *_Mem_Realloc( byte *poolptr, void *memptr, size_t size, qboolean clear, const char *filename, int fileline )
{
	...
	nb = (char*)_Mem_Alloc( poolptr, size, clear, filename, fileline );
...
byte *_Mem_AllocPool( const char *name, const char *filename, int fileline )
{
	...
	if( pool == NULL ) PRINT_CONSOLE( "Mem_AllocPool: out of memory (allocpool at %s:%i)\n", filename, fileline );
...
void _Mem_FreePool( byte **poolptr, const char *filename, int fileline )
{
	...
	if( pool )
	{
		...
		if( *chainaddress != pool ) PRINT_CONSOLE( "Mem_FreePool: pool already free (freepool at %s:%i)\n", filename, fileline );
		if( pool->sentinel1 != MEMHEADER_SENTINEL1 ) PRINT_CONSOLE( "Mem_FreePool: trashed pool sentinel 1 (allocpool at %s:%i, freepool at %s:%i)\n", pool->filename, pool->fileline, filename, fileline );
		if( pool->sentinel2 != MEMHEADER_SENTINEL1 ) PRINT_CONSOLE( "Mem_FreePool: trashed pool sentinel 2 (allocpool at %s:%i, freepool at %s:%i)\n", pool->filename, pool->fileline, filename, fileline );
...
{
	mempool_t *pool = (mempool_t *)poolptr;
	if( poolptr == NULL ) PRINT_CONSOLE( "Mem_EmptyPool: pool == NULL (emptypool at %s:%i)\n", filename, fileline );

	if( pool->sentinel1 != MEMHEADER_SENTINEL1 ) PRINT_CONSOLE( "Mem_EmptyPool: trashed pool sentinel 1 (allocpool at %s:%i, emptypool at %s:%i)\n", pool->filename, pool->fileline, filename, fileline );
	if( pool->sentinel2 != MEMHEADER_SENTINEL1 ) PRINT_CONSOLE( "Mem_EmptyPool: trashed pool sentinel 2 (allocpool at %s:%i, emptypool at %s:%i)\n", pool->filename, pool->fileline, filename, fileline );
...
void Mem_CheckHeaderSentinels( void *data, const char *filename, int fileline )
{
	...
	if( data == NULL )
		PRINT_CONSOLE( "Mem_CheckSentinels: data == NULL (sentinel check at %s:%i)\n", filename, fileline );
	...
	{
		mem->filename = Mem_CheckFilename( mem->filename ); // make sure what we don't crash var_args
		PRINT_CONSOLE( "Mem_CheckSentinels: trashed header sentinel 1 (block allocated at %s:%i, sentinel check at %s:%i)\n", mem->filename, mem->fileline, filename, fileline );
	}

	if( *((byte *) mem + sizeof(memheader_t) + mem->size) != MEMHEADER_SENTINEL2 )
	{	
		mem->filename = Mem_CheckFilename( mem->filename ); // make sure what we don't crash var_args
		PRINT_CONSOLE( "Mem_CheckSentinels: trashed header sentinel 2 (block allocated at %s:%i, sentinel check at %s:%i)\n", mem->filename, mem->fileline, filename, fileline );
	}
}
...
void _Mem_Check( const char *filename, int fileline )
{
	...
	for( pool = poolchain; pool; pool = pool->next )
	{
		if( pool->sentinel1 != MEMHEADER_SENTINEL1 )
			PRINT_CONSOLE( "Mem_CheckSentinelsGlobal: trashed pool sentinel 1 (allocpool at %s:%i, sentinel check at %s:%i)\n", pool->filename, pool->fileline, filename, fileline );
		if( pool->sentinel2 != MEMHEADER_SENTINEL1 )
			PRINT_CONSOLE( "Mem_CheckSentinelsGlobal: trashed pool sentinel 2 (allocpool at %s:%i, sentinel check at %s:%i)\n", pool->filename, pool->fileline, filename, fileline );
	}
...
void Mem_PrintStats( void )
{
	...
	PRINT_CONSOLE( "^3%lu^7 memory pools, totalling: ^1%s\n", count, Q_memprint( size ));
	PRINT_CONSOLE( "total allocated size: ^1%s\n", Q_memprint( realsize ));
}
...
void Mem_PrintList( size_t minallocationsize )
{
	...
	PRINT_CONSOLE( "memory pool list:\n""  ^3size                          name\n");
	for( pool = poolchain; pool; pool = pool->next )
	{
